...
- `ControllerPromptStrip` presents the active physical-button family. `GameShellMenuController` owns controller traversal of the existing top-menu headings and invokes their existing route, command, and system owners. The shell mounts one shared `ControllerRadialOverlay`; interaction actions retain their active component owner and shell action entries come from `GameShellCommandController`.
