...
- `CombatMacroSpells`, reached through `CombatFlowMagic`, resolves immediate and queued area and side-group spells around a retained macro source without action costs, activation advancement, or death-macro completion. A queued area rolls and creates its persistent field at the source center before its immediate resolution, using the shared field owner and event contract. It shares ordinary spell effects and lifecycle cleanup, including nested defeated-monster macro queuing; repeated-target, single-target, and ray signatures remain separate until their macro-specific Castle behavior is adjudicated. Dead sources are anchors, never targets; complete living footprints are deduplicated in party-then-monster order. Macro source context, continuation, and completion remain scenario/playthrough-owned.
