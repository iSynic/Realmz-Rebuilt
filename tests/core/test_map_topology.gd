...
func run() -> void:
	...
	var no_ids: Array[String] = []; var no_features: Array[MapFeature] = []; var no_edges := {&"north": MapEdge.new(&"open", true, false), &"east": MapEdge.new(&"open", true, false), &"south": MapEdge.new(&"open", true, false), &"west": MapEdge.new(&"open", true, false)}; var los_cells: Array[MapCell] = [MapCell.new("los:0", Vector2i.ZERO, "classic.terrain.1", true, 1, false, true, false, false, false, false, false, 0, 1, "fixture.tileset", no_ids, no_ids, no_edges, no_features), MapCell.new("los:1", Vector2i(1, 0), "classic.terrain.2", false, 1, true, true, false, false, false, false, false, 0, 2, "fixture.tileset", no_ids, no_ids, no_edges, no_features), MapCell.new("los:2", Vector2i(2, 0), "classic.terrain.3", true, 1, false, true, false, false, false, false, false, 0, 3, "fixture.tileset", no_ids, no_ids, no_edges, no_features)]; var los_map := MapDefinition.new("los", "LOS", &"land", 0, MapTopology.new(3, 1, los_cells), false, true); var los_content := RealmzContent.new("los-fixture", "0".repeat(64), "los-content", "realmz-classic-1", los_map.id, Vector2i.ZERO, WorldDefinition.new([los_map]), ScenarioDefinition.new([], []), [], [], [], loaded.content.race_definitions(), loaded.content.caste_definitions()); var los_session := GameSession.new(); assert_equal(los_session.start(los_content, 17).state, SessionStep.State.COMPLETED, "the public session starts on the synthetic LOS map")
	var los_view := los_session.view().map_view; assert_equal([los_view.uses_los, los_view.cell_at(Vector2i(1, 0)).visible, los_view.cell_at(Vector2i(2, 0)).visible, los_view.seen_coordinates(), los_map.topology.visible_cells(Vector2i.ZERO, 8, WorldState.new(), true, true)], [true, true, false, [Vector2i.ZERO, Vector2i(1, 0)], [Vector2i.ZERO, Vector2i(1, 0), Vector2i(2, 0)]], "the blocking tile enters exact saved sight memory, the sealed tile beyond stays black, and bounded Wizard's Eye ignores only occluders")
