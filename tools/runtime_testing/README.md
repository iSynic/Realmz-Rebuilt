...
Supported engine commands are `describe`, `observe`, `checkpoint`, `capture`, `restore`, `act`, `respond`, `ui`, `invoke`, and `close`. Direct invocation can prepare exact isolated AP, encounter, Battle, Treasure, and Shop surfaces for subsequent actual-input proof. Fixture lifecycle operations are narrow and explicit; unsupported future fixture names return an explicit error.
