...
$castleRoot = ""
$uiDonorRoot = ""
$destinationRoot = Join-Path $repoRoot "src/ui/assets/classic-controls"
$manifestPath = Join-Path $repoRoot "src/ui/assets/classic-ui-assets.json"
...
try {
    ...
    foreach ($entry in $catalog.assets) {
        ...
        $record = [ordered]@{
            id = $entry.id
            path = "res://src/ui/assets/classic-controls/$($entry.target_path)"
