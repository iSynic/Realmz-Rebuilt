...
- The source-backed Prelude corrections and successive compiler regenerations are recorded in `prelude-to-pestilence.corrections.json`. Providence must preserve the authored XAP 97 refusal branch, XAP 149 no-gold return branch, Simple Encounter 7 opcode 44 context, and reschedulable timed row 0 with XAP 303 from the pinned Castle records; the package and provenance catalog carry the current source/compiler/application/package identities and prior archive identities. Keep these corrections revision-guarded and do not treat a missing branch target as a VM fallback.
- The source-backed Wrath correction and compiler regeneration are recorded in `wrath-of-the-mind-lords.corrections.json`. Providence must derive player-map portrait and negative special-land marker identities from the exact application media catalog resource kind/key; preserve the native CICN IDs and bytes, and do not repair this through Rebuilt fallback inference or archive edits.
...
- The scenario-media refresh records each prior lean package identity, exact native `Scenario.rsrc` hash, accepted application-library identity, restored descriptor count, and any native key that remained unrefreshable. It retains every uniquely decoded scenario-owned CICN, including base and right-facing monster resources omitted by an earlier index; visually equivalent PNG compression and ambiguous native occurrences remain unchanged.

## Verification

- `tools/verify_bundled_scenarios.ps1` checks the exact archive set, file bytes, SHA-256 values, manifest identities, Providence compiler and application-library revisions, scenario-media refresh provenance and pinned totals, scenario-only definition/media inventories, reduced combined footprint, authored player-map titles, offset-preserving scrolling `TEXT`/`styl` pairs, effective application-plus-scenario asset resolution and base-plus-overlay separation for every emitted land CICN, source-backed Prelude and Wrath correction catalogs, City of Bywater source-snapshot identity, its dormant Ranthog Action Point preservation, and its crypt-door Encounter prompt/actions.
