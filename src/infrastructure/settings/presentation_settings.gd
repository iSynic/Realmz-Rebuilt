...
static func from_data(data: Variant) -> PresentationSettings:
	...
	if float(schema_version) != float(schema_value):
		return null
	if data.get("kind") != "realmz2.presentation-settings" or schema_version not in [1, 2, 3, SCHEMA_VERSION]:
		return null
	...
	if schema_version >= 3:
		...
		if data["windowMode"] not in [WINDOWED, BORDERLESS_FULLSCREEN]:
			return null
	if schema_version == SCHEMA_VERSION and not data.get("autoSwitchToMelee") is bool:
		return null
