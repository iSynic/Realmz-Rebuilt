...
- Original controls use nearest-neighbor filtering at exact 1x/2x only. Fonts ship with pinned source, hashes, and licenses and introduce no runtime network dependency. `Theldrow-Classic.fnt` and its atlas are deterministic, metric-preserving conversions of licensed Castle `FONT` 1601, not a lookalike vector font; keep the source strike monochrome and nearest-neighbor sampled. Black Chancery, ChicagoFLF, and Castle's Inter Geneva substitute retain their exact pinned bytes. Alegreya remains the readable-mode fallback family.
