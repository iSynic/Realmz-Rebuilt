...
func run(command: SessionDebugCommand) -> SessionCoordinatorResult:
	if command == null or not _context.state.party_setup_completed:
		return _context.failed(&"debug_command_unavailable", "Debug commands require a committed active adventure boundary.")
	...
	return _context.failed(&"debug_command_unknown", "The debug command is unknown.")


func _start_battle(classic_id: int) -> SessionCoordinatorResult:
	if _context.state.combat != null:
		return _context.failed(&"debug_battle_active", "A battle is already active.")
	var battle := _context.content.battle_by_classic_id(classic_id)
	if battle == null:
		return _context.failed(&"debug_battle_unknown", "Battle %d is unavailable." % classic_id)
...
func _win_battle() -> SessionCoordinatorResult:
	if _context.state.combat == null or _context.state.combat.completed:
		return _context.failed(&"debug_battle_unavailable", "There is no active battle to win.")
	...
	if not _context.rules.combat_flow.finish_debug_victory(_context.state, _context.content, events):
		return _context.failed(&"debug_victory_failed", "The active battle could not resolve as a victory.")
...
func _start_encounter(kind: StringName, classic_id: int) -> SessionCoordinatorResult:
	if _context.state.combat != null or kind not in [&"simple", &"complex"]:
		return _context.failed(&"debug_encounter_unavailable", "A Simple or Complex Encounter requires exploration.")
	...
	if not available:
		return _context.failed(&"debug_encounter_unknown", "%s Encounter %d is unavailable." % [String(kind).capitalize(), classic_id])
	...
	return _context.failed(result.error_code, result.error_message, events) if result.state == ScenarioVmResult.State.FAILED else _context.completed(events)
...
func _workflow(result: SessionWorkflowResult) -> SessionCoordinatorResult:
	if result == null:
		return _context.failed(&"invalid_workflow_result", "The debug workflow returned no result.")
	return _context.completed(result.events) if result.ok else _context.failed(result.error_code, result.error_message)
