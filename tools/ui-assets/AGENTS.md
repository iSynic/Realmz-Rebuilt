...
- The complete app-owned stock landlook `PICT` set, Classic `snd ` catalog, shared Data ID and target-campaign stock-supply item `cicn` sets and unidentified substitutes, plus selected source-backed battle `cicn` families decoded from the pinned Castle `The Family Jewels` resource fork.
...
- Application-media regeneration replaces only the catalog-owned `land-tilesets`, `sounds`, `item-icons`, and `combat-icons` directories. It must preserve separately generated stock music and any future sibling media banks under `classic-media`.
- Audio-only application-media regeneration uses `sync-classic-application-media.ps1 -SoundOnly`. It replaces only `sounds/` and its sound manifest records while preserving committed manifest order and every landlook and CICN byte, so an external PICT decoder cannot churn unrelated media.
...
- Every integrated Classic sound is rendered to the single 48 kHz, 16-bit mono waveform produced by the pinned Castle decoder's linear interpolation. Every WAV sidecar forces mono and selects Godot's lossless PCM import mode, so the mixer duplicates the waveform only at playback rather than storing two identical channels. The verifier rejects nearest-neighbor expansion and lossy QOA or Vorbis transcoding even when the source resource and manifest hashes remain exact.
...
- `verify-classic-application-media.ps1` verifies unique IDs/resource keys, all six stock landlook atlases, the expected integrated sound set and lossless PCM import sidecars, shared/stock-supply item-CIcon, and combat-CIcon counts and required stock-supply identities, black native boot plus the project-owner-supplied launch card, intro-video and independent-soundtrack provenance/playback contracts, SpriteCook and ImageGen chrome plus status/command-glyph provenance, paths, dimensions, and every committed byte count and SHA-256 without requiring an external Castle checkout.
