...
- **major** `classic.inventory.use-item` - Ordinary AOGM play proves fixed-power party-state items and saveable field scroll consumption. Combat items and scrolls now include source-backed fixed or random power plus actor, ordered repeated actor, group, ray, and rotatable persistent-area targets; discard, case transfer, door/XAP items, and broader specials remain explicit. Next: Finish scroll-case discard and transfer through the shared item and scroll transaction boundaries; characterize door/XAP items and broader specials only when parity or a reachable campaign requires them.
...
- `classic.inventory.use-item` - Ordinary AOGM play proves fixed-power party-state items and saveable field scroll consumption. Combat items and scrolls now include source-backed fixed or random power plus actor, ordered repeated actor, group, ray, and rotatable persistent-area targets; discard, case transfer, door/XAP items, and broader specials remain explicit.
