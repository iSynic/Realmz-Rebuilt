...
| `CCG-009` | Projectiles | Random projectile power before target selection and projectile spells with nonzero specials remain explicitly unsupported. | One normalized ordinary projectile signature has 90 stored monster-slot references across Castle in the Clouds, Grilochs Revenge, and Prelude to Pestilence. Item references and route reachability remain unaudited. | Some missile or thrown-item attacks cannot enter or complete targeting. | Inventory the source item/spell pairs, preserve the pre-picker RNG boundary, and implement each source-backed special family. |
