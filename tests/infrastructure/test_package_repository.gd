...
func run() -> void:
	...
	if prelude_scenario.is_ok():
		...
		assert_true(unpaid_branch != null and unpaid_branch.owner_id == "Data ED3:macro:97" and unpaid_branch.instruction_count() == 2 and unpaid_branch.instruction_at(0).opcode == 1 and unpaid_branch.instruction_at(0).operand_id == -248 and unpaid_branch.instruction_at(1).opcode == 24, "Prelude restores Castle's XAP 97 unpaid-gold message and Keep Codes result"); var no_gold_return := prelude_scenario.content.scenario.program_by_id("xap:149"); assert_true(no_gold_return != null and no_gold_return.owner_id == "Data ED3:macro:149" and no_gold_return.instruction_count() == 4 and no_gold_return.instruction_at(0).opcode == 1 and no_gold_return.instruction_at(0).operand_id == 368 and no_gold_return.instruction_at(1).opcode == 1 and no_gold_return.instruction_at(1).operand_id == 369 and no_gold_return.instruction_at(2).opcode == 7 and no_gold_return.instruction_at(2).operand_id == 64 and no_gold_return.instruction_at(2).extra_code == [0, 9, 31, 0, 0] and no_gold_return.instruction_at(3).opcode == 24, "Prelude preserves the authored XAP 149 no-gold message and Action Point replacement"); var encounter_seven_return := prelude_scenario.content.scenario.program_by_id("simple:7:result:3"); assert_true(encounter_seven_return != null and encounter_seven_return.instruction_count() == 3 and encounter_seven_return.instruction_at(1).opcode == 44 and encounter_seven_return.instruction_at(1).operand_id == 4 and encounter_seven_return.instruction_at(1).extra_code == [4, 0, 0, 0, 5], "Prelude preserves Simple Encounter 7's authored opcode 44 context")
	if production_scenario.is_ok():
		...
		assert_equal([production_scenario.content.items.item_by_id("classic.item.1").name, production_scenario.content.magic.spell_by_id("classic.spell.1101").id, production_scenario.content.characters.appearance_definitions(CharacterAppearanceDefinition.PORTRAIT).size()], [character_library.content.items.item_by_id("classic.item.1").name, "classic.spell.1101", 120], "application-owned definitions and appearance media remain available when the scenario provides no exact-key override")
	var half_truth := repository.load_package("res://src/storage/packages/bundled_campaigns/scenario-half-truth.realmz2"); assert_true(half_truth.is_ok(), "Half Truth's refreshed package passes the ordinary untrusted loader: %s" % half_truth.error_message)
	if half_truth.is_ok():
		var stale_archive := HalfTruthMediaSaveUpdate.new("res://src/storage/packages/bundled_campaigns/scenario-half-truth.realmz2")
		assert_true(stale_archive.eligible(half_truth.content.campaign_id, HalfTruthMediaSaveUpdate.OLD_PACKAGE_HASH, half_truth.content.package_hash) and not stale_archive.eligible(half_truth.content.campaign_id, "f".repeat(64), half_truth.content.package_hash) and not stale_archive.validate_archives(half_truth.content), "only the pinned revision pair is eligible, and a substituted archive cannot update a save")
	if half_truth.is_ok(): var left_wrecker := half_truth.media.asset_by_resource("cicn", 385); var right_wrecker := half_truth.media.asset_by_resource("cicn", BattlefieldPresentationGeometry.classic_monster_icon_id(385, true)); assert_equal([left_wrecker.id, left_wrecker.sha256, right_wrecker.id, right_wrecker.sha256], ["scenario-cicn-385", "1ed2f725f1a36ebdfd8e44a3672e9a0e1036420e7fd3c5a8883801ae27ad80da", "scenario-cicn-693", "1a1506b7be7970cee4408449910081ef6197303fcff9e8978e4cf7164c858ebf"], "Half Truth resolves Wrecker's authored left and right CICNs by exact scenario resource identity"); var half_truth_media := ClassicMediaCatalog.new(half_truth.media, ApplicationMediaCatalog.new(), character_library.media); var scenario_right_diagnostic := half_truth_media.resolution_diagnostic("cicn", 821, "classic-combat-monster", "decoded"); var mixed_right_diagnostic := half_truth_media.resolution_diagnostic("cicn", 795, "classic-combat-monster", "decoded"); var application_right_diagnostic := half_truth_media.resolution_diagnostic("cicn", 791, "classic-combat-monster", "decoded"); assert_equal([scenario_right_diagnostic["status"], scenario_right_diagnostic["sourceOwner"], scenario_right_diagnostic["packageAssetId"], scenario_right_diagnostic["resolvedAssetId"]], ["resolved", "scenario-package", "realmz-monster-icon-821", "realmz-monster-icon-821"], "Half Truth's scenario-owned right-facing CICN wins the exact scenario-before-application lookup"); assert_equal([mixed_right_diagnostic["status"], mixed_right_diagnostic["sourceOwner"], mixed_right_diagnostic["packageAssetId"], mixed_right_diagnostic["resolvedAssetId"]], ["resolved", "scenario-package", "scenario-cicn-795", "scenario-cicn-795"], "a scenario-owned right-facing override wins even when its monster's base CICN remains application-owned"); assert_equal([application_right_diagnostic["status"], application_right_diagnostic["sourceOwner"], application_right_diagnostic["characterAssetId"], application_right_diagnostic["resolvedAssetId"]], ["resolved", "application-package", "realmz-application-cicn-791", "realmz-application-cicn-791"], "a stock right-facing CICN resolves from the application package when the scenario owns no exact override")
	...
	assert_equal([loaded.content.campaign.recommended_party_levels, loaded.content.campaign.maximum_party_levels, loaded.content.campaign.guidance_authored], [6, 12, true], "Data SC aggregate party guidance remains distinct from per-character restrictions")
	assert_equal(loaded.content.combat.available_monster_sets(), [0, -1, 1], "packaged Classic monster sets retain the player-facing Normal, Mega, Monster order"); var menu_monster := loaded.content.combat.monster_by_id_for_set("classic.monster.1", 0); var bestiary := loaded.content.combat.bestiary_definitions_for_set(0); assert_equal([menu_monster.classic_name_id, menu_monster.description, menu_monster.not_on_menu], [42, "A deterministic synthetic bestiary entry.", false], "the package preserves independent Classic monster name identity and complete bestiary metadata"); assert_true(bestiary.has(menu_monster) and bestiary.all(func(definition: MonsterDefinition) -> bool: return definition.hit_dice > 0 and definition.hit_dice != 255 and not definition.not_on_menu), "the active package catalog exposes only Castle menu-visible Bestiary definitions in stable typed form")
	assert_equal(loaded.content.combat.monster_by_id_for_set("classic.monster.1", 1).id, "classic.monster-set.1.1", "Monster Monsters resolves to a stable alternate definition identity")
	assert_equal(loaded.content.combat.monster_by_id_for_set("classic.monster.1", -1).hit_dice, 12, "Mega Monsters preserves its alternate combat record")
	assert_true(loaded.content.campaign.contact.has("email"), "campaign contact metadata is validated as a fixed shape")
	...
	assert_equal(loaded.content.scenario_records.message_by_id(1).text, "The Realmz 2.0 fixture is deterministic.", "runtime message text crosses the validating factory")
	assert_true(loaded.content.scenario_records.has_option_labels(), "Classic Data OD option labels cross the validating package boundary")
	assert_equal(loaded.content.scenario_records.option_label_by_id(1).text, "Proceed", "typed option labels remain distinct from ordinary scenario messages")
	var message_program := loaded.content.scenario.program_by_id(loaded.content.scenario_records.trigger_by_id("ap.fixture.message").program_id)
	...
	assert_equal(loaded.content.scenario_records.trigger_by_id("ap.fixture.message").post_action_location.map_id, "land:0", "AP post-action map identity is typed")
	assert_equal(loaded.content.scenario_records.trigger_by_id("ap.fixture.message").post_action_location.coordinate, Vector2i(1, 0), "AP post-action coordinate is typed")
	assert_equal(loaded.content.scenario_records.trigger_by_id("ap.fixture.message").classic_record_index, 0, "Classic trigger record identity crosses the compiler boundary")
	assert_equal(loaded.content.scenario_records.simple_encounter_by_id(0).response_at(0).result_program_id, "simple:0:result:0", "Encounter choices reference ordinary result programs")
	assert_equal(loaded.content.scenario_records.complex_encounter_by_id(0).expected_word(), "open", "Complex Encounter words become typed runtime data")
	assert_equal(loaded.content.scenario_records.thief_encounter_by_id(0).type_flags().size(), 10, "Thief Encounter mutable flags have a fixed source-backed shape")
	assert_equal([loaded.content.scenario_records.timed_encounter_by_id(0).classic_macro_id, loaded.content.scenario_records.timed_encounter_by_id(0).program_id], [0, "xap:0"], "Timed Encounter schedules retain their Classic XAP identity")
	...
	assert_equal(loaded.content.items.item_by_id("classic.item.901").name, "Fixture Wand", "Providence item records become immutable runtime definitions")
	assert_equal([loaded.content.items.item_by_classic_id(800).name, loaded.content.items.item_by_classic_id(800).item_type], ["Fixture Scroll Case", 13], "the Classic type-13 scroll case crosses the compiler boundary")
	assert_equal([loaded.content.items.item_by_classic_id(806).name, loaded.content.items.item_by_classic_id(806).initial_charges, loaded.content.items.item_by_classic_id(806).weight_per_charge], ["Fixture Parchment", 3, 1], "source-backed charged parchment crosses the compiler boundary at its Classic scenario item identity")
	assert_equal(loaded.content.magic.spell_by_classic_id(1106).cost, -25, "Classic negative spell cost survives the package boundary as a fixed-power field spell")
	assert_equal(loaded.content.characters.race_definitions().size(), 30, "the package contains Castle's complete 30-record race table")
	assert_equal(loaded.content.characters.caste_definitions().size(), 30, "the package contains Castle's complete 30-record caste table")
	assert_equal(loaded.content.characters.race_by_id("classic.race.1").classic_id, 1, "race package identity preserves Castle's one-based character value")
	assert_equal(loaded.content.characters.race_by_id("classic.race.1").name, "Human", "race display names come from the imported Realmz name table")
	assert_equal(loaded.content.characters.race_by_id("classic.race.1").base_movement, 10, "race rules cross the compiler boundary as direct Realmz data")
	assert_equal(loaded.content.characters.race_by_id("classic.race.1").age_change(4).size(), 15, "the complete Castle race aging table crosses the validating package boundary")
	assert_true(loaded.content.characters.race_by_id("classic.race.1").ability_bonus(13) is int, "the separate fourteen-entry racial ability table crosses the compiler boundary")
	assert_equal(loaded.content.characters.caste_by_id("classic.caste.1").classic_id, 1, "caste package identity preserves Castle's one-based character value")
	assert_equal(loaded.content.characters.caste_by_id("classic.caste.1").name, "Fighter", "caste display names come from the imported Realmz name table")
	assert_equal(loaded.content.characters.caste_by_id("classic.caste.1").attributes.maximum_damage_bonus(), 5, "caste strength caps retain their source field meaning")
	assert_true(loaded.content.characters.caste_by_id("classic.caste.1").progression.initial_ability_value(13) is int and loaded.content.characters.caste_by_id("classic.caste.1").progression.level_ability_die(13) is int and loaded.content.characters.caste_by_id("classic.caste.1").progression.victory_threshold(29) is int, "caste abilities and all thirty victory thresholds cross the validating package boundary")
	var portraits := loaded.content.characters.appearance_definitions(CharacterAppearanceDefinition.PORTRAIT)
	var combat_icons := loaded.content.characters.appearance_definitions(CharacterAppearanceDefinition.COMBAT_ICON)
	...
	assert_equal(loaded.content.combat.monster_by_id("classic.monster.1").icon_id, 384, "monster display identity preserves Castle's exact base cicn")
	assert_not_null(loaded.media.asset_by_resource("cicn", 384), "the base facing of each reachable monster resolves through exact Classic media identity"); assert_not_null(loaded.media.asset_by_resource("cicn", 692), "the alternate Castle facing resolves through the authored base cicn plus 308"); var missing_facing_monsters: Array[MonsterDefinition] = [loaded.content.combat.monster_by_id("classic.monster.1")]; var base_only_media: Array[MediaAsset] = [loaded.media.asset_by_resource("cicn", 384)]; var facing_validator := PackageMediaValidatorResolver.new(); assert_false(facing_validator.validate_monster_media(missing_facing_monsters, base_only_media), "package validation rejects a monster archive that omits its exact right-facing CICN"); assert_true(facing_validator.validate_monster_media(missing_facing_monsters, base_only_media, true), "a deferred-reference package admits absent monster presentation media"); assert_equal(facing_validator.warnings.size(), 1, "deferred monster-media validation retains one warning"); if not facing_validator.warnings.is_empty(): var facing_warning: ScenarioCompatibilityWarning = facing_validator.warnings[0]; assert_equal([facing_warning.source_kind, facing_warning.source_id, facing_warning.field, facing_warning.target_kind, facing_warning.target_id], [&"monster", "classic.monster.1", "facingRightIconId", &"cicn", "692"], "the deferred monster-media warning retains the exact source and target identity")
	assert_equal(loaded.content.combat.monster_by_id("classic.monster.1").starting_conditions()[ConditionRules.REFLECTING_SPELLS], -1, "all forty authored monster starting conditions cross the validating package boundary")
	assert_equal(loaded.content.combat.battle_by_id("classic.battle.0").monster_slots()[0].monster_id, "classic.monster.1", "battle placements reference stable monster IDs")
	assert_equal([loaded.content.economy.shop_by_id("classic.shop.0").quantity(0), loaded.content.economy.shop_by_id("classic.shop.0").stock_slot(0)], [2, 817], "shop stock compiles to stable item references, quantities, and sparse native slots")
	assert_equal(loaded.content.economy.treasure_by_id("classic.treasure.0").item_ids()[0], "classic.item.901", "treasures use the same item identity as inventory")
	assert_equal(loaded.content.magic.spell_by_id("classic.spell.5101").damage_max, 4, "custom spells use packed Realmz class/level/slot identity")
	assert_not_null(loaded.content.magic.spell_by_id("classic.spell.1101"), "Providence compiles standard Data S spells into normalized runtime definitions")
	assert_equal(loaded.content.magic.spell_by_id("classic.spell.1108").name, "Magic Darts", "standard spell names follow Castle's positive Custom Names STR# lookup")
	...
	assert_contains(sandbox_error, "no secure external host", "the manifest readiness path rejects deferred GDScript backends at the security boundary"); assert_equal(PackageRepository.package_capability_error("realmz.scenario.deferred-references-v1"), "", "the runtime advertises the deferred-reference capability without weakening unrelated manifest gates"); assert_contains(PackageRepository.package_capability_error("realmz.scenario.unknown-v1"), "unknown capability", "the same readiness path rejects unrecognized package capabilities")
