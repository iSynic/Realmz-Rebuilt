...
Supported engine commands are `describe`, `observe`, `checkpoint`, `capture`, `restore`, `act`, `respond`, `ui`, `invoke`, and `close`. Direct invocation can prepare exact isolated AP, encounter, Battle, Treasure, and Shop surfaces for subsequent actual-input proof. In a fixture with an active expected Classic Battle, `invoke` also accepts `{"target":{"kind":"battle-victory","id":<classic-battle-id>}}`; this calls the ordinary debug victory transaction and records forced-victory continuation evidence, never ordinary-combat proof. Fixture lifecycle operations are narrow and explicit; unsupported future fixture names return an explicit error. A fixture controller button release or exact axis-neutral event remains deliverable after an intervening gameplay revision so a held direction cannot prevent its own release; initiating input and live-session mutation keep the ordinary revision gates.
...
Each job retains its original baseline checkpoint, accepted input sequence, complete before/after observations, trace deltas, wire errors, readiness failures, and captures in `evidence.json`. Readiness is semantic: journeys wait for a ready fixture and the required exploration, interaction, or UI boundary. `$pending` is accepted only as `params.response.requestId` and resolves to the exact observed pending request identity. A UI recipe may supply `{ "action": "click", "controlLabel": "Shop" }`; exactly one enabled current label must match, and evidence retains both the requested selector and the resolved engine control ID. It may instead supply bounded `controller-button` press/release or `controller-axis` value/neutral events, which enter the real controller owner even while playback or a host dialog owns application input. Observations expose `focusControlId` and mark the matching visible control as focused. Journeys never restore or reseed between steps. Failed jobs and their artifacts are retained; failures receive a screenshot or an explicit reason why it was unavailable, including failures before any input was submitted. The 256 MiB inclusive checkpoint-plus-JSON-plus-PNG bound reserves capacity before another action or capture.
