...
- `ControllerPromptStrip` is a non-layout three-second translucent hint shown only for Actions, Workspaces, Top Menu, System, and explicit inspection; its labels resolve the active bindings and controller family. `GameShellMenuController` owns controller traversal of the existing top-menu headings and invokes their existing route, command, and system owners. The shell mounts one shared `ControllerRadialOverlay`; interaction actions retain their active component owner, shell action entries come from `GameShellCommandController`, and the curated workspace pages exclude unrestricted internal combat or service routes.
