...
func run() -> void:
	...
	_reset_files(campaign_path, ["quick.r2save", "quick.r2save.bak", "mismatch.r2save", "broken.r2save"])
	...
	var second := SaveEnvelope.from_data(first.to_data())
	...
	var mismatch_data := first.to_data()
	...
	assert_equal(previews.size(), 4, "primary, backup, mismatch, and corrupt records are indexed independently")
