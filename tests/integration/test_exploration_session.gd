...
func run() -> void:
	...
	_test_map_view_projection_edges(content); _test_field_heal(content); _test_scripted_party_loss(content); _test_movement_fatigue_gate(content); _test_attempted_land_move_search(content); _test_terrain_replacement_topology(content)
	...
	var search_mode_on := session.submit_intent(ExplorationIntents.toggle_search())
	...
	assert_true(searching_restore.view().party_summary.searching, "the detached view preserves restored Search mode"); assert_equal(session.submit_intent(ExplorationIntents.toggle_search()).state, SessionStep.State.COMPLETED, "Search toggles off at the same committed boundary")
	...
	var open_step := open_session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
	var open_view := open_session.view(open_step.events)
	assert_equal([open_view.domain_revisions.party, open_session.snapshot().game_state.clock.total_minutes()], [before_open_view.domain_revisions.party, 5], "ordinary movement reuses the unchanged party projection and defaults absent land-scale metadata to five-minute clicks"); assert_equal(open_view.domain_revisions.exploration, open_view.revision, "ordinary movement advances the exploration projection revision"); assert_true(open_view.map_view.presentation_delta != null and open_view.map_view.presentation_delta.viewport_shift == Vector2i.RIGHT and open_view.map_view.presentation_delta.newly_visited == [Vector2i(2, 1)], "strict ordinary movement carries only its detached viewport and visibility delta")
	...
	var los_content := _open_movement_content(content, true); var los_session := GameSession.new(); los_session.start(los_content, 1); _begin_fixture_adventure(los_session, los_content); var initial_los_view := los_session.view(); var los_step := los_session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT)); var moved_los_view := los_session.view(los_step.events); assert_true(moved_los_view.map_view.presentation_delta != null and moved_los_view.domain_revisions.party == initial_los_view.domain_revisions.party and moved_los_view.domain_revisions.exploration == moved_los_view.revision, "LOS movement identifies its visibility-sensitive cells while retaining the ordinary shell and unchanged view domains")
	...
	var diagonal_step := diagonal_session.submit_intent(ExplorationIntents.move(Vector2i(-1, -1)))
	...
	assert_equal(diagonal_session._context.state.clock.total_minutes(), content.world.map_by_id("land:0").topology.cell_at(Vector2i.ZERO).movement_cost, "indoor land movement retains the authored Classic timeclick count in minutes")
	...
	var diagonal_layout_step := diagonal_layout_session.submit_intent(ExplorationIntents.move(Vector2i(-1, -1)))
	...
	var north := session.submit_intent(ExplorationIntents.move(Vector2i.UP))
	...
	var hidden_start_minutes := session._context.state.clock.total_minutes()
	var hidden_entry := session.submit_intent(ExplorationIntents.move(Vector2i.LEFT))
	...
	assert_equal(session._context.state.clock.total_minutes(), hidden_start_minutes + hidden_destination.movement_cost, "concealed-square entry pays the underlying tile's one-minute indoor Classic timeclick cost")
	...
	session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
	var transitioned := session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
	...
	var self_program := ScenarioProgramDefinition.new(keep_source.program_id, &"trigger", keep_source.id, [ClassicActionDefinition.new(0, 3, 3, 0, false, [1, 0, 0, 0, 0]), ClassicActionDefinition.new(1, 45, 45, 0, false, [-1, 1, 0, 0, 0]), ClassicActionDefinition.new(2, 3, 3, 0, false, [1, 0, 0, 0, 0])]); var self_trigger := TriggerDefinition.new(keep_source.id, self_program.id, keep_source.map_id, keep_source.coordinate, true, 100, TriggerDestinationDefinition.new(keep_source.map_id, keep_source.coordinate), keep_source.classic_record_index)
	var self_content := RealmzContent.new(content.campaign_id, content.package_hash, content.content_id, content.rules_version, "land:1", Vector2i(0, 1), content.world, ScenarioDefinition.new([self_program], []), [], [self_trigger], [], content.characters.race_definitions(), content.characters.caste_definitions()); var self_session := GameSession.new(); self_session.start(self_content, 1); _begin_fixture_adventure(self_session, self_content); var self_wait := self_session.submit_intent(ExplorationIntents.move(Vector2i.UP)); var self_teleported := self_session.respond(InteractionResponse.yes_no(self_wait.interaction, true)); var self_boundary := save_round_trip(self_session.snapshot())
	assert_equal(self_session.restore(self_content, self_boundary).state, SessionStep.State.COMPLETED, "a yielded AP restores with its issuing cell distinct from its teleported party"); var self_completed := self_session.respond(InteractionResponse.yes_no(self_session.view().pending_interaction, true)); assert_equal([self_completed.state, self_session.view().party_coordinate, self_session.snapshot().rng_state.to_data()], [SessionStep.State.COMPLETED, Vector2i(1, 0), self_boundary.rng_state.to_data()], "a source-self header neither undoes the saved teleport nor consumes another RNG draw")
	assert_true(_has_event(self_teleported, &"party_teleported") and not _has_event(self_completed, &"party_teleported") and not _has_event(self_completed, &"trigger_fired"), "the saved VM teleport completes once without re-firing the source AP"); var corrupt_self := save_round_trip(self_boundary); corrupt_self.continuation.exploration().coordinate = Vector2i(1, 0); assert_equal(self_session.restore(self_content, corrupt_self).error_code, &"invalid_session_continuation", "a saved AP cannot move its issuing identity to the party destination")
	...
	var overhead_blocked := dungeon_session.submit_intent(ExplorationIntents.overhead_dungeon_move(Vector2i.UP))
	...
	var direct_auto_enabled := restored_surprise.submit_intent(CombatIntents.set_auto(direct_auto_character.id, true))
	...
	var direct_auto_disabled := restored_surprise.submit_intent(CombatIntents.set_auto(direct_auto_character.id, false))
	...
	var blocked_during_battle := restored_surprise.submit_intent(ExplorationIntents.move(Vector2i.LEFT))
	...
	door_session.submit_intent(ExplorationIntents.move(Vector2i.DOWN))
	...
	var second_door := restored_door.submit_intent(ExplorationIntents.move(Vector2i.LEFT))
	...
	var relocated := destination_session.submit_intent(ExplorationIntents.move(Vector2i.UP))
...
func _test_location_notes(content: RealmzContent) -> void:
	...
	var original_coordinate := session._context.state.party.coordinate
	...
	var clock_before := session._context.state.clock.total_minutes()
	var created := session.submit_intent(ExplorationIntents.set_location_note("The road narrows beside the old stones."))
	...
	var too_long := session.submit_intent(ExplorationIntents.set_location_note("é".repeat(128)))
	...
	assert_equal(session.submit_intent(ExplorationIntents.set_location_note("The lower road.")).state, SessionStep.State.COMPLETED, "a second land note commits at a different current location")
	...
	assert_equal(session.submit_intent(ExplorationIntents.set_location_note("Dungeon entrance.")).state, SessionStep.State.COMPLETED, "a dungeon note commits into its independent record stream")
	...
	var cleared := restored.submit_intent(ExplorationIntents.set_location_note(""))
...
func _test_contextual_encounter_command(source_content: RealmzContent) -> void:
	...
	session._context.rng = ScriptedRng.new([0, 32767, -32767])
	var opened := session.submit_intent(ExplorationIntents.contextual_encounter())
	...
	assert_equal([session.rng_trace().size(), session.rng_trace()[0]["tag"], session.snapshot().rng_state.draw_count], [3, "contextual-encounter.contextual.fixture.region.door.0", 3], "Encounter draws for all three source-ordered door slots, including zero chances")
	...
	assert_equal([fallback.state, fallback_event.payload["programId"], fallback_event.payload["coordinate"], fallback_event.payload["defaultProgram"], restored.snapshot().rng_state.draw_count], [SessionStep.State.COMPLETED, "xap:0", Vector2i.RIGHT, true, 6], "Encounter draws for all consumed slots before running XAP 0 at the faced land coordinate")
	for repeat_index: int in 8:
		var repeated := restored.submit_intent(ExplorationIntents.contextual_encounter())
...
func _test_field_heal(content: RealmzContent) -> void:
	...
	_begin_fixture_adventure(session, content, 2); var members := session._context.state.party.characters()
	...
	var healed := session.submit_intent(ExplorationIntents.heal()); assert_true(_has_event(healed, &"health_recovered") and _has_event(healed, &"spell_points_spent"), "Heal publishes its source-ordered recovery and spell-point events")
	assert_equal([session._context.state.clock.total_minutes() - before_minutes, healer.spell_points, wounded.current_health > 1, wounded.current_health <= wounded.maximum_health], [1, 15, true, true], "one indoor land Heal pulse spends one minute and ten spell points for its eligible wounded recipient")
	...
	assert_equal([restored._context.state.party.character_by_id(healer.id).spell_points, restored._context.state.party.character_by_id(wounded.id).current_health], [healer.spell_points, wounded.current_health], "Heal spell points and stamina persist together")
	session._context.state.character_spellcasting_blocked = true
	var blocked_before := JSON.stringify(session._context.state.to_data())
	var blocked_rng_before := session.rng_trace().size()
	var blocked := session.submit_intent(ExplorationIntents.heal())
	...
	assert_equal([JSON.stringify(session._context.state.to_data()), session.rng_trace().size()], [blocked_before, blocked_rng_before], "blocked field Heal changes no health, spell points, clock, fatigue, RNG-owned state, or scenario state")


func _test_movement_fatigue_gate(content: RealmzContent) -> void:
	var land_content := _open_movement_content(content); var land := GameSession.new(); land.start(land_content, 1); _begin_fixture_adventure(land, land_content); land._context.state.party.fatigue = 135; var land_before := JSON.stringify(land._context.state.to_data()); var land_rng_before := land.rng_trace().size(); var blocked_land := land.submit_intent(ExplorationIntents.move(Vector2i.RIGHT)); var land_warning := _event(blocked_land, &"classic_notification_requested")
	assert_equal([blocked_land.state, land_warning.payload.get("text") if land_warning != null else "", land_warning.payload.get("soundId") if land_warning != null else 0, JSON.stringify(land._context.state.to_data()), land.rng_trace().size()], [SessionStep.State.COMPLETED, "You are too tired to continue.  You need to rest.", 6000, land_before, land_rng_before], "fatigue 135 blocks ordinary land movement with Castle warning 54 without changing position, time, RNG, camp, wealth, or scenario state")
	land._context.state.camping_allowed = false; var escaped_land := land.submit_intent(ExplorationIntents.move(Vector2i.RIGHT)); assert_equal([escaped_land.state, land.view().party_coordinate], [SessionStep.State.COMPLETED, Vector2i(2, 1)], "Castle's exhausted-land escape hatch still permits travel where the scenario disabled camping so the party cannot be soft-locked")
	var dungeon_source := GameSession.new(); dungeon_source.start(content, 1); _begin_fixture_adventure(dungeon_source, content); var dungeon_save := dungeon_source.snapshot(); dungeon_save.game_state.party.map_id = "dungeon:0"; dungeon_save.game_state.party.coordinate = Vector2i(2, 0); dungeon_save.game_state.party.fatigue = 135; dungeon_save.game_state.dungeon_heading = 4; var dungeon := GameSession.new(); assert_equal(dungeon.restore(content, dungeon_save).state, SessionStep.State.COMPLETED, "the exhausted dungeon fixture restores transactionally"); var dungeon_before := JSON.stringify(dungeon._context.state.to_data()); var dungeon_rng_before := dungeon.rng_trace().size(); var blocked_dungeon := dungeon.submit_intent(ExplorationIntents.overhead_dungeon_move(Vector2i.UP)); var dungeon_warning := _event(blocked_dungeon, &"classic_notification_requested")
	assert_equal([blocked_dungeon.state, dungeon_warning.payload.get("text") if dungeon_warning != null else "", dungeon.view().party_coordinate, dungeon.view().map_view.dungeon_heading, JSON.stringify(dungeon._context.state.to_data()), dungeon.rng_trace().size()], [SessionStep.State.COMPLETED, "You are too tired to continue.  You need to rest.", Vector2i(2, 0), 4, dungeon_before, dungeon_rng_before], "full fatigue blocks dungeon movement before overhead heading alignment and changes no location, heading, clock, RNG, or scenario state")
...
func _test_special_dungeon_bits(source_content: RealmzContent) -> void:
	...
	for dungeon_case: Dictionary in cases:
		...
		var moved := session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
...
func _test_scripted_party_loss(source_content: RealmzContent) -> void:
	var open := _open_movement_content(source_content); var ap := ScenarioProgramDefinition.new("ap:scripted-loss", &"trigger", "scripted-loss", [ClassicActionDefinition.new(0, 16, 16, 0, false, [-20, 1, 1, 0, 0]), ClassicActionDefinition.new(1, 24, 24, 0, false, [])]); var trigger := TriggerDefinition.new("scripted-loss", ap.id, open.start_map_id, Vector2i(2, 1), true, 100); open.world.map_by_id(open.start_map_id).topology.cell_at(trigger.coordinate)._trigger_ids.append(trigger.id); var death := ScenarioProgramDefinition.new("xap:scripted-death-hook", &"application-hook", "party-death", [ClassicActionDefinition.new(0, 1, 1, 1, false, []), ClassicActionDefinition.new(1, 119, 119, 0, false, [])]); var hooks := ScenarioApplicationHooks.new("", death.id, "", "", ""); var content := RealmzContent.new(open.campaign_id, open.package_hash, open.content_id, open.rules_version, open.start_map_id, open.start_coordinate, open.world, ScenarioDefinition.new([ap, death], [], hooks), [MessageDefinition.new(1, "The party falls.")], [trigger], [], open.characters.race_definitions(), open.characters.caste_definitions()); var session := GameSession.new(); assert_equal(session.start(content, 16).state, SessionStep.State.COMPLETED, "scripted-loss fixture starts"); _begin_fixture_adventure(session, content, 2); var entered := session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT)); assert_not_null(entered.interaction, "scripted-loss AP enters Party Death (state %s, error %s: %s, events %s)" % [entered.state, entered.error_code, entered.error_message, entered.events.map(func(event: DomainEvent) -> StringName: return event.kind)])
	if entered.interaction == null: return
	assert_equal([entered.state, entered.interaction.body.to_data().get("prompt"), session._context.state.party.characters().map(func(member: CharacterState) -> int: return member.current_health)], [SessionStep.State.WAITING_FOR_INTERACTION, "The party falls.", [-10, -10]], "ordinary AP movement applies scripted damage, stops at Castle's total-party-loss boundary, and enters the death hook"); var pending := save_round_trip(session.snapshot()); var restored := GameSession.new(); assert_equal(restored.restore(content, pending).state, SessionStep.State.COMPLETED, "pending scripted party death restores with its suspended AP"); var resumed := restored.respond(InteractionResponse.acknowledge(restored.view().pending_interaction)); assert_equal([resumed.state, restored.view().session_started, restored._context.state.party.characters().map(func(member: CharacterState) -> int: return member.current_health)], [SessionStep.State.COMPLETED, true, [1, 1]], "opcode 119 revives and resumes the remaining authored AP instructions without replaying damage"); var no_hook := RealmzContent.new(open.campaign_id, open.package_hash, open.content_id, open.rules_version, open.start_map_id, open.start_coordinate, open.world, ScenarioDefinition.new([ap], []), [], [trigger], [], open.characters.race_definitions(), open.characters.caste_definitions()); var doomed := GameSession.new(); assert_equal(doomed.start(no_hook, 16).state, SessionStep.State.COMPLETED, "no-hook scripted-loss fixture starts"); _begin_fixture_adventure(doomed, no_hook, 2); var closed := doomed.submit_intent(ExplorationIntents.move(Vector2i.RIGHT)); assert_true(closed.events.any(func(event: DomainEvent) -> bool: return event.kind == &"session_ended") and not doomed.view().session_started, "scripted total-party loss without a revival hook ends the adventure")
...
func _test_boat_movement(source_content: RealmzContent) -> void:
	...
	var board_prompt := session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
	...
	var crossed_water := restored_board.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
	...
	for attempt: int in 2:
		var shore_block := restored_board.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
	...
	var leave_prompt := restored_attempts.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
	...
	var stepped_ashore := restored_leave.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
	...
	var declined_prompt := declined_session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT))
	...
	var entered_ap := ap_session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT)); assert_equal([entered_ap.state, ap_session.view().party_coordinate, _has_event(entered_ap, &"trigger_fired")], [SessionStep.State.COMPLETED, Vector2i(1, 0), true], "an embarked party enters a placed land action point through Castle's door-band branch")
	var ordinary_land_block := ap_session.submit_intent(ExplorationIntents.move(Vector2i.RIGHT)); assert_equal([ordinary_land_block.state, ap_session.view().party_coordinate, _sound_ids(ordinary_land_block)], [SessionStep.State.COMPLETED, Vector2i(1, 0), [-148, 11]], "the action-point exception does not let an embarked party traverse ordinary needBoat-0 terrain")
...
func _test_attempted_land_move_search(source_content: RealmzContent) -> void:
	...
	var ordinary := GameSession.new(); ordinary.start(content, 1); _begin_fixture_adventure(ordinary, content); var rng_before := ordinary.rng_trace().size(); var ordinary_attempt := ordinary.submit_intent(ExplorationIntents.move(Vector2i.LEFT)); var ordinary_search := _event(ordinary_attempt, &"movement_secret_search_completed")
	...
	var searching := GameSession.new(); searching.start(content, 1); _begin_fixture_adventure(searching, content); assert_equal(searching.submit_intent(ExplorationIntents.toggle_search()).state, SessionStep.State.COMPLETED, "the blocked-attempt fixture enables persistent Search mode"); var minutes_before := searching.snapshot().game_state.clock.total_minutes(); var searched_attempt := searching.submit_intent(ExplorationIntents.move(Vector2i.LEFT))
