...
Media validation deliberately has two phases. `PackageMediaValidatorResolver.validate_package_assets` proves the scenario archive's own index, payload paths, hashes, dimensions, local keys, and music slots. `PackageMediaComposer` then overlays those typed records over application media by exact Classic resource key or stable render ID. Only that effective catalog is used to validate required appearances, battle capabilities, topology render references, monster art, player maps, and scenario references. The load result still exposes a scenario-only `PackageMediaCatalog`, preserving which ZIP owns each byte while presentation performs the same scenario-before-application lookup.
