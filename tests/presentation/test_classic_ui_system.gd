...
func _test_fixture_gallery_coverage() -> void:
	...
	capacity_component.free(); var unidentified_component := TreasureDistributionInteraction.new(); unidentified_component.configure(item_media, null, false); unidentified_component.build(ClassicUiFixtureGallery.request_for(InteractionRequest.TREASURE_DISTRIBUTION, &"unidentified")); var unidentified_name := unidentified_component.find_child("TreasureSelectedItemName", true, false) as Label; var detect_picker := unidentified_component.find_child("TreasureDetectMagicCaster", true, false) as OptionButton; var identify_picker := unidentified_component.find_child("TreasureIdentifyCaster", true, false) as OptionButton; assert_true(unidentified_name.theme_type_variation == &"ClassicUnidentifiedItem" and not unidentified_name.has_theme_color_override("font_color") and detect_picker.theme_type_variation == &"ClassicTheldrowOptionButton" and identify_picker.theme_type_variation == &"ClassicTheldrowOptionButton" and detect_picker.get_item_text(0) == "Hero 1 • SP 30 • Cost 5" and identify_picker.get_item_text(0) == "Hero 1 • SP 30 • Cost 25", "unidentified Treasure naming and lore-caster selectors use their Castle-backed typography roles while distinguishing remaining SP from action cost"); unidentified_component.free()
