...
func _init(source_battle_id: String, initial_monsters: Array[MonsterState] = []) -> void:
...
func to_data() -> Dictionary:
	...
	return {"battleId": battle_id, "round": round_number, "turnIndex": turn_index, "completed": completed, "outcome": String(outcome), "turnOrder": _turn_order.duplicate(), "monsters": monster_data}
...
static func from_data(data: Variant) -> CombatState:
	...
	if not data["battleId"] is String or data["battleId"].is_empty() or loaded_round < 1 or loaded_turn < 0 or not data["completed"] is bool or not data["outcome"] is String or not data["turnOrder"] is Array or not data["monsters"] is Array:
		return null
	...
	var result := CombatState.new(data["battleId"], loaded_monsters)
