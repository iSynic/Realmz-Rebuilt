...
A subsequent physical-layout audit found that the earlier gate did not enforce the plan's feature-folder topology. The manifest now names the exact final directories under all six source boundaries, and the verifier exposes three additional ratchets. The corrective batches have moved persistent shell/navigation composition into `src/ui/shell`, the generic request host into `src/ui/shared/interactions`, the complete retained battlefield/command deck/targeting/playback stack into `src/ui/combat`, retained exploration maps and dungeons into `src/ui/exploration`, Character/Allies/Bestiary/Character Files into `src/ui/characters`, Places/acquired Maps/Journal/scrolling text into `src/ui/journal`, Party Wealth/Shop/Temple/Bank/Treasure into `src/ui/services`, and campaign selection/party assembly/character creation into `src/ui/setup`. The persistent roster, System workspace, route-content coordinator, reusable screen records, and typed route resources now live with their shell or shared owners, eliminating the generic `screens`, `controllers`, and root-level `routes` buckets. The repository still has 16 missing target feature directories, 20 undeclared legacy or generic directories, and 70 production files loose at a boundary root. These are active migration debt with final targets of zero; Beta 1 cannot treat the earlier zero size/interface counters as complete architecture certification.
