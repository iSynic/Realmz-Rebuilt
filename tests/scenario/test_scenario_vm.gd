...
func _test_public_application_transitions(content: RealmzContent) -> void:
	...
	var map := content.world.player_map_by_classic_id(1); var shop := content.shop_by_id("classic.shop.0"); var shop_offer := api.execute_classic(ClassicActionDefinition.new(0, 6, 6, 0, false, []), "shop.offer"); var shop_result := api.request_available_shop("shop.open"); var shop_data := shop_result.interaction.body.to_data(); assert_equal([shop_offer.state, state.active_shop_id, shop_offer.events[0].kind, shop_result.state, shop_data["stock"][0]["stockKey"], shop_data["stock"][0]["index"], shop_data["stock"][0]["category"], shop_data["characters"][0]["portraitId"]], [ScenarioRuntimeOperationResult.State.COMPLETED, shop.id, &"shop_available", ScenarioRuntimeOperationResult.State.WAITING, "base:817", 817, "supplies", shopper.portrait_id], "positive Classic opcode 6 exposes a contextual Shop command while its public Shop projection preserves the compiler-owned sparse slot, category band, and exact shopper portrait"); var first_stock := content.item_by_id(shop.item_ids()[0]); var saved_icon_id := first_stock.icon_id; first_stock.icon_id = 0; var iconless_shop := api.request_available_shop("shop.iconless"); first_stock.icon_id = saved_icon_id; assert_true(iconless_shop.state == ScenarioRuntimeOperationResult.State.WAITING and not iconless_shop.interaction.body.to_data()["stock"][0].has("iconId"), "a source-valid shop item without a CICN remains an actionable typed row instead of collapsing the interaction")
