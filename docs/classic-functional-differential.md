...
The rule, clock, spell, complete monster-special attack table, battle-allegiance, battle-reward, persistence, `showageupdate` click boundaries, character melee foundation, and bounded ordinary monster melee resolver are covered. Battle placement distance no longer contaminates melee rules; magical-plus, blunt, sharp, and corrected specific-item gates use distinct typed monster fields. Monster fumbles, reflection, missile-weapon replacement, behind/facing derivation, critical ability values, attack-point cadence, missile toggling, and full equipment side effects remain separate unresolved boundaries. Random-weapon table 2 also needs spawn-origin state because Castle resolves its overlapping roll-85 rows differently for summoned and ordinary battle monsters. The next rolling combat slice should model and serialize active-turn attack half-units before implementing cadence-dependent actions; the fumble preference and dropped-item queue can then join the same tactical state boundary.
