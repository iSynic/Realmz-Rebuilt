...
The active `rebuilt-hotspot-test-performance` maintenance pause holds that same parity batch and every workflow count steady while the remaining oversized product owners are decomposed, the test estate is consolidated to one owning proof per behavior, and exploration cadence is restored. Its exit gate is mechanical and player-visible: production files/functions and test suites must satisfy their permanent caps, tests must stop reaching private product methods, warm six-character AOGM movement must meet its latency/rate targets, and an ordinary AOGM route must remain intact. These maintenance results do not upgrade workflow delivery states.
