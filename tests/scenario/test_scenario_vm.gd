...
func _test_public_vm_repeated_combat_item(content: RealmzContent) -> void:
	...
	for seed: int in range(1, 33):
		...
		var ally := CharacterState.new("fixture.vm-item.ally", "VM Item Ally", 30, 30); ally.race_id = selected_race.id; ally.caste_id = selected_caste.id; ally.normal_attacks = 4; ally.maximum_movement = 12; ally.movement = 12; ally.spell_points = 17; ally.set_inventory([ItemInstance.new("instance.vm-repeated-wand.ally", item.id, 2, false, true), ItemInstance.new("instance.flask-oil.ally", flask.id, 5, false, true)])
