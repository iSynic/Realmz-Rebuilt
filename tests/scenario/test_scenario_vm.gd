...
func _test_classic_choice_labels_and_sound_wait(content: RealmzContent) -> void:
	...
	assert_true(positive_text.interaction.payload["journalEligible"], "a positive source message inside Castle's saved note table can be recorded")
	assert_false(positive_text.interaction.payload["journalRecorded"], "an undiscovered source message is detached explicitly")
	...
	var forged_note := api.resume_classic(positive_text.continuation, InteractionResponse.new(positive_text.interaction.request_id, &"acknowledge", {"takeNote": "yes"}), "request.positive-text.resume")
	...
	var acknowledged_text := api.resume_classic(positive_text.continuation, InteractionResponse.new(positive_text.interaction.request_id, &"acknowledge", {"takeNote": true}), "request.positive-text.resume")
	...
	assert_true(recorded_text.interaction.payload["journalRecorded"], "subsequent source text exposes its existing discovery without changing it")
...
func _test_session_save_resume_boundary(content: RealmzContent) -> void:
	...
	var first_message_id := int(textbox_restored.view().pending_interaction.payload["messageId"])
	var action_text := textbox_restored.respond(InteractionResponse.new(textbox_restored.view().pending_interaction.request_id, &"acknowledge", {"takeNote": true}))
	...
	message_texts.append_array(_message_texts(action_text.events))
	var journal_save := SaveEnvelope.from_data(textbox_restored.snapshot().to_data())
	...
	var completed := journal_restored.respond(InteractionResponse.new(journal_restored.view().pending_interaction.request_id, &"acknowledge", {}))
	message_texts.append_array(_message_texts(completed.events))
	...
	assert_equal(journal_restored.snapshot().session_continuation, {}, "completed action timeline clears the serialized host continuation")
...
func _test_age_update_precedes_post_move(content: RealmzContent) -> void:
	...
	assert_equal(session.snapshot().session_continuation["resumeKind"], "post-clock", "the age dialog owns the unstarted post-clock continuation")
	assert_equal(session.snapshot().session_continuation["resumeContinuation"]["resumeKind"], "post-move", "the post-clock continuation retains destination AP discovery after its timed scan")
...
func _test_classic_shell_domain_route(content: RealmzContent) -> void:
	...
	while session.view().pending_interaction != null and combat_steps < 32:
		...
		var payload := {"actorId": String(request.payload.get("actorId", "")), "action": "finish", "targetId": ""}
...
func _test_classic_player_map_workflow(content: RealmzContent) -> void:
	...
	assert_equal([shown.interaction.kind, shown.interaction.payload.get("presentation"), shown.interaction.payload.get("playerMapId")], [&"acknowledge", "player-map", definition.id], "immediate display identifies the typed player-map presentation")
	var forged := api.resume_classic(shown.continuation, InteractionResponse.new(shown.interaction.request_id, &"acknowledge", {"accepted": true}), "request.player-map.forged")
	assert_equal(forged.error_code, &"invalid_interaction_response", "player-map acknowledgement rejects forged fields")
	var resumed := api.resume_classic(shown.continuation, InteractionResponse.new(shown.interaction.request_id, &"acknowledge", {}), "request.player-map.resume")
	...
	assert_equal([vm_wait.state, vm_wait.interaction.payload.get("playerMapId")], [ScenarioVmResult.State.WAITING, definition.id], "the issuing VM frame owns the immediate player-map request")
	...
	var vm_resumed := restored_vm.resume(InteractionResponse.new(vm_snapshot.pending_request.request_id, &"acknowledge", {}), api)
	...
	assert_equal([pending_step.state, pending_step.interaction.payload.get("playerMapId")], [SessionStep.State.WAITING_FOR_INTERACTION, definition.id], "GameSession owns the negative opcode-29 interaction boundary")
	var pending_envelope := SaveEnvelope.from_data(pending_session.snapshot().to_data())
	...
	assert_equal(pending_restored.respond(InteractionResponse.new("wrong-request", restored_request.kind, {})).error_code, &"interaction_mismatch", "restored player-map requests retain exact request identity")
	var pending_completed := pending_restored.respond(InteractionResponse.new(restored_request.request_id, restored_request.kind, {}))
...
func _test_classic_character_ability_picker(content: RealmzContent) -> void:
	...
	assert_equal([waiting.interaction.kind, waiting.interaction.payload["count"], waiting.interaction.payload["eligible"].size()], [&"character_selection", 1, 2], "ability picker exposes one living-character choice with stable IDs")
...
func _test_monster_aging_attack_continuations(content: RealmzContent) -> void:
	...
	var aged := vm.resume(InteractionResponse.new(player_turn.interaction.request_id, &"combat_action", {"actorId": character.id, "action": "finish", "targetId": ""}), api)
