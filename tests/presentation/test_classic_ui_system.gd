...
func _test_fixture_gallery_coverage() -> void:
	...
	capacity_component.free(); var unidentified_component := TreasureDistributionInteraction.new(); unidentified_component.configure(item_media, null, false); unidentified_component.build(ClassicUiFixtureGallery.request_for(InteractionRequest.TREASURE_DISTRIBUTION, &"unidentified")); var unidentified_name := unidentified_component.find_child("TreasureSelectedItemName", true, false) as Label; var detected_magic_glow := unidentified_component.find_child("TreasureMagicGlow_reward_item_1", true, false) as TextureRect; var detect_picker := unidentified_component.find_child("TreasureDetectMagicCaster", true, false) as OptionButton; var identify_picker := unidentified_component.find_child("TreasureIdentifyCaster", true, false) as OptionButton; assert_true(unidentified_name.theme_type_variation == &"ClassicUnidentifiedItem" and not unidentified_name.has_theme_color_override("font_color") and detected_magic_glow != null and detected_magic_glow.texture == ClassicUiAssetCatalog.texture(&"loot.item.glow") and detect_picker.theme_type_variation == &"ClassicTheldrowOptionButton" and identify_picker.theme_type_variation == &"ClassicTheldrowOptionButton" and detect_picker.get_item_text(0) == "Hero 1 • SP 30 • Cost 5" and identify_picker.get_item_text(0) == "Hero 1 • SP 30 • Cost 25", "Detect Magic gives the item cell Castle's exact blue-green glow while unidentified naming and lore-caster selectors retain their Castle-backed roles and distinguish remaining SP from action cost"); unidentified_component.free()
	...
	var level_two := spell_component.find_child("LevelSpellLevel2", true, false) as Button; var selected_spell := _buttons_in(spell_component.find_child("LevelSpellList", true, false))[0]; var spell_confirm := spell_component.find_child("LevelSpellConfirm", true, false) as Button; assert_true(spell_component.find_child("LevelSpellLevelRail", true, false) != null and level_two.text == "Level 2" and selected_spell.button_pressed and not selected_spell.disabled and not spell_confirm.disabled and InteractionPresenter.uses_application_modal_region(spell_request) and InteractionPresenter.preferred_modal_size(spell_request, Vector2(1280, 688)) == Vector2(1080, 668), "Learn Spells keeps chosen spells removable, permits a banked-point confirmation, and uses the full application-height level rail"); level_two.pressed.emit(); assert_true(_buttons_in(spell_component.find_child("LevelSpellList", true, false)).any(func(button: Button) -> bool: return button.text.begins_with("Spell 2") and button.theme_type_variation == &"ClassicTheldrowButton" and button.toggle_mode and button.disabled and button.tooltip_text.contains("only 1 remain")), "changing levels keeps an unaffordable candidate visible with an exact disabled reason")
