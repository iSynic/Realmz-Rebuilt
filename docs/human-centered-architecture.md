...
A subsequent physical-layout audit found that the earlier gate did not enforce the plan's feature-folder topology. The manifest now names the exact final directories under all six source boundaries, and the verifier exposes three additional ratchets. The first corrective batches moved the persistent shell/navigation composition into `src/ui/shell`, its generic request host into `src/ui/shared/interactions`, retained map and dungeon rendering into `src/ui/exploration`, and tactical rendering/playback into `src/ui/combat`. The repository still has 19 missing target feature directories, 23 undeclared legacy or generic directories, and 78 production files loose at a boundary root. These are active migration debt with final targets of zero; Beta 1 cannot treat the earlier zero size/interface counters as complete architecture certification.
