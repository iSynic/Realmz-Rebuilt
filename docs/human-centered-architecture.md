...
A subsequent physical-layout audit found that the earlier gate did not enforce the plan's feature-folder topology. The manifest now names the exact final directories under all six source boundaries, and the verifier exposes three additional ratchets. After moving the persistent shell/navigation composition into `src/ui/shell` and its generic request host into `src/ui/shared/interactions`, the repository still has 21 missing target feature directories, 23 undeclared legacy or generic directories, and 94 production files loose at a boundary root. These are active migration debt with final targets of zero; Beta 1 cannot treat the earlier zero size/interface counters as complete architecture certification.
