...
func _test_public_classic_encounter_iterations(content: RealmzContent) -> void:
	...
	assert_equal([second_choice.state, second_choice.interaction.kind, second_choice.interaction.body.to_data().get("options", []).size(), simple_state.scenario_progress.encounters.attempts(&"simple", 0)], [ScenarioVmResult.State.WAITING, InteractionRequest.ENCOUNTER_CHOICE, 2, 1], "Simple Encounter repeats after its result and removes the source-selected option")
	...
	if simple_save != null:
		...
		assert_equal([simple_done.state, restored_simple_state.scenario_progress.encounters.attempts(&"simple", 0)], [ScenarioVmResult.State.COMPLETED, 3], "Simple Encounter exits after the authored maximum number of selections")
	...
	assert_equal([cancelled.state, cancelled_state.scenario_progress.encounters.attempts(&"complex", 0)], [ScenarioVmResult.State.COMPLETED, 1], "backing out exits a repeating Complex Encounter without consuming another attempt")
...
func _test_public_thief_encounter(content: RealmzContent) -> void:
	...
	assert_equal([repeated_after_thief.state, repeated_after_thief.interaction.kind, loop_state.scenario_progress.encounters.attempts(&"complex", loop_complex.id), loop_state.world.trigger_is_disabled("ap.thief-loop")], [ScenarioVmResult.State.WAITING, InteractionRequest.WORD_AND_ACTION, 1, false], "Thief result retains its issuing AP context while returning to the source Complex Encounter")
...
func _test_public_classic_combat_spawn(content: RealmzContent) -> void:
	...
	assert_equal([result.state, spawned != null, spawned.traitor if spawned != null else false, field.has_actor(spawned_id), field.actor_position(spawned_id), state.combat.turns.turn_order().back(), spawn_event.payload.get("coordinates") if spawn_event != null else [], result.events.any(func(event: DomainEvent) -> bool: return event.kind == &"sound_requested" and event.payload.get("soundId") == 605)], [ScenarioRuntimeOperationResult.State.COMPLETED, true, true, true, Vector2i(48, 48), spawned_id, [Vector2i(48, 48)], true], "opcode 124 places a battle-macro summon through Castle's expanding complete-footprint scan, appends its turn, and publishes its authored cue")
...
func _test_public_opcode_61_land_shift(content: RealmzContent) -> void:
	...
	assert_equal([event.kind, event.payload.get("deltaX"), event.payload.get("deltaY"), event.payload.get("requestedDeltaX"), event.payload.get("requestedDeltaY"), event.payload.get("clamped"), state.world.was_visited(map.id, state.party.coordinate)], [&"party_shifted", -2, 4, -10, 10, true, true], "opcode 61 reports the committed boundary shift while retaining its requested random delta and visited destination")
