...
- A package-install worker retains one repository instance across sequential operations so an unchanged immutable package can reuse the repository's canonical-path, modification-time, and byte-count cache during the same application process. First access still performs complete integrity, schema, reference, and typed-content validation.
