...
- The ordinary inventory workspace is character-first, then exact-instance-first. It renders the item view's typed action availability and trade recipients and emits stable IDs only; it never reproduces slot, restriction, curse, capacity, or load rules. It has no player Store command unless a separate source-backed service contract is introduced.
