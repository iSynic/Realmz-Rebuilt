...
func _test_exploration_map_camera_preserves_viewport_geometry() -> void:
	...
	map_presenter.set_topology_debug_visible(true); var debug_replaced_retained_surface := not retained_surface.visible; map_presenter.set_topology_debug_visible(false); assert_true(debug_replaced_retained_surface and retained_surface.visible and retained_base_layer == retained_surface.find_children("*", "TileMapLayer", true, false)[0], "the debug overlay immediately replaces and restores the retained map surface without rebuilding renderer nodes")
