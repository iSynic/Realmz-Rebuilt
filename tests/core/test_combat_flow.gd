...
func _test_public_command_automation_matrix() -> void:
	...
	var delayed := rules.combat_flow.submit_action(state, content, actor.id, &"delay", "", _zeros(8))
	assert_true(delayed.ok, "Delay commits through the public command boundary")
