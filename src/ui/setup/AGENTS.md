...
- The Character Files empty state distinguishes a new empty vault from preserved revisions rejected by the current format or validation contract; presentation displays the host-supplied reason and never offers migration or deletion.
...
- Campaign discovery labels packages declaring deferred legacy references as ready with compatibility warnings. The selected summary exposes the validated warning count and exact detached summaries without claiming that warning admission proves Castle parity.
- Scenario installation uses Godot's embedded `FileDialog`, keeping drives, folders, file choices, and path entry inside the same focus and controller ownership model on every desktop platform.
- Terminal package failures retain their exact host-supplied message in the operation panel with retry/selection available and progress/Cancel hidden. A new operation replaces the failure; successful completion clears it.
