...
func _test_location_note_workspace() -> void:
	...
	assert_true(editor != null and body.find_child("MapsNotesTabs", true, false) != null and body.find_child("SavedLocationNotes", true, false) != null and body.find_child("JournalEntryDetail", true, false) != null and body.find_child("HistoricalLocationMap", true, false) != null and (body.find_child("LocationNote_1", true, false) as Button).button_pressed and MapTextureCache.darkness_mask_asset_id(3) == "classic-darkness-mask-3", "Maps/Notes selects a saved place, renders its detached recentered map with the saved Classic darkness mask, and retains the current-note editor and authored journal detail")
	assert_equal(editor.text, "Watch the ridge.", "the editor begins from detached committed note text"); assert_true(_labels_in(body).has("Journal entry 0") and (body.find_child("JournalEntryText", true, false) as Label).text.is_empty(), "an empty authored Data SD2 record retains its stable identity and renders as an immutable empty journal page")
	...
	var note_payload := intents[0].payload as ExplorationIntentPayloads.LocationNote
...
func _test_battle_weapon_mode_component() -> void:
	...
	var turn_icon := GradientTexture1D.new(); var component := instantiate_ui_scene("res://src/ui/interaction_components/battle_interaction.tscn") as BattleInteraction; var submitted_bodies: Array[InteractionResponse.Body] = []; component.response_body_submitted.connect(func(body: InteractionResponse.Body) -> void: submitted_bodies.append(body)); component.theme = load("res://src/ui/classic_ui_theme.tres"); component.configure({"hero": turn_icon, "monster": turn_icon}); component.build(request)
...
func _test_battle_live_reflow() -> void:
	var request := _fixture_request("battle.commands", InteractionRequest.COMBAT, {"actions": ["finish", "defend", "switch_weapon", "cast_spell", "use_item", "retreat"], "weaponMode": "missile", "weaponSwitch": {"enabled": true, "reason": "", "targetMode": "melee"}, "retreat": {"enabled": false, "reason": "An enemy is too close."}}); var live_presenter := load("res://src/ui/interaction_presenter.tscn").instantiate() as InteractionPresenter; (Engine.get_main_loop() as SceneTree).root.add_child(live_presenter)
	await (Engine.get_main_loop() as SceneTree).process_frame
	live_presenter.set_classic_regions(Rect2(0.0, 28.0, 992.0, 502.0), Rect2(8.0, 530.0, 984.0, 190.0), Rect2(0.0, 530.0, 1280.0, 190.0)); live_presenter.present(request); var live_component := live_presenter.find_child("BattleOverview", true, false).get_parent() as BattleInteraction; var live_attack := live_component.find_child("CombatCommandAttack", true, false) as Button; live_presenter.set_classic_regions(Rect2(0.0, 42.0, 1916.0, 753.0), Rect2(12.0, 795.0, 1916.0, 285.0), Rect2(0.0, 795.0, 2474.0, 285.0)); assert_true(live_presenter.find_child("BattleOverview", true, false).get_parent() == live_component and live_attack.custom_minimum_size.y > 57.0 and live_attack.get_theme_font_size("font_size") == 27, "the presenter reapplies wide-region command scale to the existing battle component instead of rebuilding and resetting it"); live_presenter.present(null); live_presenter.free()
...
func _test_battle_typed_option_contracts() -> void:
	...
	var component := instantiate_ui_scene("res://src/ui/interaction_components/battle_interaction.tscn") as BattleInteraction; component.theme = load("res://src/ui/classic_ui_theme.tres"); component.build(request)
	...
	for power: int in range(3, 8): opened_options.append(InteractionCombatValueDecoder.cast_option({"spellId": "classic.spell.1306", "spellName": "Brimstones", "power": power, "cost": power * 2, "targetId": "", "targetName": "All enemies", "targetCurrentHealth": -1, "targetMaximumHealth": -1, "targetMode": "automatic"}, &"spell"))
	var roster := load("res://src/ui/screens/classic_party_roster.tscn").instantiate() as ClassicPartyRoster; roster.theme = load("res://src/ui/classic_ui_theme.tres"); roster.size = Vector2(350.0, 560.0); roster.present(spellbook_view); roster.present_combat_spellbook(String(opened["actor"]), opened_options); var spellbook_labels := _labels_in(roster)
...
func _test_temple_component() -> void:
	...
	var component := instantiate_ui_scene("res://src/ui/interaction_components/temple_interaction.tscn") as TempleInteraction
...
func _test_bank_component() -> void:
	...
	var component := instantiate_ui_scene("res://src/ui/interaction_components/bank_interaction.tscn") as BankInteraction
	var submitted: Array[Dictionary] = []
	component.response_body_submitted.connect(func(body: InteractionResponse.Body) -> void: submitted.append(body.to_data()))
	component.build(request)
	var buttons := _buttons_in(component)
	...
	var departure_component := instantiate_ui_scene("res://src/ui/interaction_components/bank_interaction.tscn") as BankInteraction
...
func _test_settings_schema_and_migration() -> void:
	...
	var preferred_move := ApplicationCombatPolicy.body_with_preferences(move_body, restored)
	...
	var preferred_attack := ApplicationCombatPolicy.body_with_preferences(attack_body, restored)
...
func _test_movement_input() -> void:
	...
	var no_op_ap: Array[DomainEvent] = [DomainEvent.new(&"trigger_fired")]; var random_gate: Array[DomainEvent] = [DomainEvent.new(&"random_region_triggered")]; var transition: Array[DomainEvent] = [DomainEvent.new(&"map_transitioned")]; var blocked: Array[DomainEvent] = [DomainEvent.new(&"movement_blocked")]
	assert_equal([HeldMovementControllerScript.continues_after_step(Vector2i.RIGHT, "land:0", Vector2i.ZERO, "land:0", Vector2i.RIGHT, no_op_ap, true), HeldMovementControllerScript.continues_after_step(Vector2i.RIGHT, "land:0", Vector2i.ZERO, "land:0", Vector2i.RIGHT, random_gate, true), HeldMovementControllerScript.continues_after_step(Vector2i.RIGHT, "land:0", Vector2i.ZERO, "land:0", Vector2i(4, 4), no_op_ap, true), HeldMovementControllerScript.continues_after_step(Vector2i.RIGHT, "land:0", Vector2i.ZERO, "land:1", Vector2i.ZERO, transition, true), HeldMovementControllerScript.continues_after_step(Vector2i.RIGHT, "land:0", Vector2i.ZERO, "land:0", Vector2i.ZERO, blocked, true)], [true, true, false, false, true], "a harmless random-rectangle gate and completed no-op AP preserve held travel, while relocation and map boundaries stop it and ordinary collision permits steering")
	held.stop(&"keyboard"); held.advance(1.0)
	assert_equal(pulses.size(), 5, "release stops further held movement"); held.free()
...
func _test_fast_spell_input() -> void:
	...
	var bindings: Array[InteractionRequestValue.FastSpell] = [InteractionCombatValueDecoder.fast_spell({"slot": 0, "spellId": "classic.spell.1101", "spellName": "Discover Magic", "power": 1, "enabled": true, "reason": ""}), InteractionCombatValueDecoder.fast_spell({"slot": 1, "spellId": "classic.spell.1102", "spellName": "Flame Hands", "power": 2, "enabled": false, "reason": "Not enough spell points."})]
...
func _test_fixture_gallery_coverage() -> void:
	...
	var age_component := load("res://src/ui/interaction_components/age_update_interaction.tscn").instantiate() as AgeUpdateInteraction
	...
	age_component.free()
	var item_media := ClassicMediaCatalog.new(null, ApplicationMediaCatalog.new()); var recovery_component := instantiate_ui_scene("res://src/ui/interaction_components/treasure_distribution_interaction.tscn") as TreasureDistributionInteraction; recovery_component.configure(item_media, null, false)
	...
	var ordinary_component := instantiate_ui_scene("res://src/ui/interaction_components/treasure_distribution_interaction.tscn") as TreasureDistributionInteraction; ordinary_component.configure(item_media, null, false)
	...
	assert_true(ordinary_component.find_child("TreasureSwapControls", true, false) != null and ordinary_component.find_child("TreasureSwapCharacter", true, false) != null and ordinary_component.find_child("TreasureSwap_to-character_gold", true, false) != null, "ordinary booty exposes its scene-authored exact-denomination wealth transfer controls")
	...
	var capacity_component := instantiate_ui_scene("res://src/ui/interaction_components/treasure_distribution_interaction.tscn") as TreasureDistributionInteraction; capacity_component.configure(item_media, null, false)
	...
	var spell_request := ClassicUiFixtureGallery.request_for(InteractionRequest.LEVEL_UP, &"unidentified"); var spell_body := spell_request.body as LevelUpRequestBody; spell_body.point_total = 2; var spell_component := (load("res://src/ui/interaction_components/level_up_interaction.tscn") as PackedScene).instantiate() as LevelUpInteraction; spell_component.build(spell_request)
	...
	var selection_component := instantiate_ui_scene("res://src/ui/interaction_components/selection_interaction.tscn") as SelectionInteraction; selection_component.configure(null, selection_view); selection_component.build(request)
	...
	roster.set_media_catalog(item_media); roster.present(selection_view, "hero"); var ordinary_rows := _buttons_in(roster).filter(func(button: Button) -> bool: return button.has_meta("character_id")); var dead_portrait: PackedByteArray = (ordinary_rows[2] as Button).icon.get_image().get_data(); selection_view.party_members[2].current_health = -1; roster.present(selection_view, "hero"); var unconscious_portrait: PackedByteArray = (_buttons_in(roster).filter(func(button: Button) -> bool: return button.has_meta("character_id"))[2] as Button).icon.get_image().get_data()
	selection_view.party_members[2].current_health = -10; roster.present(selection_view, "hero"); ordinary_rows = _buttons_in(roster).filter(func(button: Button) -> bool: return button.has_meta("character_id")); var current_markers := roster.find_children("CurrentCharacterMarker", "ColorRect", true, false).filter(func(marker: Node) -> bool: return (marker as ColorRect).color.a > 0.0); assert_true(ordinary_rows.all(func(button: Button) -> bool: return not button.toggle_mode and not button.button_pressed) and current_markers.size() == 1 and String(current_markers[0].get_meta("character_id")) == "hero" and ordinary_rows[2].icon.get_size() == Vector2(50, 50) and dead_portrait != unconscious_portrait, "the ordinary Party rail marks exactly one current character, shades an unconscious portrait, and adds a distinct skull layer at Castle's exact death threshold"); ordinary_rows[1].pressed.emit(); current_markers = roster.find_children("CurrentCharacterMarker", "ColorRect", true, false).filter(func(marker: Node) -> bool: return (marker as ColorRect).color.a > 0.0); ordinary_rows[1].pressed.emit(); assert_equal([current_characters, activated_characters, current_markers.size(), String(current_markers[0].get_meta("character_id"))], [["mage"], ["mage"], 1, "mage"], "clicking another row moves the current marker, while clicking the current row activates that exact Character record")
	var mage_row_instance_id: int = (ordinary_rows[1] as Button).get_instance_id(); selection_view.party_members[1].maximum_spell_points = 100; selection_view.party_members[1].spell_points = 5; roster.present_ordinary_exploration(selection_view, "mage"); ordinary_rows = _buttons_in(roster).filter(func(button: Button) -> bool: return button.has_meta("character_id")); assert_true((ordinary_rows[1] as Button).get_instance_id() == mage_row_instance_id and (ordinary_rows[1] as Button).text.contains("SP 5/100"), "an hourly ordinary-movement refresh updates recovered spell points in the existing Party row without rebuilding the roster")
	roster.present_character_selection(request)
	var rows := _buttons_in(roster).filter(func(button: Button) -> bool: return button.has_meta("character_id"))
	...
	rows = _buttons_in(roster).filter(func(button: Button) -> bool: return button.has_meta("character_id"))
	rows[1].pressed.emit()
	...
	assert_false(_labels_in(roster.find_child("PartyList", true, false)).any(func(text: String) -> bool: return text in ["1", "2"]), "leaving the interaction clears temporary Party-list numbering")
	roster.free()
...
func _test_lifecycle_interaction() -> void:
	...
	var component := instantiate_ui_scene("res://src/ui/interaction_components/lifecycle_interaction.tscn") as InteractionComponent
	var submitted: Array[Dictionary] = []
	component.response_body_submitted.connect(func(body: InteractionResponse.Body) -> void: submitted.append(body.to_data()))
	component.build(request)
	var buttons := _buttons_in(component)
	...
	var idle_quit := ApplicationLifecycleScript.quit_application_request(false, false); assert_equal(idle_quit.body.to_data()["options"].size(), 2, "Quit without an active session offers only quit and cancel"); assert_equal([ApplicationLifecycleHost.response_owner(true, true), ApplicationLifecycleHost.response_owner(false, true), ApplicationLifecycleHost.response_owner(false, false)], [&"host", &"standalone-creator", &"session"], "a host Quit confirmation owns its response before standalone character creation or campaign-session interactions")
	quit_component.free()
...
func _test_classic_choice_context() -> void:
	...
	var journal_component := instantiate_ui_scene("res://src/ui/interaction_components/text_choice_interaction.tscn") as TextChoiceInteraction; var journal_payloads: Array[Dictionary] = []
	...
	assert_equal(journal_payloads, [{"takeNote": true}], "an explicitly enabled Auto Note preference preserves the typed journal flag on ordinary acknowledgement"); journal_component.free(); var default_journal := instantiate_ui_scene("res://src/ui/interaction_components/text_choice_interaction.tscn") as TextChoiceInteraction; var default_journal_payloads: Array[Dictionary] = []; default_journal.response_body_submitted.connect(func(body: InteractionResponse.Body) -> void: default_journal_payloads.append(body.to_data())); default_journal.build(journal_request); assert_true(default_journal.submit_acknowledgement(), "default-off Auto Note retains the ordinary acknowledgement boundary"); assert_equal(default_journal_payloads, [{}], "Castle's PRFN default-off Auto Note advances without a record flag unless the player uses manual N"); default_journal.free()
	var empty_acknowledgement := instantiate_ui_scene("res://src/ui/interaction_components/text_choice_interaction.tscn") as TextChoiceInteraction; empty_acknowledgement.build(InteractionRequest.acknowledge("empty-text", "")); assert_equal(_buttons_in(empty_acknowledgement).map(func(button: Button) -> String: return button.text), ["Continue"], "an authored empty positive message exposes an explicit acknowledgement instead of an internal interaction label"); empty_acknowledgement.free()
	...
	var scrolling_presenter := load("res://src/ui/interaction_presenter.tscn").instantiate() as InteractionPresenter
	...
	await (Engine.get_main_loop() as SceneTree).process_frame
	var yes_no := instantiate_ui_scene("res://src/ui/interaction_components/text_choice_interaction.tscn") as TextChoiceInteraction; yes_no.build(InteractionRequest.yes_no("layout-choice", "Continue?", "Yes", "No"))
...
func _test_exploration_map_camera_preserves_viewport_geometry() -> void:
	...
	var viewport_cells := MapPresentationGeometry.viewport_cells_for(map_presenter.size, map_presenter.map_origin.y, map_presenter.cell_size); var projection_cells := MapPresentationGeometry.projection_cells_for(map_presenter.size, map_presenter.map_origin.y, map_presenter.cell_size)
	var draw_origin := MapPresentationGeometry.map_draw_origin_for(map_presenter.size, map_presenter.map_origin, map_presenter.cell_size, viewport_cells)
	...
	for coordinate: Vector2i in positions:
		...
		var camera := MapPresentationGeometry.camera_top_left(coordinate, map_size, viewport_cells)
		...
		assert_equal(MapPresentationGeometry.viewport_cells_for(map_presenter.size, map_presenter.map_origin.y, map_presenter.cell_size), viewport_cells, "party position %s keeps the bounded cell window" % coordinate)
		assert_equal(MapPresentationGeometry.map_draw_origin_for(map_presenter.size, map_presenter.map_origin, map_presenter.cell_size, viewport_cells), draw_origin, "party position %s keeps the map draw origin inside the fixed viewport" % coordinate)
	...
	assert_equal([MapPresentationGeometry.darkness_mask_rect(Rect2(100.0, 100.0, 32.0, 32.0)), RetainedMapSurfaceScript.darkness_memory_opacity(), memory_rects], [Rect2(-60.0, -60.0, 320.0, 320.0), 0.8, [Rect2(0.0, 0.0, 64.0, 32.0), Rect2(96.0, 0.0, 32.0, 32.0)]], "the fixed Classic darkness mask keeps its source-aligned party anchor while contiguous discovered terrain remains beneath its authored veil")
	assert_true(party_rects[0].position.x != party_rects[2].position.x and party_rects[0].size == party_rects[2].size and MapPresentationGeometry.classic_visible_rect(Vector2i(45, 45), map_size) == Rect2i(37, 39, 15, 13) and MapPresentationGeometry.classic_visible_rect(Vector2i.ZERO, map_size) == Rect2i(0, 0, 15, 13) and MapPresentationGeometry.land_discovery_coordinates([Vector2i(45, 45)], map_size).has(Vector2i(37, 39)) and MapPresentationGeometry.land_discovery_coordinates([Vector2i(45, 45)], map_size).has(Vector2i(51, 51)) and not MapPresentationGeometry.land_discovery_coordinates([Vector2i(45, 45)], map_size).has(Vector2i(36, 39)) and [MapPresentationGeometry.facing_label(Vector2i.UP), MapPresentationGeometry.facing_label(Vector2i.DOWN + Vector2i.LEFT)].all(func(label: String) -> bool: return label in ["N", "SW"]) and MapPresentationGeometry.random_region_outline_segments(Rect2i(10, 10, 4, 3), Rect2i(0, 0, 20, 20)).size() == 4 and MapPresentationGeometry.random_region_outline_segments(Rect2i(10, 10, 4, 3), Rect2i(11, 9, 10, 10)).size() == 3 and ClassicUiAssetCatalog.texture(&"map.party.camp") != null and ResourceLoader.exists("res://src/ui/assets/ui/classic-exploration-surround-tile.png", "Texture2D"), "map presentation preserves viewport geometry, exact clipped random-rectangle outlines, Castle's 15x13 current and remembered views, seamless unrevealed surround, exact camp-marker availability, and typed cardinal/intercardinal facing labels")
	...
	map_presenter.set_topology_debug_visible(true); var debug_replaced_retained_surface := not retained_surface.visible; map_presenter.set_topology_debug_visible(false); assert_true(debug_replaced_retained_surface and not map_presenter.show_cell_topology_details and retained_surface.visible and retained_base_layer == retained_surface.find_children("*", "TileMapLayer", true, false)[0], "the AP and random-rectangle overlay stays separate from dense cell details while immediately replacing and restoring the retained map surface without rebuilding renderer nodes")
	...
	map_presenter.present(GameView.new(3, true, null, second_los_view.map_id, remembered_room, 0, 12, 0, second_los_view)); assert_equal([remembered_room_initial_blackout, los_blackout.get_cell_source_id(first_room), los_blackout.get_cell_source_id(remembered_room), MapPresentationGeometry.los_cell_requires_blackout(true, false), MapPresentationGeometry.los_cell_requires_blackout(false, false)], [0, 0, -1, true, false], "the live LOS projection blacks a previously seen room, blacks the room just left, and incrementally reveals only the newly visible room")
	var package := load_test_package("res://tests/fixtures/packages/realmz2-synthetic-fixture.realmz2"); assert_true(package.is_ok(), "the retained dungeon proof loads the validated package fixture")
	if package.is_ok():
...
func _test_battlefield_presenter() -> void:
	var presenter := ClassicBattlefieldPresenter.new(); var control_size := Vector2(912.0, 486.0); var visible_cells := BattlefieldPresentationGeometry.viewport_cells_for(control_size)
	...
	var tracked_camera := Vector2i(30, 34); var draw_origin := BattlefieldPresentationGeometry.battlefield_draw_origin(control_size, visible_cells); var rendered_coordinate := Vector2i(40, 39); var rendered_point := BattlefieldPresentationGeometry.cell_rect(rendered_coordinate, tracked_camera, draw_origin).get_center()
	assert_true(BattlefieldPresentationGeometry.coordinate_for_point(rendered_point, tracked_camera, visible_cells, control_size) == rendered_coordinate and BattlefieldPresentationGeometry.click_direction_for_point(BattlefieldPresentationGeometry.cell_rect(rendered_coordinate, tracked_camera, draw_origin), rendered_point + Vector2(90.0, -65.0)) == Vector2i(1, -1) and BattlefieldPresentationGeometry.tracked_camera_top_left(tracked_camera, Vector2i(tracked_camera.x + visible_cells.x - 1, 39), visible_cells) == BattlefieldPresentationGeometry.camera_top_left(Vector2i(tracked_camera.x + visible_cells.x - 1, 39), visible_cells), "battlefield input uses the rendered camera and eight centered sectors while edge focus recenters the view")
	var large_move := CombatPlaybackFrame.new(&"move_start", 1.0); large_move.from_coordinate = Vector2i(10, 10); large_move.to_coordinate = Vector2i(11, 10); large_move.progress = 0.5; assert_equal([BattlefieldPresentationGeometry.footprint_rect([], Vector2i.ZERO, Vector2.ZERO), BattlefieldPresentationGeometry.moving_footprint_rect(BattlefieldGrid.footprint_cells(Vector2i(10, 10), 3), Vector2i(10, 10), large_move, Vector2i.ZERO, Vector2.ZERO), BattlefieldPresentationGeometry.classic_monster_icon_id(384, true)], [Rect2(), Rect2(304.0, 288.0, 64.0, 64.0), 692], "terminal playback tolerates a removed combatant while a moving two-by-two monster preserves its upper-left footprint offset and authored right-facing CICN")
	var view := _combat_playback_view(20, Vector2i(45, 45), Vector2i(47, 45), &"active")
	presenter.present(view)
	presenter.interaction.set_movement_costs_visible(true)
	var playback_move := CombatPlaybackFrame.new(&"move_start", 0.18); playback_move.actor_id = "hero"; playback_move.camera_focus_id = "hero"; presenter.interaction.focus_combatant("monster"); presenter.present_playback_frame(playback_move); presenter.clear_playback_frame(); assert_true(presenter.interaction.movement_costs_visible and view.combat_view.persistent_fields.size() == 1 and view.combat_view.persistent_fields[0].affected_coordinates == [Vector2i(45, 44), Vector2i(45, 45)] and BattlefieldTextureCache.persistent_field_tile_id(view.combat_view.persistent_fields[0].queue_icon) == 214 and presenter.interaction.focused_combatant_id.is_empty() and BattlefieldPresentationGeometry.camera_focus_id_for(presenter.playback_frame(), presenter.interaction.focused_combatant_id, view.combat_view.active_actor_id) == "hero", "the battlefield renders detached persistent-field cells and committed playback retires stale target focus so consecutive actions remain centered on the active actor"); presenter.free()
...
func _combat_playback_view(monster_health: int, hero_position: Vector2i, monster_position: Vector2i, outcome: StringName) -> GameView:
	var tiles: Array[int] = []; tiles.resize(BattlefieldGrid.CELL_COUNT); tiles.fill(232)
	var battlefield := BattlefieldState.new("land:0", tiles)
	assert_true(battlefield.actors.place_character("hero", hero_position), "playback fixture places the party actor")
	var monster := MonsterState.new("monster", "classic.monster.1", "Goblin", monster_health, 20); monster.icon_id = 384
	assert_true(battlefield.actors.place_monster(monster.id, monster_position, 0), "playback fixture places the target")
...
func _test_field_spell_workspace() -> void:
	...
	var cast_payload := submitted[0].payload as SpellIntentPayload
...
func _test_inventory_workspace() -> void:
	...
	var controller := InventoryScreenController.new(); var intents: Array[PlayerIntent] = []; var back_requests := [0]
	controller.intent_submitted.connect(func(intent: PlayerIntent) -> void: intents.append(intent))
	...
	var buttons := _base_buttons_in(body); var ledger_theme := load("res://src/ui/classic_ui_theme.tres") as Theme
...
func _test_money_workspace() -> void:
	...
	var pool_payload := intents[0].payload as EconomyIntentPayloads.Money
	var to_pool_payload := intents[1].payload as EconomyIntentPayloads.Money
	var to_character_payload := intents[2].payload as EconomyIntentPayloads.Money
	...
	assert_equal([to_character_payload.action, to_character_payload.character_id, to_character_payload.denomination, to_character_payload.amount], [&"to-character", source.id, "gold", 5], "pool-to-character Swap carries stable identity and exact Classic increment"); var retained_screen := instantiate_ui_scene("res://src/ui/screens/services_screen.tscn") as ServicesScreen; controller.present(retained_screen, view, media); controller.present(retained_screen, view, media); var retained_values := retained_screen.workspace().pool_summary().get_node("MoneyPoolValues")
	assert_true(buttons.any(func(button: Button) -> bool: return button.text == "Done") and ["Gold", "Gems", "Jewelry"].all(func(node_name: String) -> bool: return retained_values.has_node(node_name)), "Swap has a presentation-only Done path and retained rebinds keep the editor-authored wealth-chip identities stable"); retained_screen.free()
	body.free()
...
func _test_character_sheet_workspace() -> void:
	...
	var sheet := (load("res://src/ui/screens/classic_character_sheet.tscn") as PackedScene).instantiate() as ClassicCharacterSheet; sheet.present([view], view.id, {}, 1.0, &"equipment", [], [], ActionAvailabilityView.new(&"change_character_appearance", true), null, UiLayoutProfile.COMPACT)
...
func _test_scene_composition() -> void:
	...
	var party_effects_row := shell.find_child("PartyEffectsRow", true, false) as BoxContainer; var party_command_column := shell.find_child("PartyCommandColumn", true, false) as VBoxContainer; var effects_panel := shell.find_child("EffectsPanel", true, false) as PanelContainer; var effects_grid := shell.find_child("EffectsGrid", true, false) as GridContainer; assert_true(party_effects_row != null and not party_effects_row.vertical and party_command_column != null and party_command_column.get_parent() == party_effects_row and effects_panel.get_parent() == party_effects_row and party_command_column.get_index() < effects_panel.get_index() and effects_panel.size_flags_horizontal == Control.SIZE_SHRINK_CENTER and effects_panel.size_flags_vertical == Control.SIZE_SHRINK_CENTER and effects_grid.columns == 4 and [GameShellLayoutPolicy.party_effect_icon_size(1), GameShellLayoutPolicy.party_effect_icon_size(2)] == [32.0, 64.0] and [GameShellLayoutPolicy.party_effect_slot_size(1), GameShellLayoutPolicy.party_effect_slot_size(2)] == [36.0, 68.0] and [ClassicPartyEffects.resource_id(1, 0), ClassicPartyEffects.resource_id(8, 7)] == [14000, 14063] and GameShellCommandController.HELD_COMMAND_INTERVAL <= 1.0 / 60.0, "the wide Party footer places a compact 2-by-2 command block left of Castle's native-pixel 4-by-2 animated effects bank while held Rest/Area Search retain one-tick non-catching-up cadence")
	var exploration_commands := ClassicCommandCatalog.for_context(&"exploration"); assert_true([&"search_mode", &"contextual", &"camp", &"heal"].all(func(id: StringName) -> bool: return exploration_commands.any(func(definition: Dictionary) -> bool: return definition["id"] == id and definition["group"] == &"world")) and ClassicCommandCatalog.command(&"camp")["asset_id"] == &"command.camp" and ClassicCommandCatalog.command(&"camp")["art_region"] == [9, 3, 32, 33] and ClassicCommandCatalog.command(&"contextual")["asset_id"] == &"command.encounter_original" and ClassicCommandCatalog.command(&"contextual")["art_mask"] == &"circle" and ClassicUiAssetCatalog.definition(&"command.shop_original").get("source_resource_id", -1) == 225 and ClassicUiAssetCatalog.texture(&"command.shop_original") != null and load("res://src/ui/assets/ui/commands/shop.png") is Texture2D and ClassicCommandCatalog.command(&"heal")["asset_path"] == "res://src/ui/assets/ui/commands/heal.png" and ClassicCommandCatalog.command(&"heal")["art_region"] == [13, 18, 47, 40] and ClassicCommandCatalog.command(&"heal")["hold_repeat"] and not GameShellCommandController.should_stop_held_command_on_button_up(true) and GameShellCommandController.should_stop_held_command_on_button_up(false) and [GameShellCommandController.command_route(&"money"), GameShellCommandController.command_route(&"inventory"), GameShellCommandController.command_route(&"spells"), GameShellCommandController.command_route(&"maps")].all(func(route: StringName) -> bool: return not route.is_empty()), "World and Party controls separate exact or app-owned command art from live captions while retaining stable routes and pressed identities"); assert_equal([GameShellCommandController.command_activation_sound_id(&"rest", false), GameShellCommandController.command_activation_sound_id(&"area_search", false), GameShellCommandController.command_activation_sound_id(&"rest", true), GameShellCommandController.command_activation_sound_id(&"contextual", false)], [6001, 6001, 0, 141], "held Rest and Area Search request Castle sound 6001 once while Castle's shared contextual control requests sound 141 on activation")
	...
	assert_equal(ApplicationSpatialLayout.classic_textbox_rect(Rect2(0.0, 32.0, 992.0, 498.0), 190.0, 1280.0), Rect2(0.0, 530.0, 1280.0, 190.0), "the application retains one full-width fallback until the shell's exact narrative well settles")
	...
	for viewport_size: Vector2 in [Vector2(800, 600), Vector2(1280, 720)]:
		var profile := UiLayoutProfile.for_viewport(viewport_size, PresentationSettings.UI_SCALE_AUTO)
		var rect := ScreenNavigator.campaign_rect_for(profile, viewport_size); var spell_rect := ScreenNavigator.spell_screen_rect_for(profile, viewport_size); var expected_spell_width := (288.0 if profile.id == UiLayoutProfile.COMPACT else 420.0) * profile.ui_scale
		assert_true(rect.position.x >= 0.0 and rect.end.x <= viewport_size.x - profile.party_width and spell_rect == Rect2(viewport_size.x - expected_spell_width, profile.menu_height, expected_spell_width, viewport_size.y - profile.menu_height) and GameShellLayoutPolicy.exploration_footer_width(viewport_size, profile, &"spells") == spell_rect.position.x and GameShellLayoutPolicy.exploration_footer_width(viewport_size, profile, &"exploration") == viewport_size.x, "field spellcasting owns the complete right edge while the footer ends at its left edge at %s" % viewport_size)
	assert_equal([GameShellLayoutPolicy.party_roster_height(720.0, 28.0, 502.0, true), GameShellLayoutPolicy.party_roster_z_index(true)], [692.0, 81], "combat spellcasting expands above the inert Party-command footer and owns the complete right rail"); assert_equal([GameShellLayoutPolicy.party_roster_height(720.0, 28.0, 502.0, false), GameShellLayoutPolicy.party_roster_z_index(false)], [502.0, 14], "closing the combat spellbook restores the ordinary six-character roster stage"); assert_equal([GameShellLayoutPolicy.combat_spellbook_roster_width(800.0, 208.0, 1.0, true), GameShellLayoutPolicy.combat_spellbook_roster_width(800.0, 208.0, 1.0, false), GameShellLayoutPolicy.combat_spellbook_roster_width(1280.0, 352.0, 1.0, true), GameShellLayoutPolicy.combat_spellbook_stage_width(592.0, 800.0, 352.0), GameShellLayoutPolicy.combat_spellbook_stage_width(928.0, 1280.0, 352.0)], [352.0, 208.0, 352.0, 448.0, 928.0], "combat spellcasting overlays enough of the compact battlefield to keep its complete selector and all targeting actions inside the viewport, then restores the ordinary roster width")
	var modal_parent := Control.new(); (Engine.get_main_loop() as SceneTree).root.add_child(modal_parent); var modal_presenter := load("res://src/ui/interaction_presenter.tscn").instantiate() as InteractionPresenter; modal_parent.add_child(modal_presenter); await (Engine.get_main_loop() as SceneTree).process_frame; var modal_request := InteractionRequest.yes_no("modal-order", "Choose.", "Yes", "No"); var textbox_overlay_style := InteractionLayoutPolicyScript.textbox_theme_variation(); var combat_footer_style := InteractionLayoutPolicyScript.textbox_theme_variation(ClassicUiFixtureGallery.request_for(InteractionRequest.COMBAT)); modal_presenter.present(modal_request); var modal_shield := modal_parent.get_node("LockedModalShield") as Control; var first_order := [modal_shield.get_index(), modal_presenter.get_index()]; modal_presenter.present(modal_request); var repeated_order := [modal_shield.get_index(), modal_presenter.get_index()]; var floating_choice_rect := InteractionLayoutPolicyScript.floating_choice_rect(Rect2(0.0, 28.0, 992.0, 502.0), Rect2(286.0, 530.0, 620.0, 190.0), Vector2(286.0, 46.0)); var compact_choice_rect := InteractionLayoutPolicyScript.floating_choice_rect(Rect2(0.0, 28.0, 592.0, 410.0), Rect2(10.0, 438.0, 780.0, 162.0), Vector2(286.0, 46.0)); assert_true(textbox_overlay_style == &"ClassicTextboxOverlay" and combat_footer_style == &"ClassicOpenRight" and first_order[0] < first_order[1] and repeated_order[0] < repeated_order[1] and repeated_order == first_order and InteractionLayoutPolicyScript.uses_floating_choice_modal(InteractionRequest.yes_no("floating-choice", "Do you follow him?", "Yes", "No")) and floating_choice_rect.end.y <= 522.0 and floating_choice_rect.size == Vector2(520.0, 116.0) and compact_choice_rect.end.x <= 582.0 and compact_choice_rect.end.y <= 430.0, "narrative interactions use one opaque textbox surface, combat exposes the textured command slate, locked modals retain pointer order, and AP choices stay together above the narrative region in both supported profiles"); var flash_sounds: Array[int] = []; modal_presenter.presentation_sound_requested.connect(func(sound_id: int) -> void: flash_sounds.append(sound_id)); modal_presenter.queue_classic_flash_messages([{"text": "Map one", "soundId": 30005}, {"text": "Map two", "soundId": 30005}, {"text": "Map three", "soundId": 30005}]); var flash_blocks := modal_presenter.has_blocking_request(); var flash_panel := modal_parent.find_child("ClassicFlashMessage", true, false) as PanelContainer; var flash_continue := modal_parent.find_child("ClassicFlashContinue", true, false) as Button; flash_continue.pressed.emit(); var second_flash := modal_parent.find_child("ClassicFlashMessage", true, false) as PanelContainer; var second_text := (modal_parent.find_child("ClassicFlashText", true, false) as Label).text; flash_continue.pressed.emit(); var third_flash := modal_parent.find_child("ClassicFlashMessage", true, false) as PanelContainer; assert_true(flash_blocks and modal_presenter.has_blocking_request() and flash_panel == second_flash and second_flash == third_flash and third_flash.size == Vector2(520.0, 118.0) and second_text == "Map two" and (modal_parent.find_child("ClassicFlashText", true, false) as Label).text == "Map three" and flash_sounds == [30005, 30005, 30005], "source-owned flash messages reuse one compact ordered click boundary, block exploration input, and play their cue once per displayed record"); flash_continue.pressed.emit(); assert_false(modal_presenter.has_blocking_request(), "dismissing the final Classic flash message releases exploration input"); modal_parent.free()
	var spatial_view := GameView.new(1, true, null); var locked_dungeon := MapView.new("dungeon:test", "Locked Dungeon", &"dungeon", 1, 1, Vector2i.ZERO, [], false, [], {}, Vector2i.ZERO, -1, 1, false, false, -1, true, true); spatial_view.party_map_id = locked_dungeon.map_id; spatial_view.map_view = locked_dungeon; assert_true(PresentationCoordinator.should_show_exploration_stage(&"spells", spatial_view, true) and not PresentationCoordinator.dungeon_view_toggle_available(locked_dungeon) and GameShellStatusController.location_fact_text(spatial_view) == "dungeon:test • ?,? • Compass N", "field Spells preserves the live exploration renderer, a locked dungeon requires Wizard's Eye before Space exposes the overhead view, and authored coordinate/compass flags alter only their footer facts"); locked_dungeon.wizard_eye_active = true; locked_dungeon.coordinates_hidden = false; locked_dungeon.compass_enabled = false; assert_true(PresentationCoordinator.dungeon_view_toggle_available(locked_dungeon) and GameShellStatusController.location_fact_text(spatial_view) == "dungeon:test • 0,0", "Wizard's Eye restores Castle's overhead-view toggle while a disabled compass leaves the exact coordinates visible without a direction fact"); shell.free()
func _test_automatic_workflow_routes() -> void:
	...
	assert_equal(GameShellAvailability.route_change_reason(setup_view), "Begin the adventure first.", "Explore and other browsing routes stay disabled until party setup commits Begin Adventure")
	...
	var persistent_response := ApplicationCombatPolicy.persistent_auto_response(auto_view)
	assert_true(queued_off == {"characterId": "hero", "enabled": false} and ApplicationCombatPolicy.auto_change_to_queue(CombatIntents.set_auto("hero", false), false).is_empty() and ApplicationCombatPolicy.auto_abort_ids(auto_view, {"queued": true}) == ["hero", "queued"] and persistent_response != null and (persistent_response.body as InteractionResponse.CombatBody).actor_id == "hero", "the host queues individual manual control, can enumerate a full-party Escape abort, and prepares only the next Auto activation")
	assert_equal([GameShellRoutePolicy.automatic_route(&"exploration", view), GameShellRoutePolicy.automatic_route(&"inventory", view)], [&"combat", &"combat"], "battle setup replaces exploration or browsing with the tactical workspace")
	view.pending_interaction = ClassicUiFixtureGallery.request_for(InteractionRequest.ALLY_SELECTION)
	assert_equal(GameShellAvailability.route_change_reason(view), "Resolve the current interaction first.", "a mandatory post-battle response disables misleading route changes such as Adventure Explore")
	view.pending_interaction = null
	view.combat_view = null
	assert_equal(GameShellRoutePolicy.automatic_route(&"combat", view), &"exploration", "completed battle cleanup returns the ordinary shell to exploration")
	assert_equal(GameShellRoutePolicy.automatic_route(&"inventory", view), &"inventory", "ordinary non-combat workspaces remain presentation-owned")
	view.pending_interaction = _fixture_request("shop.route", InteractionRequest.SHOP)
	assert_equal(GameShellRoutePolicy.automatic_route(&"exploration", view), &"services", "application services open their dedicated workspace")
	assert_equal(GameShellRoutePolicy.automatic_route(&"inventory", view), &"services", "a service interaction replaces an unrelated browsing workspace")
	view.pending_interaction = null
	assert_equal(GameShellRoutePolicy.automatic_route(&"services", view, true), &"exploration", "completing a contextual service returns to exploration instead of leaving its interaction workspace open")
	assert_equal(GameShellRoutePolicy.automatic_route(&"services", view, false), &"services", "ordinary Money browsing remains open until the player chooses Done")
	view.pending_interaction = InteractionRequest.yes_no("action-point.route", "Will you approach?", "Yes", "No")
	assert_equal(GameShellRoutePolicy.automatic_route(&"inventory", view), &"exploration", "an Action Point interaction returns an unrelated browsing workspace to exploration")
	view.pending_interaction = null
	assert_equal(GameShellRoutePolicy.automatic_route(&"inventory", view), &"inventory", "free browsing remains presentation-owned after the interaction closes")
