...
func _test_lifecycle_interaction() -> void:
	...
	var component := LIFECYCLE_INTERACTION_SCENE.instantiate() as InteractionComponent
	var submitted: Array[Dictionary] = []
	component.response_body_submitted.connect(func(body: InteractionResponse.Body) -> void: submitted.append(body.to_data()))
	component.build(request)
	var buttons := _buttons_in(component)
	...
	var quit_component := LIFECYCLE_INTERACTION_SCENE.instantiate() as InteractionComponent
...
func _test_classic_choice_context() -> void:
	...
	var journal_component := TEXT_CHOICE_INTERACTION_SCENE.instantiate() as TextChoiceInteraction; var journal_payloads: Array[Dictionary] = []
	...
	assert_equal(journal_payloads, [{"takeNote": true}], "an explicitly enabled Auto Note preference preserves the typed journal flag on ordinary acknowledgement"); journal_component.free(); var default_journal := TEXT_CHOICE_INTERACTION_SCENE.instantiate() as TextChoiceInteraction; var default_journal_payloads: Array[Dictionary] = []; default_journal.response_body_submitted.connect(func(body: InteractionResponse.Body) -> void: default_journal_payloads.append(body.to_data())); default_journal.build(journal_request); assert_true(default_journal.submit_acknowledgement(), "default-off Auto Note retains the ordinary acknowledgement boundary"); assert_equal(default_journal_payloads, [{}], "Castle's PRFN default-off Auto Note advances without a record flag unless the player uses manual N"); default_journal.free()
	var empty_acknowledgement := TEXT_CHOICE_INTERACTION_SCENE.instantiate() as TextChoiceInteraction; empty_acknowledgement.build(InteractionRequest.acknowledge("empty-text", "")); assert_equal(_buttons_in(empty_acknowledgement).map(func(button: Button) -> String: return button.text), ["Continue"], "an authored empty positive message exposes an explicit acknowledgement instead of an internal interaction label"); empty_acknowledgement.free()
	...
	var scrolling_component := SCROLLING_TEXT_INTERACTION_SCENE.instantiate() as ScrollingTextInteraction
	...
	await (Engine.get_main_loop() as SceneTree).process_frame
	var yes_no := TEXT_CHOICE_INTERACTION_SCENE.instantiate() as TextChoiceInteraction; yes_no.build(InteractionRequest.yes_no("layout-choice", "Continue?", "Yes", "No"))
...
func _test_character_creator_workflow() -> void:
	...
	var app_spell_media := ClassicMediaCatalog.new(null, ApplicationMediaCatalog.new()); var valid_spell_preview := SPELL_EFFECT_PREVIEW_SCENE.instantiate() as ClassicSpellEffectPreview; var incomplete_spell_preview := SPELL_EFFECT_PREVIEW_SCENE.instantiate() as ClassicSpellEffectPreview; assert_true(valid_spell_preview.present(app_spell_media, "cicn", [12088, 12089, 12090, 12091, 12092, 12093, 12094, 12095]) and valid_spell_preview.find_child("StartingSpellAnimationFrames", true, false) is TextureRect and not incomplete_spell_preview.present(app_spell_media, "cicn", [11992, 11993, 11994, 11995, 11996, 11997, 11998, 11999]), "spell previews require one complete meaningful Castle-backed eight-frame sequence and omit unavailable identities"); valid_spell_preview.free(); incomplete_spell_preview.free(); router.free()
