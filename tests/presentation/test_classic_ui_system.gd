...
func _test_character_creator_workflow() -> void:
	...
	setup.creator_cancel_button.pressed.emit(); assert_equal(setup.setup_mode, &"assembly", "creator cancellation rebuilds assembly after its prior dynamic controls are freed"); var spell_state := CharacterState.new("creator.spells", "Ari", 12, 12); spell_state.spellcaster_type = 1; view.character_draft = CharacterView.new(spell_state); view.character_draft_spell_points_total = 3; view.character_draft_spell_points_remaining = 2; view.character_draft_spell_options = [CharacterSpellOptionView.new(SpellDefinition.new("classic.spell.1101", 1101, "Discover Magic", "Reveals magic."), 1, true), CharacterSpellOptionView.new(SpellDefinition.new("classic.spell.1201", 1201, "Flame Hands", "Calls flame."), 2, false)]; setup.setup_mode = &"creator"; setup.creator_step = 4; setup.render_creator_step(); assert_true(setup.creator_page.find_child("StartingSpellLevelRail", true, false) != null and setup.creator_page.find_child("StartingSpellListPanel", true, false) != null and setup.creator_page.find_child("StartingSpellRecord", true, false) != null and setup.creator_page.find_child("StartingSpellAllowance", true, false) != null, "Starting Spells keeps the Classic level rail, available records, selected facts, and allowance in one bounded workspace")
	router.free()
