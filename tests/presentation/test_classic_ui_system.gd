...
func run() -> void:
	...
	await _test_classic_click_modal()
	_test_startup_shell()
...
func _test_classic_click_modal() -> void:
	...
	var expected := InteractionPresenter.classic_click_modal_rect(Rect2(0.0, 28.0, 1280.0, 692.0), textbox_rect)
	var shield := parent.get_node_or_null("LockedModalShield") as Control
	assert_true(InteractionPresenter.uses_classic_click_modal(request) and presenter.position == expected.position and presenter.size == expected.size and expected.size == Vector2(174.0, 60.0) and presenter.theme_type_variation == &"ClassicInset" and shield != null and shield.visible, "opcode 26 uses the narrow half-height blocking click-modal geometry instead of the narrative well (expected %s at %s; got %s at %s)" % [expected.size, expected.position, presenter.size, presenter.position])
...
func _test_lifecycle_interaction() -> void:
	...
	assert_equal(buttons.map(func(button: Button) -> String: return button.text), ["Save and Quit", "Quit", "Cancel"], "Quit keeps one compact host question with three content-sized actions"); assert_true(quit_component.find_child("LifecycleActions", true, false) is HBoxContainer and quit_component.find_child("LifecycleConsequence", true, false) == null and InteractionPresenter.interaction_vertical_scroll_mode(quit_request) == ScrollContainer.SCROLL_MODE_DISABLED, "Quit omits duplicate consequence prose, keeps its actions in one centered row, and cannot acquire a content scrollbar")
	assert_equal([InteractionPresenter.preferred_modal_size(quit_request, Vector2(1280.0, 688.0)), InteractionPresenter.preferred_modal_size(quit_request, Vector2(800.0, 568.0))], [Vector2(460.0, 135.0), Vector2(460.0, 135.0)], "Quit retains the same shortest usable content frame at canonical and Classic application sizes")
...
func _test_combat_playback_controller() -> void:
	...
	assert_true(not next_actor_cues.is_empty() and (next_actor_cues[-1] as CombatPlaybackFrame).hides("monster"), "the next-turn cue retains the accumulated defeated-combatant mask instead of flashing a dead enemy back onto the battlefield")
