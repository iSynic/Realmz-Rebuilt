...
func _capture_gallery() -> void:
	...
	var treasure_completion := InteractionRequest.from_payload("gallery.treasure.completion", InteractionRequest.TREASURE_DISTRIBUTION, {"mode": "completion-confirmation", "summary": "One item remains unclaimed. Leave it behind?"})
	_interaction.present(treasure_completion, "", gallery_view, gallery_media)
	await _settle()
	await _capture("wide-treasure-completion-modal-1280x720")
	await _resize(Vector2i(800, 600)); await _capture("classic-treasure-completion-modal-800x600"); await _resize(Vector2i(1280, 720))
