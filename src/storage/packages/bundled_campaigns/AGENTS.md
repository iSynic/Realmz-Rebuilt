...
- The source-backed Prelude correction and compiler regeneration are recorded in `prelude-to-pestilence.corrections.json`. Providence must preserve the authored XAP 97 refusal branch, XAP 149 no-gold return branch, and Simple Encounter 7 opcode 44 context from the pinned Castle records; the package and provenance catalog carry the source/compiler/application/package identities and superseded identity. Keep this correction revision-guarded and do not treat a missing branch target as a VM fallback.
