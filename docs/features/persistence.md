...
Character Files presentation is owned by `src/ui/screens/vault_screen.tscn`. Its library, history, empty, and inspection regions are editor-authored; the controller binds immutable revision views into exported card and history-row scenes and never changes repository truth.
