## Presents the dynamic pick lock tumbler interaction without owning gameplay state.
...
func _draw() -> void:
	...
	var yellow_x := track.position.x + track.size.x * float(yellow_threshold) / float(ClassicPickLockRules.TRACK_END)
	var green_x := track.position.x + track.size.x * float(green_threshold) / float(ClassicPickLockRules.TRACK_END)
	...
	var marker_x := track.position.x + track.size.x * float(marker_position) / float(ClassicPickLockRules.TRACK_END)
