...
- Tests: `test_scenario_vm.gd::_test_public_classic_combat_spawn` covers normalized signatures from Castle in the Clouds `xap:273` (Spell 3208, power 1), Trouble in the Sword Lands `xap:1605` (Spell 1108, power 5, extraSaveAdjust -50), and Mithril Vault `xap:9` (Spell 1304 Destroy Magic, power 5, cannot 4), plus typed rejections for targetType 1 (`unsupported_macro_single_target`) and targetType 6 (`unsupported_macro_ray`). These are public-VM controlled cases, not execution of the packaged XAP programs or ordinary campaign routes.
...
- Castle evidence: commit `491816ad60037394f92c428e99c004494d3c28b3`, `main.c:132,1188-1195`, `loadland-loadpixmap.c:131-137`, and `buttonchoice.c:75-134,158-163,217-223`. The array has 402 zero-initialized entries. Land loading owns indices 0–200, shared Combat Data BD owns 201–400 after that overlay, and 401 remains zero. Marked entry instead falls back to the base only above 402, admitting an out-of-bounds read at 402.
- Observable oracle behavior: the index-402 result is undefined and has no portable expected time. The source-observation fixture is `tests/fixtures/oracle/classic-land-entry-mapstats-boundary.json`, SHA-256 `b21e38d2534dda7b2703950563a30ce80f033fbcae9866851d14b0dc60ece324`. This is source/control-flow evidence, not controlled Castle execution.
- Player-facing problem: entry time can depend on adjacent memory instead of an authored terrain record.
...
- Castle evidence: commit `491816ad60037394f92c428e99c004494d3c28b3`, `newland.c` cases 33, 46 and 87 plus `flashrange-loaddoor.c::loaddoor2`. War's source files are byte-identical to the bundle's pinned `ef95fcff` snapshot. Exact hashes, original operands, selected corrections and Providence commands are in `src/storage/packages/bundled_campaigns/war-in-the-sword-lands.corrections.json`.
- Observable source behavior: Simple 52's 500-gem refusal names XAP 16361 beyond the 3,696 native records; Castle does not check the failed read before using an uninitialized door. Two rum encounters use opcode -87 with shared settings 1153, interpreting its quest-check fields as an ally check and referencing nonexistent Simple 782. These are source/control-flow observations, not controlled Castle execution claims.
- Player-facing problem: following the payment refusal or those rum branches cannot produce a valid deterministic destination from the authored operands. Strict compilation exposes the invalid references when payment-result dependencies are retained.
- Chosen behavior: the owner-approved Providence edits redirect the 500-gem refusal to XAP 1224, already used by the neighboring 100-gem offer, and change only XAP 3329/3367 slot 3 from -87 to -46. The matching rum variants XAP 777/2194 already use -46 with unchanged settings 1153 to test quest 29 and call XAP 782. Preserve the negative opcode's GOSUB semantics, shared settings, all other actions, and every original source byte. Intended destinations are inferred from these authored parallels, not asserted to be the author's confirmed intent.
- Verification boundary: Providence's existing revision-guarded scalar/opcode commands own the edits. Package regeneration, exact semantic-delta checks and live route acceptance are required before the corrected bundle is accepted; compilation alone does not certify these encounter outcomes.
- Corrected Rhum caller evidence: the package-pinned `war-corrected-rhum-ap58-quest-clear` and `war-corrected-rhum-ap60-quest-clear` recipes in `tests/fixtures/oracle/ap-behavior-recipes.json` each replay eight ordinary-movement, actual-control and typed-response inputs identically from the same validated baseline. Both charge the authored 180 gold across the party, replace their issuing AP, teleport, acknowledge message 670 and skip XAP 782 because quest 29 is clear. The separate current-package `war-corrected-rhum-ap60-quest-set` recipe starts from explicitly documented direct quest preparation, then repeats all nine ordinary caller inputs and the taken GOSUB/return identically. Its nested post-teleport save now restores and finishes byte-identically to uninterrupted completion; the public exploration proof retains rejection of a forged issuing cell. Ally 248 is absent. Neither setup nor Rebuilt execution proves ordinary quest acquisition, native Castle equivalence or original author intent.
- The same correction record also retargets Land 10 AP 88 slot 4 from settings 6096 to 6095. APs 84 through 87 have otherwise identical eight-action dart-trap timelines and use 6095's valid spell 1108 at power 5 and save adjustment -75. AP 88 instead requests spell 1; `loadspell.c::loadspell2` returns below 1101 without replacing global `spellinfo`. Reusing the matching traps' settings avoids a history-dependent stale spell while leaving both original EDCD rows intact. The current-package `war-corrected-dart-trap-ap88` ordinary-entry recipe repeats the six Magic Darts effects identically and restores the post-damage acknowledgement without repeated damage or RNG. Ally-present and protective-condition outcomes remain outside this proof.
- The current-package `war-corrected-simple52-refusal` recipe repeats direct XAP 1221 invocation followed by actual choice/payer/warning controls and typed acknowledgements. The 500-gem refusal enters XAP 1224, preserves the party and RNG, grants no map, replaces AP 6 and shifts X +1; the abandoned result does not reopen. This proves the corrected result but not AP 6's hour/quest eligibility. Preview snapshots remain unavailable until the directly invoked interaction finishes.
