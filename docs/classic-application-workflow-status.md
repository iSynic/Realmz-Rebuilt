...
The contextual/owner-bound batch repairs AOGM's zero-slot contextual RNG defect and verifies entry, wrong-set repetition and exact reward/region-disable/default reentry. Wrath AP 44 now has equal detection/disarm, armed-trap failure, unarmed lock failure and eight-item lock-reward repeats through actual controls and ordinary reentry. Detected-trap and trap-warning restores reproduce exact final game/RNG state; pending Treasure reproduces all twelve suffix states. The lock grants 1800 XP only to its actor; Done alone sets chance 0 and prevents duplicate rewards. All thirteen bundled scenarios have recorded representatives without certification upgrades. Focused Thief/encounter/treasure checks pass 91 assertions, and the clean-reference Tier 2 workflow gate passes; the final aggregate gate remains pending. No tool implementation was needed for these Wrath cases. Next coverage favors the retained Wrath pending-tumbler save boundary, then War pooled-payment edges using existing baselines. Native CPU/dungeon performance, complete combat replay and full campaign certification remain separate; reported failures take priority.
