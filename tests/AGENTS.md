...
- `tests/fixtures/packages/fixture-provenance.json` owns the hashes and Providence commit for both the positive `.realmz2` fixture and its valid-ZIP stale-hash negative derivative. The current fixture contains two land maps, one packed-source dungeon map, explicit topology features and Layout transitions, direct Complex/Thief/Timed Encounters, direct gameplay definitions, 420 normalized standard spells plus one packed scenario spell, and an AP to Simple Encounter to result to Scenario Action to XAP/CODE 111 route.
