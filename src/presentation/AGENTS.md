...
- The battle action component renders the session-owned melee/missile mode. It emits the no-cost typed switch response, never offers melee target buttons in missile mode, and keeps Fire visible but disabled with the request's exact tactical-state reason until positions/range/LOS are available.
