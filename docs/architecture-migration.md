...
- The Character route now exposes its party-order summary, reorder editor, persistent shared sheet, empty state, identity, complete tab rail, and content frame through authored scenes. `party_order_row.tscn` supplies variable order records, and Character Files inspection, setup inspection, and creation review instantiate the same `classic_character_sheet.tscn`. Tab-specific and Character Files records remain the next migration slices; this intermediate boundary lowers runtime-created Controls to 396 without claiming the feature is complete.
