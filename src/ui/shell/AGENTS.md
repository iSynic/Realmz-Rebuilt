...
- The exploration Area Search command keeps its source-backed eye crop and renders the live caption as separate `Area` and `Search` lines so the native icon stage and readable command identity remain distinct.
...
- Save & Load exposes Update Save only for a detached preview marked eligible for the pinned Half Truth media transition. Its explicit action requests a new verified copy while ordinary mismatched saves keep Load disabled; the source record remains visible and untouched.
- Display also exposes Responsive, whole-window integer, world-canvas integer, and Fill modes with an independent smoothing scope. World zoom is editable only in world-canvas mode; inactive values stay saved. Whole-window modes use baseline logical interface density; the compact 800x600 footer presents all twelve commands and effect slots within the window.
- Display also exposes an independent default-off CRT toggle, CRT-Pi and CRT-Lottes presets, and World canvas or Whole window scope. The preset and scope remain editable and saved while the effect is off.
...
- `party_roster_member_row.tscn` owns the Classic record layout: full name and armor, race/caste, HP/SP, then attacks per round and movement, segmented by horizontal and vertical field rules. Roster text uses the custom Theldrow Rebuilt face in Classic mode and Inter in Readable mode, with consistent numeric sizing. Narrow widths wrap or stack and scroll instead of shrinking or clipping facts. Six ordinary records fit the canonical rail. Native 44-by-44 portraits remain centered in the unscaled 50-by-50 effect stage; selection uses a thin outer outline. The character button retains selection, activation, accessible facts, and the complete hover/focus condition summary; its editable tooltip scene owns the hover surface. Combat Auto remains an independent toggle.
- Roster stat cells keep labels left-aligned and values right-aligned, with restrained health, spell, attack, movement, and armor accents that remain readable in both typography modes.
