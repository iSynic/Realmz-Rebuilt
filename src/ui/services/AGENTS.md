...
- `treasure_distribution_interaction.tscn` owns ordinary loot distribution, wealth actions, lore controls, recipient selection, item facts, recovery, and the nested completion confirmation. `ClassicTreasureTakeEffect` is presentation-only pickup animation; `TreasureDisplayText` formats detached public summaries.
