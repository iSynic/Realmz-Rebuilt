...
- Generated structural chrome must contain no text, symbols, scenario content, or executable metadata. App-owned transient status glyphs are a separate generated UI category: they may depict one semantic symbol, contain no embedded words, record their SpriteCook asset/job provenance and exact hashes in `ui/spritecook-assets.json`, and render only at native integer scale.
...
- Selected SpriteCook status glyphs remain their direct production outputs. The save disk and journal book/quill retain their recorded 32×32 and 20×20 pixels respectively and may not be stretched to a shared display size.
