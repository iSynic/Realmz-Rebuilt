...
- Field spell, scroll, and charged-item targeting rebuild the pending character-selection request from the typed targeting continuation during restore. Detached projection and restore legality consult `ClassicSpellCapabilityCatalog` rather than duplicating special-ID allowlists. The reconstructed request must retain the exact selected power and source-authored spell display facts while response handling continues to validate only stable target identities through the owning workflow. The field-invalid scroll Discard/Keep interaction is a distinct typed continuation that restores only when the exact character, equipped case, slot, spell, and power still match; acceptance clears one slot and decline commits no gameplay mutation.
