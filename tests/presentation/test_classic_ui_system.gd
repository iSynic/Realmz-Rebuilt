...
func _test_scene_composition() -> void:
	...
	var spatial_view := GameView.new(1, true, null); var locked_dungeon := MapView.new("dungeon:test", "Locked Dungeon", &"dungeon", 1, 1, Vector2i.ZERO, [], false, [], {}, Vector2i.ZERO, -1, 1, false, false, -1, true, true); spatial_view.party_map_id = locked_dungeon.map_id; spatial_view.map_view = locked_dungeon; assert_true(PresentationCoordinator.should_show_exploration_stage(&"spells", spatial_view, true) and not PresentationCoordinator.dungeon_view_toggle_available(locked_dungeon) and GameShell.location_fact_text(spatial_view) == "dungeon:test • ?,? • Compass N", "field Spells preserves the live exploration renderer, a locked dungeon requires Wizard's Eye before Space exposes the overhead view, and authored coordinate/compass flags alter only their footer facts"); locked_dungeon.wizard_eye_active = true; locked_dungeon.coordinates_hidden = false; locked_dungeon.compass_enabled = false; assert_true(PresentationCoordinator.dungeon_view_toggle_available(locked_dungeon) and GameShell.location_fact_text(spatial_view) == "dungeon:test • 0,0", "Wizard's Eye restores Castle's overhead-view toggle while a disabled compass leaves the exact coordinates visible without a direction fact"); shell.free()
func _test_automatic_workflow_routes() -> void:
	...
	assert_equal([GameShell.automatic_workflow_route(&"exploration", view), GameShell.automatic_workflow_route(&"inventory", view)], [&"combat", &"combat"], "battle setup replaces exploration or browsing with the tactical workspace")
	...
	assert_equal(GameShell.automatic_workflow_route(&"combat", view), &"exploration", "completed battle cleanup returns the ordinary shell to exploration")
	assert_equal(GameShell.automatic_workflow_route(&"inventory", view), &"inventory", "ordinary non-combat workspaces remain presentation-owned")
	view.pending_interaction = _fixture_request("shop.route", InteractionRequest.SHOP)
	assert_equal(GameShell.automatic_workflow_route(&"exploration", view), &"services", "application services open their dedicated workspace")
	assert_equal(GameShell.automatic_workflow_route(&"inventory", view), &"services", "a service interaction replaces an unrelated browsing workspace")
	view.pending_interaction = null
	assert_equal(GameShell.automatic_workflow_route(&"services", view, true), &"exploration", "completing a contextual service returns to exploration instead of leaving its interaction workspace open")
	assert_equal(GameShell.automatic_workflow_route(&"services", view, false), &"services", "ordinary Money browsing remains open until the player chooses Done")
	view.pending_interaction = InteractionRequest.yes_no("action-point.route", "Will you approach?", "Yes", "No")
	assert_equal(GameShell.automatic_workflow_route(&"inventory", view), &"exploration", "an Action Point interaction returns an unrelated browsing workspace to exploration")
	view.pending_interaction = null
	assert_equal(GameShell.automatic_workflow_route(&"inventory", view), &"inventory", "free browsing remains presentation-owned after the interaction closes")
