...
func _test_session_money_workflow(content: RealmzContent) -> void:
	...
	var invalid := session.submit_intent(EconomyIntents.money(&"to-pool", first.id, "gold", 1)); assert_equal(invalid.error_code, &"invalid_money_increment", "forged non-Classic gold increments fail explicitly")
	assert_equal(session._context.state.party.character_by_id(first.id).money.gold, 10, "rejected money action mutates no wealth")
	var pooled := session.submit_intent(EconomyIntents.money(&"pool")); assert_equal(pooled.state, SessionStep.State.COMPLETED, "typed Pool commits synchronously")
	...
	var duplicate_pool := session.submit_intent(EconomyIntents.money(&"pool")); assert_equal(duplicate_pool.error_code, &"money_action_unavailable", "a no-op Pool intent fails transactionally")

	var restored := GameSession.new(); assert_equal(restored.restore(content, save_round_trip(session.snapshot())).state, SessionStep.State.COMPLETED, "pooled wealth restores through the central save aggregate")
	assert_equal(restored.view().money_workspace.pooled_jewelry, 1, "restored detached money facts retain non-gold denominations")
	var to_character := restored.submit_intent(EconomyIntents.money(&"to-character", first.id, "gold", 5)); assert_equal(to_character.state, SessionStep.State.COMPLETED, "Swap moves one Classic gold increment from pool to character")
	...
	var to_pool := restored.submit_intent(EconomyIntents.money(&"to-pool", first.id, "gold", 5)); assert_equal(to_pool.state, SessionStep.State.COMPLETED, "Swap returns one Classic gold increment to the pool")
	...
	var carried_first := restored._context.state.party.character_by_id(first.id)
	...
	var shared := restored.submit_intent(EconomyIntents.money(&"share")); assert_equal(shared.state, SessionStep.State.COMPLETED, "typed Share commits synchronously")
	...
	var carried_second := restored._context.state.party.character_by_id(second.id)
	...
	var duplicate_share := restored.submit_intent(EconomyIntents.money(&"share")); assert_equal(duplicate_share.error_code, &"money_action_unavailable", "a no-op Share intent fails transactionally")
	...
	assert_equal(final_restore._context.state.party.to_data(), restored._context.state.party.to_data(), "save restoration preserves complete pooled, personal, load, and movement state")
	assert_false(final_restore.view().money_workspace.changing_available, "money changing is unavailable away from a shop or temple"); final_restore._context.state.party.pooled_wealth = WealthState.new(115, 1, 1)
	assert_equal(final_restore.submit_intent(EconomyIntents.money(MoneyChangingRules.JEWELRY_TO_GEMS, "", "jewelry", 1)).error_code, &"money_action_unavailable", "forged changing away from a service cannot commit")
	final_restore._context.state.location_services.temple_available = true; final_restore._view_projector.clear()
	assert_true(final_restore.view().money_workspace.changing_available, "temple availability exposes Castle changing rates")
	var jewelry_change := final_restore.submit_intent(EconomyIntents.money(MoneyChangingRules.JEWELRY_TO_GEMS, "", "jewelry", 1)); assert_equal([jewelry_change.state, final_restore._context.state.party.pooled_wealth.to_data()], [SessionStep.State.COMPLETED, {"gold": 115, "gems": 6, "jewelry": 0}], "one jewelry becomes five pooled gems")
	assert_true(jewelry_change.events.any(func(event: DomainEvent) -> bool: return event.kind == &"sound_requested" and event.payload.get("soundId") == 10129), "changing requests Castle sound 10129")
	assert_equal(final_restore.submit_intent(EconomyIntents.money(MoneyChangingRules.GOLD_TO_GEMS, "", "gold", 100)).error_code, &"invalid_money_increment", "forged conversion rate is rejected")
	assert_equal(final_restore.submit_intent(EconomyIntents.money(MoneyChangingRules.GOLD_TO_GEMS, "", "gold", 115)).state, SessionStep.State.COMPLETED, "115 pooled gold buys one gem")
	assert_equal(final_restore.submit_intent(EconomyIntents.money(MoneyChangingRules.GEMS_TO_GOLD, "", "gems", 1)).state, SessionStep.State.COMPLETED, "one pooled gem becomes 100 gold")
	assert_equal(final_restore._context.state.party.pooled_wealth.to_data(), {"gold": 100, "gems": 6, "jewelry": 0}, "sequential conversions preserve exact pooled denominations")
...
func _test_pooled_wealth_departure(content: RealmzContent) -> void:
	...
	assert_equal(blocked_restored._context.state.party.pooled_wealth.gold, 0, "declining distribution clears the abandoned pool")
	...
	var character_id: String = distribution_restored._context.state.party.characters()[0].id
	...
	assert_equal(distribution_restored._context.state.party.pooled_wealth.gold, 10, "the rejected transfer preserves exact pooled wealth")
	...
	assert_equal([distribution_restored._context.state.party.pooled_wealth.gold, distribution_restored._context.state.party.character_by_id(character_id).money.gold], [5, 5], "the departure Swap transfers one exact Classic increment")
	...
	assert_equal(distribution_restored._context.state.party.pooled_wealth.to_data(), {"gold": 0, "gems": 0, "jewelry": 0}, "departure Share processes jewelry, gems, and gold from the same pooled workspace")
	...
	assert_equal(mutation_restored._context.state.party.pooled_wealth.gold, 0, "Done leaves no pooled remainder after Share")
	...
	assert_equal(continue_session._context.state.party.pooled_wealth.gold, 0, "continued movement leaves no pooled wealth behind")
	...
	assert_false(camp_session._context.state.party_camping, "declining distribution resumes the source camp-departure path")
