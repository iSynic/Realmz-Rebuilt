...
- App-owned textures, verified original bitmap controls and mounted/camp map markers, the complete integrated Classic sound bank, shared item CIcon library, proven built-in combat CIcon families, and bundled fonts used by the Classic-wide shell.
...
- The front-door Realmz intro is app-owned licensed art, not scenario media. Its source GIF remains external; the deterministic 12-fps frame set, exact source hash, preserved timing, license, dimensions, and per-frame hashes live in `ui/intro/intro-animation.json` and are verified offline. Keep this portable frame player instead of adding a native GIF or MP4 codec dependency. Its app-owned SpriteCook ornament remains separate generated chrome; `ui/spritecook-assets.json` records the style asset, generation job, source hash, transparent-center derivation, dimensions, and production hash.
