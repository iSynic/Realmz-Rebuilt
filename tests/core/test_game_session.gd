...
func run() -> void:
	...
	var encounter_session := GameSession.new(); encounter_session.start(debug_content, 19); encounter_session.submit_intent(PartyIntents.create([CharacterCreationSpec.new("Encounter Debugger", debug_race.id, debug_caste, 1)])); var encounter_step := encounter_session.apply_debug_command(SessionDebugCommand.start_encounter(&"simple", 0)); assert_equal([encounter_step.state, encounter_step.interaction.kind, encounter_session.snapshot()], [SessionStep.State.WAITING_FOR_INTERACTION, InteractionRequest.ENCOUNTER_CHOICE, null], "a debug encounter uses the ordinary typed interaction but cannot enter a save"); var battle_session := GameSession.new(); battle_session.start(debug_content, 23); battle_session.submit_intent(PartyIntents.create([CharacterCreationSpec.new("Battle Debugger", debug_race.id, debug_caste, 1), CharacterCreationSpec.new("Swap Ally", debug_race.id, debug_caste, 1)])); assert_equal(battle_session.apply_debug_command(SessionDebugCommand.start_battle(0)).state, SessionStep.State.COMPLETED, "a debug battle uses ordinary deterministic battle construction"); var tactical := battle_session.view(); var swap_options := tactical.combat_view.movement_options.filter(func(option: CombatMoveOptionView) -> bool: return option.enabled and option.movement_cost == 5); assert_false(swap_options.is_empty(), "the detached public combat view enables an adjacent allied footprint as Castle's five-movement swap"); var swap_step := battle_session.submit_intent(CombatIntents.move(tactical.combat_view.active_actor_id, swap_options[0].destination)); assert_equal([swap_step.state, swap_step.interaction.kind, swap_step.interaction.body.options.map(func(option: InteractionRequestValue.ChoiceOption) -> String: return option.label), swap_step.interaction.body.can_back_out], [SessionStep.State.WAITING_FOR_INTERACTION, InteractionRequest.INDEXED_CHOICE, ["Swap Positions", "Attack Friend"], true], "manual movement opens Castle's typed cancelable friendly-collision choice"); var swap_boundary := battle_session.snapshot(); assert_not_null(swap_boundary, "the friendly-collision choice is a saveable committed boundary"); var cancelled_swap := GameSession.new(); assert_equal(cancelled_swap.restore(debug_content, swap_boundary).state, SessionStep.State.COMPLETED, "the friendly-collision choice restores without rerunning movement"); var cancelled_step := cancelled_swap.respond(InteractionResponse.new(cancelled_swap.view().pending_interaction.request_id, InteractionRequest.INDEXED_CHOICE, InteractionResponse.ChoiceBody.new(-1, true))); assert_true(cancelled_step.state == SessionStep.State.COMPLETED and cancelled_step.events.any(func(event: DomainEvent) -> bool: return event.kind == &"combat_friendly_collision_cancelled") and cancelled_swap._context.state.combat.pending_reaction == null, "Back out cancels only the post-Guard collision attempt and leaves the activation available"); var resumed_swap := GameSession.new(); assert_equal(resumed_swap.restore(debug_content, swap_boundary).state, SessionStep.State.COMPLETED, "the saved friendly-collision choice may still resume independently"); assert_true(resumed_swap.respond(InteractionResponse.new(resumed_swap.view().pending_interaction.request_id, InteractionRequest.INDEXED_CHOICE, InteractionResponse.ChoiceBody.new(0))).events.any(func(event: DomainEvent) -> bool: return event.kind == &"combatants_swapped"), "selecting Swap Positions resumes the restored combat transaction exactly once"); battle_session = resumed_swap; var victory := battle_session.apply_debug_command(SessionDebugCommand.win_battle()); assert_true(victory.state != SessionStep.State.FAILED and victory.events.any(func(event: DomainEvent) -> bool: return event.kind == &"battle_completed" and event.payload.get("outcome") == "victory"), "debug victory enters the ordinary terminal battle and reward flow"); var bridge_controller := GameSessionController.new(); bridge_controller.start(debug_content, 31); bridge_controller.submit_intent(PartyIntents.create([CharacterCreationSpec.new("Bridge Debugger", debug_race.id, debug_caste, 1)])); assert_equal(bridge_controller.apply_debug_command(SessionDebugCommand.start_battle(0)).state, SessionStep.State.COMPLETED, "the fixture bridge prepares an active battle through the ordinary debug boundary"); var bridge_observer := RuntimeTestingObserver.new(bridge_controller, func() -> RealmzContent: return debug_content, func() -> Dictionary: return {}); var bridge_commands := RuntimeTestingCommands.new(null, bridge_controller, bridge_observer, func() -> RealmzContent: return debug_content); var wrong_battle := bridge_commands.invoke({"target": {"kind": "battle-victory", "id": 1}}); assert_equal(wrong_battle["error"]["code"], "battle_victory_wrong_battle", "fixture victory rejects a mismatched expected Classic Battle ID without mutating combat"); var forced_victory := bridge_commands.invoke({"target": {"kind": "battle-victory", "id": 0}}); assert_true(forced_victory["error"] == null and forced_victory["result"]["mode"] == "fixture-forced-victory" and forced_victory["result"]["events"].any(func(event: Dictionary) -> bool: return event.get("kind") == "battle_completed" and event.get("payload", {}).get("outcome") == "victory"), "fixture victory uses the ordinary reward and continuation path"); var duplicate_victory := bridge_commands.invoke({"target": {"kind": "battle-victory", "id": 0}}); assert_equal(duplicate_victory["error"]["code"], "battle_victory_unavailable", "repeating forced victory cannot duplicate rewards after the combat boundary closes"); bridge_controller.free()
	var deferred_baseline := debug_session.snapshot(); debug_content.requires_deferred_references = true; debug_content.compatibility_warnings.append(ScenarioCompatibilityWarning.new(&"scenario-program", ScenarioVm.DEBUG_PROGRAM_ID, "id", 0, &"TEXT", 2_000_000, "The preserved Classic instruction references an unavailable optional resource.")); var deferred_fault := debug_session.apply_debug_command(SessionDebugCommand.start_scrolling_text(2_000_000)); var fault_view := debug_session.view(); assert_equal([deferred_fault.state, deferred_fault.error_code, fault_view.runtime_fault.program_id, fault_view.runtime_fault.slot, fault_view.runtime_fault.opcode, fault_view.runtime_fault.operands, fault_view.runtime_fault.target_kind, fault_view.runtime_fault.target_id], [SessionStep.State.FAILED, &"scenario_content_fault", ScenarioVm.DEBUG_PROGRAM_ID, 0, 62, [2_000_000], &"TEXT", "2000000"], "selecting an unavailable deferred target halts with exact campaign, program, slot, opcode, operand, and target context"); assert_true(deferred_fault.events.any(func(event: DomainEvent) -> bool: return event.kind == &"scenario_runtime_faulted") and debug_session.snapshot() == null, "a deferred-reference fault is retained for diagnostics and cannot be saved"); assert_equal(debug_session.apply_debug_command(SessionDebugCommand.restore_party()).error_code, &"scenario_session_faulted", "a faulted scenario session cannot continue gameplay or repeat effects"); var restored_after_fault := GameSession.new(); assert_equal(restored_after_fault.restore(debug_content, deferred_baseline).state, SessionStep.State.COMPLETED, "loading the previously committed adventure clears the nonserialized fault without changing the save"); assert_equal(debug_session.close().events[-1].payload.get("reason"), "scenario-content-fault", "a faulted session can return directly to the main menu without running scenario hooks"); debug_content.compatibility_warnings.pop_back(); debug_content.requires_deferred_references = false
