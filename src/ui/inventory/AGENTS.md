...
- A Shop-owned browse presentation reuses the same complete workspace with a visible Back to shop action. It keeps inspection, character selection, and exact-instance Trade active but disables other item mutations with the exact Shop-ownership reason. When mounted as the root workspace, presentation reuses its scene-owned children rather than nesting or clearing another workspace.
