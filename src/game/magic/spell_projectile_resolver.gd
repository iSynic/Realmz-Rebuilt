...
func resolve_character_projectile(caster: CharacterState, caste: CasteDefinition, projectile_item: ItemDefinition, target: MonsterState, spell: SpellDefinition, power_level: int, rng: RealmzRng) -> ProjectileResolution:
	if caster == null or projectile_item == null or target == null or spell == null or rng == null or power_level < 0 or absi(spell.spell_class) != 9 or absi(spell.damage_type) != 9 or absi(spell.special) not in [0, 7, 28, 49]:
		return null
	...
	damage = _special_projectile_damage(spell, target.current_health, damage, duration)
	var hit_count := 0
	var miss_count := 0
	var total_damage := 0
	...
	for hit_index: int in maximum_hits:
		...
		if absi(spell.special) == 28:
			_apply_combat_condition(target.conditions, spell, duration, true)
...
func resolve_monster_projectile(caster: MonsterState, projectile_item: ItemDefinition, target: CharacterState, spell: SpellDefinition, power_level: int, rng: RealmzRng) -> ProjectileResolution:
	...
	target.current_health -= damage
	_apply_projectile_special(spell, target, duration)
	return ProjectileResolution.new(true, 1, 0, damage, damage, duration, target.current_health <= 0)


static func _special_projectile_damage(spell: SpellDefinition, target_health: int, damage: int, duration: int) -> int:
	if absi(spell.special) == 49:
		return target_health + 10
	if absi(spell.special) == 28:
		return duration
	return damage


static func _apply_projectile_special(spell: SpellDefinition, target: CharacterState, duration: int) -> void:
	if absi(spell.special) == 7:
		target.movement /= 2
	elif absi(spell.special) == 28:
		_apply_combat_condition(target.conditions, spell, duration, false)
