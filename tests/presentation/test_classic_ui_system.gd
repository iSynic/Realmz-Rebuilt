...
func _test_scene_composition() -> void:
	...
	assert_true([&"search_mode", &"contextual", &"camp", &"heal"].all(func(id: StringName) -> bool: return exploration_commands.any(func(definition: Dictionary) -> bool: return definition["id"] == id and definition["group"] == &"world")) and ClassicCommandCatalog.command(&"camp")["asset_id"] == &"command.camp" and ClassicCommandCatalog.command(&"contextual")["symbol"] == &"yin_yang" and ClassicCommandCatalog.command(&"heal")["hold_repeat"], "Search, source-shaped Encounter, held Heal, and the persistent Camp bitmap belong to world controls")
