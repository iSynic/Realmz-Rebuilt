...
- Named `InputMap` actions are the host input boundary. Land movement binds the complete keypad compass, including 7/9/1/3 diagonals; the host suppresses those diagonals on dungeon maps before creating a typed intent. Camp, Rest, and Heal are separate typed commands; held Rest, Heal, and Area Search cadence is presentation-owned and emits one intent per pulse. The application catches physical mouse release so rerendering the pressed footer button cannot strand a held command. During a manual character activation, physical Alt press/release owns the transient Fast Spell dock and Alt plus top-row `1` through `0` invokes the matching binding through the ordinary combat response; focus loss always releases that presentation state. Back first asks the active typed interaction to cancel its local subtask or emit a declared cancel/back response, then resolves passive modals, contextual drawers, and route history before returning to exploration; pending interactions suppress route and exploration input.
