...
static func is_combat_healing_spell(spell: SpellDefinition) -> bool:
	if spell == null or absi(spell.special) != 57 or not spell.in_combat or spell.queue_icon != 0 or spell.target_type not in [0, 1, 5, 9, 10, 12] or absi(spell.damage_type) != 8:
		return false
