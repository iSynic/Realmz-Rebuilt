...
- `import-rebuilt-intro.ps1` accepts only the approved OGV hash and byte length, copies it byte-for-byte, and records its fixed 832x480/24-fps/5.167-second Theora/Vorbis media contract. The external source path never enters committed metadata.
...
- `verify-classic-application-media.ps1` verifies unique IDs/resource keys, the expected integrated sound, shared/stock-supply item-CIcon, and combat-CIcon counts and required stock-supply identities, generated-chrome plus SpriteCook status/command-glyph provenance, paths, dimensions, and every committed byte count and SHA-256 without requiring an external Castle checkout.
