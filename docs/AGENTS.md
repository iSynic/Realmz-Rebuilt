...
- `classic-compatibility-gap-register.md` tracks confirmed sub-workflow blockers, coverage gaps, reported ordinary-play defects, exposure counts, closure evidence, and the machine-readable spell signature denominator (`tests/fixtures/oracle/classic-spell-signature-denominator.json`) that the coarser generated workflow and opcode/spell denominators do not yet express.
- `compatibility-batch-report-2026-09-15.md` records the verified Guard/collision/monster-macro repair batch, its exact aggregate evidence, unchanged format contracts, and remaining executable cases without promoting campaign status.
...
- `controller-acceptance.md` owns the implemented controller contract, automated evidence, performance results, and the explicit physical-device and native-OS acceptance matrix.
