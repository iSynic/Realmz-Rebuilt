...
- `PartySetupCharacterCreationController` owns the five-stage local draft surface. Identity, Race/Class, Appearance, Review, and Spells may retain only presentation-local selections before emitting typed intents. Review consumes the detached provisional `CharacterView`; it may resolve that view's exact appearance and item media, but cannot calculate rules, invent restrictions, or substitute resource identities. The compact creator keeps actions outside the scrolling record body.
