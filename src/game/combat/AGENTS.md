...
The enclosing `CombatState` remains under `src/game/state`: its roster/turn/status/dropped-item/spell-runtime owners hold cohesive mutable facts, and `CombatStateCodec` preserves the stable flat save shape. `RealmzContent.combat` owns immutable definition lookup. Detached views remain under `src/game/view`, and the established rule collaborators remain under `src/game/rules` until a complete feature migration moves each file, test, UID, and reference atomically.
