...
- Controls are configurable as a draft with conflict handling, live input feedback, required-navigation validation, and persistent Apply and Restore Defaults actions. Presentation settings schema 16 adds the default-off Hurry Spell Resolution preference while preserving older values and custom bindings; schema 14 receives the Top Menu default only when View/Create/Minus is unused.
...
| Aggregate repository verification | Passed, 4,377 assertions across 29 suites plus architecture, package alignment, media, icon, export-contract, provenance, workflow-inventory, and gameplay-parity checks |
