...
The next batch broadens contextual and owner-bound encounter callers. AOGM region 18/XAP 25/Complex 5 exposed a shared contextual RNG defect: zero door slots were skipped although Castle draws all three. The narrow correction passes fail-before/pass-after public exploration proof, equal repeated ordinary movement/actual Encounter entry, wrong Action-set fallback/repetition, and exact Action-set reward/region-disable/default-encounter reentry. Saved prompts resume through their original contextual caller. Immediate high-risk aggregate verification passes all 4212 assertions and package, architecture, export and inventory gates in the detached verification copy, excluding only the live editor's unrelated project autoload additions. These recipes add AOGM to the collection without changing certification counts. Wrath dungeon 1 AP 44/Complex 28 and its replacement XAP 927/Complex 29 remain selected but unexecuted: verify owner-bound trap/lock and spell outcomes next, then recompute selection at batch closeout. Native CPU/dungeon performance, complete combat replay and full campaign certification remain separate; reported failures still take priority.
