...
func _test_public_classic_combat_spawn(content: RealmzContent) -> void:
	...
	var darts_spell := SpellDefinition.new("classic.spell.3208", 3208, "Magic Darts"); darts_spell.in_combat = true; darts_spell.target_type = 0; darts_spell.range_min = 10; darts_spell.fixed_target_count = 1; darts_spell.spell_class = 1; darts_spell.damage_type = 1; darts_spell.damage_min = 6; darts_spell.damage_max = 6
	var multi_darts := SpellDefinition.new("classic.spell.1108", 1108, "Magic Darts Multi"); multi_darts.in_combat = true; multi_darts.target_type = 0; multi_darts.range_min = 10; multi_darts.spell_class = 1; multi_darts.damage_type = 1; multi_darts.damage_min = 4; multi_darts.damage_max = 4
	var dispel_spell := SpellDefinition.new("classic.spell.1304", 1304, "Destroy Magic"); dispel_spell.in_combat = true; dispel_spell.target_type = 0; dispel_spell.special = 61; dispel_spell.cannot = 4; dispel_spell.range_min = 10; dispel_spell.spell_class = 8; dispel_spell.damage_type = 8; dispel_spell.cost = 10
	var single_spell := SpellDefinition.new("classic.spell.2505", 2505, "Heal Wounds"); single_spell.in_combat = true; single_spell.target_type = 1; single_spell.cannot = 4
	var ray_spell := SpellDefinition.new("classic.spell.3202", 3202, "Acid Splash"); ray_spell.in_combat = true; ray_spell.target_type = 6
	spawn_content.magic = SpellCatalog.new([macro_spell, damage_spell, field_spell, darts_spell, multi_darts, dispel_spell, single_spell, ray_spell]); source.current_health = 0
	...
	var hero2 := CharacterState.new("opcode124.hero2", "Hero Two", 20, 20); state.party.add_character(hero2); field.actors.place_character(hero2.id, Vector2i(46, 45))
	var ally := MonsterState.new("opcode124.ally", summoned_definition.id, "Ally", 20, 20, 1, 1, 0, 0, 0, true); state.combat.roster.add_monster(ally); assert_true(field.actors.place_monster(ally.id, Vector2i(48, 48), 0), "ally placed at unoccupied battlefield cell"); ally.conditions.set_value(ConditionRules.MAGIC_AURA, 5)
	var darts_program := ScenarioProgramDefinition.new("xap:111", &"extra-action-point", "111", [ClassicActionDefinition.new(0, 17, 17, 391, false, [3208, 1, 0, 1]), ClassicActionDefinition.new(1, 84, 84, 0, false, [])]); var darts_vm := ScenarioVm.new(); darts_vm.configure(ScenarioDefinition.new([darts_program], [])); darts_vm.start_program(darts_program.id, ScenarioExecutionContext.calling(&"monster-death-macro").set_battle(battle.id).set_combatant(source.id)); var darts_result := darts_vm.run(api); var darts_events := darts_result.events.filter(func(event: DomainEvent) -> bool: return event.kind == &"combat_spell_resolved")
	assert_equal([darts_result.state, hero.current_health, hero2.current_health, darts_events.size(), state.combat.turns.active_actor_id(), hero.spell_points], [ScenarioVmResult.State.COMPLETED, 14, 20, 1, hero.id, 0], "a power-1 repeated-target death macro hits the single living enemy target within range without affecting others or spending resources")
	var multi_program := ScenarioProgramDefinition.new("xap:112", &"extra-action-point", "112", [ClassicActionDefinition.new(0, 17, 17, 392, false, [1108, 5, -50, 0]), ClassicActionDefinition.new(1, 84, 84, 0, false, [])]); var multi_vm := ScenarioVm.new(); multi_vm.configure(ScenarioDefinition.new([multi_program], [])); multi_vm.start_program(multi_program.id, ScenarioExecutionContext.calling(&"monster-death-macro").set_battle(battle.id).set_combatant(source.id)); var multi_result := multi_vm.run(api); var multi_events := multi_result.events.filter(func(event: DomainEvent) -> bool: return event.kind == &"combat_spell_resolved")
	assert_equal([multi_result.state, hero.current_health, hero2.current_health, multi_events.size()], [ScenarioVmResult.State.COMPLETED, 10, 16, 2], "a multi-target repeated death macro with negative save adjustment caps at available living enemies and resolves each in order")
	var dispel_program := ScenarioProgramDefinition.new("xap:113", &"extra-action-point", "113", [ClassicActionDefinition.new(0, 17, 17, 393, false, [1304, 5, 0, 0]), ClassicActionDefinition.new(1, 84, 84, 0, false, [])]); var dispel_vm := ScenarioVm.new(); dispel_vm.configure(ScenarioDefinition.new([dispel_program], [])); dispel_vm.start_program(dispel_program.id, ScenarioExecutionContext.calling(&"monster-death-macro").set_battle(battle.id).set_combatant(source.id)); var dispel_result := dispel_vm.run(api)
	assert_equal([dispel_result.state, ally.conditions.value(ConditionRules.MAGIC_AURA), hero.conditions.value(ConditionRules.MAGIC_AURA)], [ScenarioVmResult.State.COMPLETED, 0, 0], "cannot-4 friendly repeated macro selects living allies and clears positive conditions on the friendly target")
	var single_program := ScenarioProgramDefinition.new("xap:114", &"extra-action-point", "114", [ClassicActionDefinition.new(0, 17, 17, 394, false, [2505, 7, 0, 1]), ClassicActionDefinition.new(1, 84, 84, 0, false, [])]); var single_vm := ScenarioVm.new(); single_vm.configure(ScenarioDefinition.new([single_program], [])); single_vm.start_program(single_program.id, ScenarioExecutionContext.calling(&"monster-death-macro").set_battle(battle.id).set_combatant(source.id)); var single_result := single_vm.run(api)
	var ray_program := ScenarioProgramDefinition.new("xap:115", &"extra-action-point", "115", [ClassicActionDefinition.new(0, 17, 17, 395, false, [3202, 2, 0, 0]), ClassicActionDefinition.new(1, 84, 84, 0, false, [])]); var ray_vm := ScenarioVm.new(); ray_vm.configure(ScenarioDefinition.new([ray_program], [])); ray_vm.start_program(ray_program.id, ScenarioExecutionContext.calling(&"monster-death-macro").set_battle(battle.id).set_combatant(source.id)); var ray_result := ray_vm.run(api)
	assert_equal([single_result.state, single_result.error_code, ray_result.state, ray_result.error_code], [ScenarioVmResult.State.FAILED, &"unsupported_macro_single_target", ScenarioVmResult.State.FAILED, &"unsupported_macro_ray"], "single-target and ray monster macros fail with typed reasons due to Classic memory corruption and ray bypass")
