...
func _test_inventory_workspace() -> void:
	...
	var cancel_operation := _buttons_in(body.find_child("InventoryOperationActions", true, false)).filter(func(button: Button) -> bool: return button.text == "Cancel")[0] as Button; cancel_operation.pressed.emit(); controller.present(body, view, media, 1.0); buttons = _base_buttons_in(body); var trade := buttons.filter(func(button: BaseButton) -> bool: return button.tooltip_text == "Open the two-pack Trade workspace")[0] as ClassicBitmapButton; trade.command_requested.emit(&"inventory.action.trade"); controller.present(body, view, media, 1.0); var destination_ledger := body.find_child("InventoryTradeLedger_destination", true, false); assert_true(body.find_child("InventoryTradeLedgers", true, false) != null and body.find_child("InventoryTradeDivider", true, false) != null and destination_ledger.call("_can_drop_data", Vector2.ZERO, {"kind": &"inventory-trade-item", "sourceId": source.id, "instanceId": item_view.instance_id}), "Trade presents two white item ledgers with a divider and accepts an exact eligible cross-pack drag"); destination_ledger.call("_drop_data", Vector2.ZERO, {"kind": &"inventory-trade-item", "sourceId": source.id, "instanceId": item_view.instance_id}); controller.select_roster_character(destination.id, view); controller.present(body, view, media, 1.0); trade = _base_buttons_in(body).filter(func(button: BaseButton) -> bool: return button.tooltip_text == "Open the two-pack Trade workspace")[0] as ClassicBitmapButton; trade.command_requested.emit(&"inventory.action.trade"); controller.present(body, view, media, 1.0); (_buttons_in(body.find_child("InventoryTradeActions", true, false)).filter(func(button: Button) -> bool: return button.text == "Transfer to Alis")[0] as Button).pressed.emit()
	assert_equal([intents.size(), intents[0].kind, intents[0].payload.actor_id, intents[0].payload.destination_character_id, intents[1].payload.actor_id, intents[1].payload.destination_character_id], [2, PlayerIntent.Kind.TRADE_ITEM, source.id, destination.id, destination.id, source.id], "drag and click alternatives emit exact bidirectional typed transfers")
	body.free()
