...
func _test_character_creator_workflow() -> void:
	...
	assert_equal(setup.setup_mode, &"creator", "creator opens from party assembly"); assert_true(not setup.party_pane.visible and setup.creator.size_flags_vertical == Control.SIZE_EXPAND_FILL and setup.creator_page.size_flags_vertical == Control.SIZE_EXPAND_FILL and setup.creator_page.find_child("IdentityPreview", true, false) != null and setup.creator_page.find_child("IdentityFields", true, false) != null and setup.creator_page.find_child("IdentityCampaignContext", true, false) != null, "character creation owns and expands through the full setup stage without the assembly roster")
	...
	assert_true(setup.creator_page.find_child("StartingSpellLevelRail", true, false) != null and setup.creator_page.find_child("StartingSpellListPanel", true, false) != null and setup.creator_page.find_child("StartingSpellRecord", true, false) != null and setup.creator_page.find_child("StartingSpellAllowance", true, false) != null and selected_spell != null and selected_spell.toggle_mode and selected_spell.button_pressed and available_spell != null and spell_description != null and spell_description.text == "Reveals magic." and spell_animation != null and spell_animation.get("resource_ids") == [12032, 12033, 12034, 12035, 12036, 12037, 12038, 12039], "Starting Spells reuses one expanding level/list/detail hierarchy with description, exact Classic animation frames, and ordinary pressed toggle state"); available_spell.pressed.emit(); var spell_payload := creator_intents[-1].payload as PlayerIntent.StringListPayload; assert_equal(spell_payload.values, ["classic.spell.1101", "classic.spell.1102"], "ordinary spell toggles preserve selections on the current and hidden levels without a modifier key")
	router.free()
