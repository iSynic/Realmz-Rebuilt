...
func _test_location_notes(content: RealmzContent) -> void:
	...
	assert_equal([session.view().current_location_note.darkness_value, session.view().location_notes[0].preview_map.party_coordinate, session.view().location_notes[0].preview_map.dark, session.view().location_notes[0].preview_map.darkness_level, session.view().location_notes[0].preview_map.cells().size() <= 195, SessionViewProjector.classic_darkness_level(0), SessionViewProjector.classic_darkness_level(119), SessionViewProjector.classic_darkness_level(999)], [3, original_coordinate, true, 3, true, 0, 4, 6], "a dark land note and live authored darkness retain Castle's clamped torch-derived level in a bounded detached preview recentered on the saved coordinate")
