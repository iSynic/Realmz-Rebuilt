...
func _test_character_creator_workflow() -> void:
	...
	prior_character_list.free(); setup.name_edit.text = "Ari"; setup.creator_next_button.pressed.emit(); var race_button := setup.race_list.option_button("race.human"); var caste_button := setup.caste_list.option_button("caste.sorcerer"); assert_true(setup.creator_page.find_child("RaceSelectorPanel", true, false) != null and setup.creator_page.find_child("CasteSelectorPanel", true, false) != null and setup.race_list.item_count == 1 and setup.caste_list.item_count == 1 and race_button != null and race_button.toggle_mode and race_button.button_pressed and caste_button != null and caste_button.toggle_mode and caste_button.button_pressed and (setup.creator_page.find_child("RaceDescription", true, false) as Label).text == "Adaptable." and (setup.creator_page.find_child("CasteDescription", true, false) as Label).text == "Arcane caster.", "Race and Caste reuse the pressed selector language while literal fallback records stay out of ordinary creation")
