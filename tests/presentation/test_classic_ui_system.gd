...
func _test_scene_composition() -> void:
	...
	assert_equal([world_panel.size_flags_horizontal, narrative_well.size_flags_horizontal, party_panel.size_flags_horizontal, (shell.find_child("WorldCommandGrid", true, false) as GridContainer).size_flags_horizontal, (shell.find_child("CommandGrid", true, false) as GridContainer).size_flags_horizontal], [Control.SIZE_EXPAND_FILL, Control.SIZE_SHRINK_CENTER, Control.SIZE_EXPAND_FILL, Control.SIZE_SHRINK_CENTER, Control.SIZE_SHRINK_CENTER], "the wide footer keeps a bounded narrative measure and centers both command groups in their flexible side panes")
	...
	var modal_parent := Control.new(); var modal_presenter := load("res://src/presentation/interaction_presenter.tscn").instantiate() as InteractionPresenter; modal_parent.add_child(modal_presenter); modal_presenter.call("_update_modal_shield", true); var modal_shield := modal_parent.get_node("LockedModalShield") as Control; var first_order := [modal_shield.get_index(), modal_presenter.get_index()]; modal_presenter.call("_update_modal_shield", true); var repeated_order := [modal_shield.get_index(), modal_presenter.get_index()]; var floating_choice_rect := InteractionPresenter.floating_choice_rect(Rect2(0.0, 28.0, 992.0, 502.0), Rect2(286.0, 530.0, 620.0, 190.0), Vector2(286.0, 46.0)); var compact_choice_rect := InteractionPresenter.floating_choice_rect(Rect2(0.0, 28.0, 592.0, 410.0), Rect2(10.0, 438.0, 780.0, 162.0), Vector2(286.0, 46.0)); assert_true(first_order[0] < first_order[1] and repeated_order[0] < repeated_order[1] and repeated_order == first_order and InteractionPresenter.uses_floating_choice_modal(InteractionRequest.yes_no("floating-choice", "Do you follow him?", "Yes", "No")) and floating_choice_rect.end.y <= 522.0 and floating_choice_rect.size == Vector2(520.0, 116.0) and compact_choice_rect.end.x <= 582.0 and compact_choice_rect.end.y <= 430.0, "locked modals retain pointer order while AP choices keep their question and actions together above the narrative region in both supported profiles"); var flash_sounds: Array[int] = []; modal_presenter.presentation_sound_requested.connect(func(sound_id: int) -> void: flash_sounds.append(sound_id)); modal_presenter.queue_classic_flash_messages([{"text": "Map one", "soundId": 30005}, {"text": "Map two", "soundId": 30005}]); (modal_parent.find_child("ClassicFlashContinue", true, false) as Button).pressed.emit(); var second_flash := modal_parent.find_child("ClassicFlashMessage", true, false) as PanelContainer; assert_true(second_flash != null and second_flash.size == Vector2(520.0, 118.0) and (modal_parent.find_child("ClassicFlashText", true, false) as Label).text == "Map two" and flash_sounds == [30005, 30005], "source-owned flash messages retain one compact ordered click boundary and play their cue once per displayed record"); modal_parent.free()
