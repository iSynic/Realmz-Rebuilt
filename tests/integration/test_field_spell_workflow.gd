...
func run() -> void:
	...
	assert_equal(session.submit_intent(PartyIntents.import_vault_character(caster.id, "1".repeat(64), caster, "fixture", content.package_hash)).state, SessionStep.State.COMPLETED, "caster enters party setup")
	assert_equal(session.submit_intent(PartyIntents.import_vault_character(target.id, "2".repeat(64), target, "fixture", content.package_hash)).state, SessionStep.State.COMPLETED, "target enters party setup")
	assert_equal(session.submit_intent(PartyIntents.begin_adventure()).state, SessionStep.State.COMPLETED, "field-spell fixture begins")
	...
	assert_equal(bound_restored.restore(content, bound_save).state, SessionStep.State.COMPLETED, "Fast Spell state restores through the save v5 envelope")
	assert_equal(bound_restored.view().party_members[0].fast_spells[0].power, 2, "restoration retains the exact Fast Spell power")
	var invalid_binding := session.submit_intent(MagicIntents.set_fast_spell(active_caster.id, 1, "classic.spell.missing", 1))
	...
	var fast_cast := MagicIntents.cast(active_caster.fast_spell_at(0).spell_id, active_caster.id, active_target.id, active_caster.fast_spell_at(0).power)
	session._context.rng = ScriptedRng.new([0, 0, 32_767])
	...
	active_target.current_health = 20
	session._context.rng = ScriptedRng.new([0, 0, 32_767])
	var exact_cost_result := session.submit_intent(MagicIntents.cast("classic.spell.field-bolt", active_caster.id, active_target.id, 1))
	...
	session._context.rng = ScriptedRng.new([0, 0, 32_767])
	var requested := session.submit_intent(MagicIntents.cast("classic.spell.field-bolt", active_caster.id, "", 1))
	...
	var target_context := (requested.interaction.body as CharacterSelectionRequestBody).spell_context
	...
	assert_equal(session._context.rng.snapshot().draw_count, 0, "opening target selection consumes no effect randomness")
	...
	restored._context.rng = ScriptedRng.new([0, 0, 32_767])
	var pending := restored.view().pending_interaction
	assert_equal((pending.body as CharacterSelectionRequestBody).spell_context.to_data(), target_context.to_data(), "save restoration reconstructs the exact field-spell target display context")
	...
	assert_equal([restored._context.state.party.character_by_id(active_caster.id).spell_points, restored._context.rng.snapshot().draw_count], [50, 0], "a rejected target spends neither spell points nor RNG")
	...
	assert_equal(restored._context.state.party.character_by_id(active_caster.id).spell_points, 48, "field casting spends the absolute Classic cost once")
	assert_equal(restored._context.state.party.character_by_id(active_target.id).current_health, 16, "field damage applies after the selected character fails its indexed save")
	assert_equal(restored._context.rng.snapshot().draw_count, 3, "field casting rolls shared duration, damage, and save without a magic-resistance roll")
	...
	var restored_caster := restored._context.state.party.character_by_id(active_caster.id); var restored_target := restored._context.state.party.character_by_id(active_target.id); restored_caster.spell_points = 50; restored_target.conditions.set_value(ConditionRules.POISONED, 5); var cure_requested := restored.submit_intent(MagicIntents.cast("classic.spell.heal-poison", restored_caster.id, "", 1)); assert_equal([cure_requested.state, cure_requested.interaction.kind], [SessionStep.State.WAITING_FOR_INTERACTION, InteractionRequest.CHARACTER_SELECTION], "a field condition cure uses the same typed target boundary")
	...
	restored._context.rng = ScriptedRng.new([0, 0])
	var light := restored.submit_intent(MagicIntents.cast("classic.spell.field-light", active_caster.id, "", 1))
	assert_equal(light.state, SessionStep.State.COMPLETED, "party-state field magic commits without a character picker")
	assert_equal(restored._context.state.party.conditions.value(0), 29, "Classic light stores thirty turns per power minus one")
	...
	var wizard_caster := restored._context.state.party.character_by_id(active_caster.id); wizard_caster.spell_points = 200; restored._context.rng = ScriptedRng.new([0, 0, 0, 0]); var wizard_eye := restored.submit_intent(MagicIntents.cast("classic.spell.1512", wizard_caster.id, "", 1))
	assert_equal([wizard_eye.state, restored._context.state.party.conditions.is_active(ConditionRules.PARTY_WIZARDS_EYE), restored.view().map_view.cell_at(wizard_probe).visible, restored.view().map_view.seen_coordinates().has(wizard_probe)], [SessionStep.State.COMPLETED, true, true, true], "casting Wizard's Eye immediately extends authoritative LOS and sight memory beyond the ordinary radius")

	restored._context.state.party.fatigue = 90
	restored._context.rng = ScriptedRng.new([0, 0])
	var rested := restored.submit_intent(MagicIntents.cast("classic.spell.field-rest", active_caster.id, "", 1))
	assert_equal(rested.state, SessionStep.State.COMPLETED, "Classic fatigue magic commits as a party-state field spell")
	assert_equal(restored._context.state.party.fatigue, 4, "Castle's updatefat clamp makes special 68 produce fatigue four")
	var heroism_caster := restored._context.state.party.character_by_id(active_caster.id); var heroism_before := [heroism_caster.current_health, heroism_caster.spell_points, heroism_caster.conditions.to_data()]; restored._context.rng = ScriptedRng.new([0, 0]); var heroism_result := restored.submit_intent(MagicIntents.cast("classic.spell.4308", heroism_caster.id, "", 1)); assert_equal([ClassicSpellDispositionRules.field_character_disposition(content.magic.spell_by_id("classic.spell.4308")), heroism_result.state, heroism_caster.current_health, heroism_caster.spell_points, heroism_caster.conditions.to_data()], [ClassicSpellDispositionRules.DISPOSITION_EXECUTABLE, SessionStep.State.COMPLETED, heroism_before[0], heroism_before[1], heroism_before[2]], "field Heroism spends its zero cost and leaves the self target unchanged through the public intent boundary"); assert_equal(restored.rng_trace().map(func(entry: Dictionary) -> String: return String(entry.get("tag", ""))), ["field-spell.4308.duration", "field-spell.4308.damage"], "field Heroism preserves Castle's duration-before-damage draw order"); var discover_single := content.magic.spell_by_id("classic.spell.1101"); var discover_group := content.magic.spell_by_id("classic.spell.2102"); var discover_group_peer := content.magic.spell_by_id("classic.spell.3102"); heroism_caster.spell_points = 10; var discover_before := [heroism_caster.current_health, heroism_caster.conditions.to_data(), restored._context.state.party.character_by_id(active_target.id).current_health, restored._context.state.party.character_by_id(active_target.id).conditions.to_data()]; restored._context.rng = ScriptedRng.new([0, 0, 0, 0, 0, 0]); var discover_all := restored.submit_intent(MagicIntents.cast(discover_group.id, heroism_caster.id, "", 1)); var discover_one := restored.submit_intent(MagicIntents.cast(discover_single.id, heroism_caster.id, active_target.id, 1)); var discover_saved := GameSession.new(); assert_equal(discover_saved.restore(content, save_round_trip(restored.snapshot())).state, SessionStep.State.COMPLETED, "field Discover Magic state restores through the public save envelope"); assert_equal([discover_single.special, discover_single.target_type, discover_group.special, discover_group.target_type, ClassicSpellDispositionRules.field_character_disposition(discover_single), ClassicSpellDispositionRules.field_character_disposition(discover_group), ClassicSpellDispositionRules.field_character_disposition(discover_group_peer), discover_all.state, discover_one.state, heroism_caster.spell_points, heroism_caster.current_health, heroism_caster.conditions.to_data(), restored._context.state.party.character_by_id(active_target.id).current_health, restored._context.state.party.character_by_id(active_target.id).conditions.to_data(), discover_saved._context.state.party.character_by_id(heroism_caster.id).spell_points], [63, 1, 63, 4, ClassicSpellDispositionRules.DISPOSITION_EXECUTABLE, ClassicSpellDispositionRules.DISPOSITION_EXECUTABLE, ClassicSpellDispositionRules.DISPOSITION_EXECUTABLE, SessionStep.State.COMPLETED, SessionStep.State.COMPLETED, 8, discover_before[0], discover_before[1], discover_before[2], discover_before[3], 8], "package Discover Magic I and both Discover Magic records preserve their target-one or whole-party field launch, spend once, leave character state unchanged, and save the committed cost"); var utility_ids := [1609, 2504, 2609, 2611, 3111, 3112, 3410, 3709, 3711]; var utility_spells := utility_ids.map(func(classic_id: int) -> SpellDefinition: return content.magic.spell_by_id("classic.spell.%d" % classic_id)); assert_true(utility_spells.all(func(spell: SpellDefinition) -> bool: return ClassicSpellDispositionRules.field_character_disposition(spell) == ClassicSpellDispositionRules.DISPOSITION_EXECUTABLE), "all nine application encounter-utility profiles are admitted through their ordinary field launch"); heroism_caster.spell_points = 200; var utility_before := [heroism_caster.current_health, heroism_caster.conditions.to_data(), restored._context.state.party.character_by_id(active_target.id).current_health, restored._context.state.party.character_by_id(active_target.id).conditions.to_data()]; restored._context.rng = ScriptedRng.new([0, 0, 0, 0]); var shape := restored.submit_intent(MagicIntents.cast("classic.spell.1609", heroism_caster.id, "", 1)); var teleport := restored.submit_intent(MagicIntents.cast("classic.spell.3711", heroism_caster.id, active_target.id, 1)); var utility_saved := GameSession.new(); assert_equal(utility_saved.restore(content, save_round_trip(restored.snapshot())).state, SessionStep.State.COMPLETED, "committed encounter-utility casts restore through the public save envelope"); assert_equal([shape.state, teleport.state, heroism_caster.spell_points, heroism_caster.current_health, heroism_caster.conditions.to_data(), restored._context.state.party.character_by_id(active_target.id).current_health, restored._context.state.party.character_by_id(active_target.id).conditions.to_data(), utility_saved._context.state.party.character_by_id(heroism_caster.id).spell_points, shape.events.any(func(event: DomainEvent) -> bool: return event.kind == &"sound_requested" and event.payload.get("soundId") == 633 and not event.payload.get("waitForCompletion"))], [SessionStep.State.COMPLETED, SessionStep.State.COMPLETED, 105, utility_before[0], utility_before[1], utility_before[2], utility_before[3], 105, true], "Shape Earth preserves target-eleven party selection and completion sound while target-zero Teleport preserves one selected character; both spend fixed absolute cost without inventing a terrain effect"); assert_equal(restored.rng_trace().map(func(entry: Dictionary) -> String: return String(entry.get("tag", ""))), ["field-spell.1609.duration", "field-spell.1609.damage", "field-spell.3711.duration", "field-spell.3711.damage"], "encounter-utility field casts retain Castle's shared duration-before-damage draw order")
	var vapor_item := content.items.item_by_id("classic.item.609"); var refresh_item := content.items.item_by_id("classic.item.723"); heroism_caster.set_inventory([]); heroism_caster.carried_load = 0; var item_rules := RealmzRules.new(); var vapor_instance := item_rules.inventory.add_item(heroism_caster, vapor_item, "instance.vaporous-form", true); var refresh_instance := item_rules.inventory.add_item(heroism_caster, refresh_item, "instance.refresh-potion", true); restored._context.state.party.conditions.set_value(0, 0); restored._context.state.party.fatigue = 90; restored._context.rng = ScriptedRng.new([0, 0, 0, 0]); var vapor_result := restored.submit_intent(InventoryIntents.use(vapor_instance.id, heroism_caster.id)); var refresh_result := restored.submit_intent(InventoryIntents.use(refresh_instance.id, heroism_caster.id)); var item_saved := GameSession.new(); assert_equal(item_saved.restore(content, save_round_trip(restored.snapshot())).state, SessionStep.State.COMPLETED, "application party-state field items restore through the public save envelope"); assert_equal([ClassicSpellDispositionRules.field_character_disposition(content.magic.spell_by_id("classic.spell.4310")), ClassicSpellDispositionRules.field_character_disposition(content.magic.spell_by_id("classic.spell.4713")), vapor_result.state, refresh_result.state, restored._context.state.party.conditions.value(0), restored._context.state.party.fatigue, vapor_instance.charges, refresh_instance.charges, heroism_caster.spell_points, item_saved._context.state.party.conditions.value(0), item_saved._context.state.party.fatigue], [ClassicSpellDispositionRules.DISPOSITION_EXECUTABLE, ClassicSpellDispositionRules.DISPOSITION_EXECUTABLE, SessionStep.State.COMPLETED, SessionStep.State.COMPLETED, 1, 4, 1, 1, 105, 1, 4], "Vaporous Form and Refresh Potion spend one package-owned charge and reproduce Castle's party-condition-zero and fatigue-four effects without spell-point cost"); assert_equal(restored.rng_trace().map(func(entry: Dictionary) -> String: return String(entry.get("tag", ""))), ["field-spell.4310.duration", "field-spell.4310.damage", "field-spell.4713.duration", "field-spell.4713.damage"], "both application field items retain shared duration-before-damage RNG order"); var ally_definition: MonsterDefinition = content.combat.monster_by_id("classic.monster.field-ally"); var ally := MonsterState.new("field.ally", ally_definition.id, ally_definition.name, 12, 12, ally_definition.hit_dice, ally_definition.agility, ally_definition.armor, 127, 0, false); restored._context.state.party.set_allies([ally]); restored._context.rng = ScriptedRng.new([0, 0, 0]); var aura := restored.submit_intent(MagicIntents.cast("classic.spell.2106", heroism_caster.id, "", 1)); var aura_saved := GameSession.new(); assert_equal(aura_saved.restore(content, save_round_trip(restored.snapshot())).state, SessionStep.State.COMPLETED, "a committed field ally effect restores through the public save envelope"); assert_equal([aura.state, heroism_caster.spell_points, heroism_caster.conditions.value(4), restored._context.state.party.character_by_id(active_target.id).conditions.value(4), ally.conditions.value(4), aura_saved._context.state.party.allies()[0].conditions.value(4), aura.events.filter(func(event: DomainEvent) -> bool: return event.kind == &"field_spell_resolved").map(func(event: DomainEvent) -> Array: return [event.payload.get("targetId"), event.payload.get("targetKind")]), restored._context.rng.snapshot().draw_count], [SessionStep.State.COMPLETED, 101, 1, 1, 1, 1, [[heroism_caster.id, "character"], [active_target.id, "character"], [ally.id, "monster"]], 3], "target-nine field magic affects party characters then every held-over ally, ignores ally magic resistance, publishes target kinds, spends once, and consumes only the shared duration and damage draws")
...
func _field_content(source: RealmzContent) -> RealmzContent:
	...
	var ally_definition := MonsterDefinition.new("classic.monster.field-ally", 1, "Field Ally", 1, 0, 1, 0, 0, [], [], [], [], [], [], []); var library := PackageRepository.new().load_bundled_package(ApplicationLibraryIdentity.PATH, ApplicationLibraryIdentity.CAMPAIGN_ID, ApplicationLibraryIdentity.PACKAGE_HASH); items.append(library.content.items.item_by_classic_id(609)); items.append(library.content.items.item_by_classic_id(723)); var spells: Array[SpellDefinition] = [bolt, fixed, light, rest, cure, remove_item, heroism, library.content.magic.spell_by_classic_id(1101), library.content.magic.spell_by_classic_id(1512), library.content.magic.spell_by_classic_id(2102), library.content.magic.spell_by_classic_id(3102), library.content.magic.spell_by_classic_id(2106)]; for classic_id: int in [1609, 2504, 2609, 2611, 3111, 3112, 3410, 3709, 3711, 4310, 4713]: spells.append(library.content.magic.spell_by_classic_id(classic_id))
