...
- The composition root owns one `HeldMovementController` shared by keyboard and mouse exploration. It submits one immediate typed single-step movement, requires a 150-millisecond hold before repetition begins, then uses the persisted 25–400 percent cadence from twenty scheduled steps per second at 100 percent without entering `.r2save` or simulation state. The scheduler emits at most one step per rendered frame and never catches up missed intervals. It stops at every host/session boundary. The same host settings boundary applies the optional traveled-area preview, default-on Auto Note behavior, Classic/readable typography choice, separate effects/music volumes, global music enablement, and twenty playlist modes to presentation only.
...
- Debug builds alone bind F12 to `DebugToolsHost`. The host exposes the presentation-only floating tool surface, whose warp coordinates accept direct text or spinner input, retains unsaved no-clip state and a bounded recent history translated from committed Auto events, and translates every mutation into a typed public `SessionDebugCommand`; release UI, saves, and ordinary gameplay intents never carry these controls, logs, or flags.
