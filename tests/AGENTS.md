...
- Weapon-mode tests preserve Castle's melee-first/type-15-only fallback, no-cost toggle, hand-to-hand return, unique type-2/type-10/type-15 slot validation, whole-state restoration, detached-view/request parity, and exact presenter payload. Character Fire and monster missile execution must remain explicitly unavailable until adjacency/range/LOS and projectile resolution exist; a monster missile decision may never call or label an ordinary melee row. Tactical evidence explicitly records that positive `behind` is withdrawal, not icon facing.
...
- Tactical combat tests use explicit battlefield placement. They cover eight-direction one-step probes, complete large footprints, maximum terrain cost, adjacent-only melee, rejected-action transactionality, movement save/restore, explicit remote-monster suppression, and source-legal placement in scenario continuations. `death-macro-battlefield-occupancy-correction.json` records `FD-COMBAT-007`; a revived monster must remain positioned and saveable.
