...
- Ordinary monster melee receives a detached attack context containing the resolved carried weapon, Realmz day, effective defender luck/armor, and party Dragon Hide state. With optional fumbles disabled, it preserves Castle's luck-before-hit and weapon/unarmed damage order, evil-conditional protection, ten-percent floor, typed weapon gates, and Dragon Hide's physical-only reduction plus asynchronous sound 694. `FD-COMBAT-004` floors negative physical damage at zero instead of allowing Castle's subtraction to heal the defender. Fumbles, reflection, missile replacement, and derived behind/facing state remain unavailable until their tactical state is session-owned.
