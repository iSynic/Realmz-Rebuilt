...
- Package-local media integrity plus effective application-and-scenario media validation and resolution.
...
- `PackageDomainAssembler` coordinates typed package construction through public decoder and validator operations. After application-first exact-ID composition with legal scenario overlays, it constructs one `RealmzContent` directory whose `ScenarioContentCatalog`, `CharacterCatalog`, `ItemCatalog`, `SpellCatalog`, `CombatCatalog`, and `EconomyContentCatalog` own feature lookups directly. `PackageMediaComposer` is the shared scenario-first media overlay policy: only an exact Classic resource-type-and-ID key replaces application media. `PackageContentDecoder` is the stable record-family entry point over `PackageStoryContentDecoder`, `PackageCharacterContentDecoder`, and `PackageEncounterContentDecoder`. `PackageWorldDecoder` exposes world-family `decode_*` operations, `PackageScenarioDecoder.decode_scenario` owns scenario construction, and cross-reference/media validation uses named public validation or resolution methods; package collaborators must not call one another's private helpers.
- `ApplicationLibraryIdentity` is the one GDScript source for the committed Providence-built application package path, acceptance-lock path, campaign ID, and package hash. Runtime and developer probes consume that identity rather than repeating literals. `application/` owns those immutable release inputs.
...
- External packages receive complete integrity, semantic, topology, media, and cross-reference validation before installation. Media validation has two ordered phases: package-local integrity validates only the scenario archive's asset identities, metadata, manifest entries, hashes, and bytes; effective completeness then composes application media with scenario-owned records and exact overrides before validating appearance denominators, render references, battle capabilities, monster art, player maps, and scenario references. The returned `PackageMediaCatalog` remains scenario-owned so the runtime can read bytes from the correct archive. Before cross-reference validation, assembly materializes an immutable empty `MessageDefinition` only for a missing direct Simple/Complex Encounter prompt ID: Castle preloads an empty string and leaves it in place when its unchecked `Data SD2` read misses. Item-specific race/caste ID `-32768` is the other narrow source sentinel: Castle resolves no display name and no character can match it, so the typed unresolved identity remains an unmatchable use restriction; arbitrary missing identities still reject. Missing scenario-text opcodes, battle messages, result programs, and every other unresolved reference also reject. A valid app-owned receipt binds the scenario to the exact application package hash and permits later startup to validate cheap file metadata and manifest identity without repeating compiler-level semantic validation. An installed revision may restore its four primitive-only decoded documents from a Zstd sidecar tied to the receipt's exact archive SHA-256, schema hash, decoder version, payload size, and payload SHA-256. The sidecar never replaces manifest/header/domain validation and never deserializes objects; absence, mismatch, or corruption forces one complete archive SHA-256 before reparsing the immutable archive and replacing the sidecar atomically.
- Castle-distributed bundled campaigns remain ordinary untrusted scenario inputs. Manifest-only discovery may advertise them from `res://`, but first Play performs the same complete validation and immutable `user://packages` installation as any external package. Only the separately pinned stock application library uses the bundled-identity load shortcut.
- An otherwise valid installation receipt naming an older application library cannot reuse its cached graph or runtime image. Verify the recorded archive SHA-256, fully revalidate the unchanged scenario against the new application catalog, then replace its receipt through the ordinary installation path. Malformed receipts, decoder/schema drift, altered bytes, and failed composition still reject without replacing the archive.
...
- `application/AGENTS.md` owns the canonical Providence-built application library and its compact acceptance lock.
