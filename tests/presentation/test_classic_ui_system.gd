...
func _test_battle_typed_option_contracts() -> void:
	...
	var spell_definition := SpellDefinition.new("classic.spell.1306", 1306, "Brimstones", "Burning stones strike a fixed battlefield area."); spell_definition.range_min = 1; spell_definition.range_max = 2; spell_definition.damage_min = 2; spell_definition.damage_max = 6; spell_definition.power_damage_min = 1; spell_definition.power_damage_max = 2; spell_definition.duration_min = 1; spell_definition.duration_max = 3; spell_definition.damage_type = 2; var actor_state := CharacterState.new(String(opened["actor"]), "Hero", 10, 10); actor_state.spell_points = 8; actor_state.maximum_spell_points = 12; var actor_view := CharacterView.new(actor_state); actor_view.spells = [SpellView.new(spell_definition)]
...
func _test_fixture_gallery_coverage() -> void:
	...
	var level_request := ClassicUiFixtureGallery.request_for(InteractionRequest.LEVEL_UP)
	level_component.build(level_request)
	...
	assert_false(InteractionPresenter.uses_application_workspace(level_request), "a committed level result uses a locked floating modal rather than replacing the application workspace")
	assert_equal(InteractionPresenter.preferred_modal_size(level_request, Vector2(984, 494)), Vector2(760, 430), "the level result keeps a compact Castle-shaped modal footprint")
	...
	assert_true(_buttons_in(spell_component.find_child("LevelSpellList", true, false)).filter(func(button: Button) -> bool: return button.has_meta(&"spell_id")).size() == 4 and _buttons_in(spell_component.find_child("LevelSpellList", true, false)).all(func(button: Button) -> bool: return button.theme_type_variation == &"ClassicTheldrowButton" and button.toggle_mode), "the level spell stage renders every detached candidate through the shared pressed Theldrow spell controls")
	...
	var ally_request := ClassicUiFixtureGallery.request_for(InteractionRequest.ALLY_SELECTION); var ally_component := SelectionInteraction.new(); ally_component.build(ally_request); assert_true(ally_component.find_child("AllyCandidates", true, false) != null and ally_component.find_child("AllySelectionContinue", true, false) != null, "surviving allies use one dominant candidate workspace with a fixed continuation"); assert_false(InteractionPresenter.uses_full_stage_region(ally_request), "surviving-allies selection is a locked Castle modal rather than a stale tactical-stage replacement"); ally_component.free()
	var completion_treasure := InteractionRequest.from_payload("fixture.treasure.complete", InteractionRequest.TREASURE_DISTRIBUTION, {"mode": "completion-confirmation", "summary": "One item remains unclaimed."})
	var completion_component := TreasureDistributionInteraction.new(); completion_component.configure(item_media, null, false); completion_component.build(completion_treasure)
	assert_true(completion_component.find_child("TreasureCompletionConfirmation", true, false) != null and _buttons_in(completion_component).any(func(button: Button) -> bool: return button.text == "Return to treasure"), "leave-behind confirmation retains an explicit path back to the Treasure workspace")
	completion_component.free()
