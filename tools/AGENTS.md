...
- `combat_performance_probe.gd` constructs a deterministic six-character battle from a caller-supplied package and reports synchronous battle setup, detached combat-view construction, full-state checkpoint, and Auto-command timings. It accepts an optional Classic battle ID, embeds no campaign path or payload, and measures simulation/projection work rather than Godot draw latency.
