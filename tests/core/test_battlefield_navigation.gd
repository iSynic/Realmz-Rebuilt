...
func _test_weighted_complete_footprint_route() -> void:
	var field := _field(1)
	field.terrain.set_tile(Vector2i(41, 44), 3)
	field.actors.place_monster("mover", Vector2i(40, 45), 1)
	field.actors.place_character("target", Vector2i(47, 45))
	...
	var reachable := rules.reachable_destinations(field, _terrain_set(), "mover", 2)
	assert_equal(reachable.costs.get(Vector2i(41, 46)), 2, "affordable destinations use the same weighted complete-footprint cost")
	assert_false(reachable.costs.has(Vector2i(41, 44)), "reachable overlay excludes a footprint whose terrain cost exceeds the remaining budget")
	assert_equal(reachable.route_to(Vector2i(41, 46)), [Vector2i(41, 46)], "exact combat route excludes origin and terminates at the selected cell")
...
func _test_static_profile_cache_and_invalidation() -> void:
	...
	field.terrain.set_tile(Vector2i(42, 45), 3)
	...
	assert_false(BattlefieldStateCodec.to_data(field).has("terrainRevision"), "the navigation invalidation revision never enters serialized battlefield state")


func _test_dynamic_occupancy_forecast() -> void:
	var forecast_field := _field(2)
	for x: int in range(40, 50):
		forecast_field.terrain.set_tile(Vector2i(x, 45), 1)
	forecast_field.actors.place_character("mover", Vector2i(40, 45))
	forecast_field.actors.place_character("traffic", Vector2i(43, 45))
	forecast_field.actors.place_character("target", Vector2i(49, 45))
	var terrain := _terrain_set()
	var forecast := BattlefieldRules.new().probe_path_step_toward_actors(forecast_field, terrain, "mover", ["target"], 20)
	var affordable := BattlefieldRules.new().reachable_destinations(forecast_field, terrain, "mover", 20)
	assert_false(affordable.costs.has(Vector2i(44, 45)), "player destination planning cannot forecast a vacancy beyond an occupied choke point")
	...
	var swap_forecast_field := _field(1); swap_forecast_field.actors.place_character("mover", Vector2i(40, 45)); swap_forecast_field.actors.place_character("traffic", Vector2i(42, 45)); swap_forecast_field.actors.place_character("target", Vector2i(44, 45)); swap_forecast_field.terrain.set_tile(Vector2i(42, 44), 2); swap_forecast_field.terrain.set_tile(Vector2i(42, 46), 2); var swap_forecast := BattlefieldRules.new().probe_path_step_toward_actors(swap_forecast_field, terrain, "mover", ["target"], 20, ["traffic"]); assert_equal([swap_forecast.allowed, swap_forecast.destination], [true, Vector2i(41, 44)], "a later legal allied swap carries its real five-movement cost so repeated route-first decisions do not steer into and then away from the same occupied lane")
	var immediate_field := BattlefieldStateCodec.from_data(BattlefieldStateCodec.to_data(forecast_field)); immediate_field.actors.move_actor("traffic", Vector2i(41, 45))
...
func _test_obstruction_and_unreachable_routes() -> void:
	...
	u_field.actors.place_character("mover", Vector2i(40, 45))
	u_field.actors.place_character("target", Vector2i(47, 45))
	for coordinate: Vector2i in [Vector2i(41, 45), Vector2i(44, 44), Vector2i(44, 45), Vector2i(44, 46), Vector2i(45, 44), Vector2i(45, 46), Vector2i(46, 44), Vector2i(46, 46)]:
		u_field.terrain.set_tile(coordinate, 2)
	...
	var sealed := _field(1)
	sealed.actors.place_character("mover", Vector2i(40, 45))
	sealed.actors.place_character("target", Vector2i(47, 45))
	for y: int in range(0, BattlefieldGrid.SIZE):
		sealed.terrain.set_tile(Vector2i(44, y), 2)
...
func _test_all_footprint_profiles() -> void:
	...
	for actor_size: int in 4:
		var field := _field(1)
		field.actors.place_monster("mover", Vector2i(40, 45), actor_size)
		field.actors.place_character("target", Vector2i(47, 45))
