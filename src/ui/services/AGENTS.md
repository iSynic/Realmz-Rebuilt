...
- `treasure_distribution_interaction.tscn` owns ordinary loot distribution, wealth actions, lore controls, recipient selection, item facts, recovery, and the nested completion confirmation. Its canonical grid reserves the recipient column before wrapping loot, and its bounded command pane keeps Done fixed while excess wealth or lore controls scroll locally. `ClassicTreasureTakeEffect` is presentation-only pickup animation; `TreasureDisplayText` formats detached public summaries.
