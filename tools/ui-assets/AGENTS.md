...
- Deterministic import of the licensed Realmz intro GIF into a portable 12-fps application loop with an offline-verifiable frame manifest, plus deterministic derivation of its generated ornamental frame.
...
- `import-realmz-intro.ps1` accepts only the approved source GIF hash, validates its 480x276/124-frame shape, samples every second frame without spatial rescaling, groups two source delays per output frame, and records every output hash. The external source path never enters committed metadata.
