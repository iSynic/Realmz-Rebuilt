...
func _test_door_item_xap(content: RealmzContent) -> void:
	...
	var option := restored._context.rules.combat_flow.character_item_spell_options(restored._context.state, content, carried.id)[0]; assert_equal([option.item_instance_id, option.spell_id, option.target_mode], [instance.id, "", &"automatic"], "negative Special 1 door item is an automatic combat item action without a fabricated spell")
