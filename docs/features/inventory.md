...
Saveable item targeting, drop confirmation, and item-owned XAP handoffs are created by `InventoryContinuations`. Their typed `TargetingContinuationBody` or `ItemXapContinuationBody` enters the central continuation codec only when a save is written or restored.
