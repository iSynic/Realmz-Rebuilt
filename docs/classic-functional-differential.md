...
The rule, clock, spell, complete monster-special attack table, battle-allegiance, battle-reward, persistence, `showageupdate` click boundaries, character melee foundation, bounded ordinary monster melee resolver, and physical attack cadence are covered. Battle placement distance no longer contaminates melee rules; magical-plus, blunt, sharp, and corrected specific-item gates use distinct typed monster fields. Charmed-character cadence, monster fumbles, reflection, missile-weapon replacement, behind/facing derivation, critical ability values, spell/item cadence, missile toggling, and full equipment side effects remain separate unresolved boundaries. Castle also disagrees with itself about zero/one half-attack display versus control flow and Speedy/Slow display versus activation math; no broad correction is inferred. Random-weapon table 2 still needs spawn-origin state because Castle resolves its overlapping roll-85 rows differently for summoned and ordinary battle monsters. The next rolling combat slice should use the serialized active-turn boundary for source-backed fumble recovery and dropped-item state without folding in unresolved missile or spell behavior.
