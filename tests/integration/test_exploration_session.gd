...
func run() -> void:
	...
	assert_equal(diagonal_session._context.state.clock.total_minutes(), content.world.map_by_id("land:0").topology.cell_at(Vector2i.ZERO).movement_cost * 5, "outdoor movement scales the authored Classic timeclick count to five-minute clicks")
	...
	assert_equal(session._context.state.clock.total_minutes(), hidden_start_minutes + hidden_destination.movement_cost * 5, "concealed-square entry pays the underlying tile's five-minute Classic timeclick cost")
...
func _test_field_heal(content: RealmzContent) -> void:
	...
	assert_equal([session._context.state.clock.total_minutes() - before_minutes, healer.spell_points, wounded.current_health > 1, wounded.current_health <= wounded.maximum_health], [5, 15, true, true], "one land Heal pulse spends five minutes and ten spell points for its eligible wounded recipient")
	...
	blocked_before = JSON.stringify(session._context.state.to_data())
