...
- `CombatMacroSpells`, reached through `CombatFlowMagic`, resolves immediate and queued area, side-group, and repeated-target spells around a retained macro source without action costs, activation advancement, or death-macro completion. A queued area rolls and creates its persistent field at the source center before its immediate resolution, using the shared field owner and event contract. Repeated-target spells (targetType 0) select living actors within range and LOS of the retained macro source, friendly if `cannot == 4` and opposed otherwise, up to power/fixed-count. Because Castle's macro path corrupts its actor selector, Rebuilt deliberately takes the first legal party-then-monster targets without a target-choice RNG draw. Single-target (targetType 1) and ray (targetType 6) spells fail explicitly with typed reasons (`unsupported_macro_single_target` and `unsupported_macro_ray`) due to Castle source coordinate-aliasing memory corruption and ray traversal bypass. It shares ordinary spell effects and lifecycle cleanup, including nested defeated-monster macro queuing. Dead sources are anchors, never targets; complete living footprints are deduplicated in party-then-monster order. Macro source context, continuation, and completion remain scenario/playthrough-owned.
...
- Character movement starts its serializable Guard reaction after validating an adjacent direction but before terrain, movement cost, contact, or friendly-collision outcome. A blocked attempt keeps the anchor and turn resources and emits its reason after Guard; a same-side collision pauses for the save-owned Swap/Attack/Back-out choice only after Guard settles, and Back out clears that attempt without replaying reactions or spending turn resources.
...
- The corpus-backed zero-cost class-nine monster spell-slot projectile consumes Castle's range-power draw before targeting, resolves at forced power one, and spends no spell points. Physical projectile items admit source-backed specials 0, 7, 28, and 49: special 7 halves character movement on a monster-fired hit, special 28 inflicts Blind through shared permanent-condition and accumulation guards, with duration dealt as damage, and special 49 replaces damage with target health plus ten. Scenario-owned missile items referencing physical damage-type-nine non-projectile spells fire through the ordinary charged-item path. Monster AI evaluates single and group healing spells (including target type 9) for wounded allies and zero-cost actor field spells without artificial cannot, cost, or class guards.
