...
- Three Castle inconsistencies remain open: manual and automated character missile paths disagree about twelve-movement cost; `attack2` uses global `monsterup` while replacing a ranged weapon; and `combatmap` evaluates the top tile with `bottom.ispath`. The runtime preserves the top-tile branch pending a named decision. Current melee target selection is still not proximity-filtered, so existing battle routes are not tactical-fidelity proof.
...
Next rolling priority: trace and implement tactical movement, adjacency, and proximity-gated melee through the persisted battlefield before projectile firing or withdrawal attacks. Keep the top-tile path typo, projectile movement-cost disagreement, wrong-global replacement hazard, spell-per-activation questions, Castle haste/display contradictions, and random-weapon-table spawn-origin split separate until each has its own evidence.
