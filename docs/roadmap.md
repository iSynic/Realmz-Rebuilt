...
## Current rolling fidelity pass — weapon modes and tactical blockers (implemented/blocked split)
...
- Full tactical implementation is blocked on package and session facts: schema v2 flattens land solidity to passability and omits the exact solid class and 3×3 combat-build data Castle uses for placement and large footprints. Positions, adjacency, Euclidean range, LOS, point-blank rules, item-backed projectile resistance, movement cost, opportunity attacks, and missile-to-melee replacement therefore remain disabled rather than approximated.
- Two Castle inconsistencies remain flagged for an explicit decision during that pass: manual and automated character missile paths appear to disagree about the twelve-movement cost, and `attack2` uses global `monsterup` when replacing a ranged weapon, potentially mutating the wrong opportunity attacker. Current 2.0 melee target selection is also not proximity-filtered, so existing battle routes are not tactical-fidelity proof.
- The typed suite passes 1,973 assertions across 12 suites before aggregate closeout. Twenty-three differential cases validate locally against the pinned Castle and Remake roots. Providence symbols at locked commit `c392abecb8c948c62c66be76170131944232fcab` were verified directly from Git; the local Providence reference worktree itself remains at the older `2b5eadfffd605b3b043680f7ad17b1a2c10bf7f1` checkpoint and was intentionally not moved in place.

Next rolling priority: make the Providence/package battle-terrain correction, then implement deterministic Castle placement and proximity-gated melee before projectile firing or withdrawal attacks. Keep spell-per-activation, Castle haste/display contradictions, and the random-weapon-table spawn-origin split separate.
