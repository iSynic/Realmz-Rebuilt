...
- The source-backed Prelude correction is recorded in `prelude-to-pestilence.corrections.json`. It restores the omitted XAP 97 payment-refusal program from the pinned Castle Data ED3 record; the package and provenance catalog carry both before/after identities. Keep this correction revision-guarded and do not treat a missing branch target as a VM fallback.
