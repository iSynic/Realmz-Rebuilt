...
- Scenario-launched total-party defeat does not yet transfer control from the issuing VM continuation into the session-owned Party Death application hook. It remains an explicit fidelity gap because a correct implementation must suspend the originating frame, run and save the hook, close on ordinary death, and resume the correct caller-specific no-reward path after opcode 119 without executing later scenario instructions early.
