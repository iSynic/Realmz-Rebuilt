...
- `startup_probe.gd` measures main-scene resource load, instantiation, synchronous `_ready`, and the first processed frame inside the running engine. It separates application construction from process-launch overhead and joins normal application teardown without treating teardown time as first-frame latency.
