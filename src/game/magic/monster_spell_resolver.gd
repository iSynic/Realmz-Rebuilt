...
func resolve_monster_group_spell(caster: MonsterState, caster_definition: MonsterDefinition, selections: Array[SpellTargetSelection], spell: SpellDefinition, power_level: int, cast_level: int, rng: RealmzRng, allow_empty: bool = false, spend_spell_points: bool = true, polymorph_context: MonsterPolymorphContext = null, allow_area_reflection: bool = false) -> GroupSpellResolution:
	...
	if spend_spell_points:
		...
		spell_cost = 0
	var effective_selections := selections
	if allow_area_reflection and absi(spell.spell_class) != 9:
		effective_selections = _reflect_monster_area_targets(caster, caster_definition, selections, rng)
	...
	var result := GroupSpellResolution.new(true, spell_cost, duration, damage)
	for selection: SpellTargetSelection in effective_selections:
...
func _reflect_monster_area_targets(caster: MonsterState, caster_definition: MonsterDefinition, selections: Array[SpellTargetSelection], rng: RealmzRng) -> Array[SpellTargetSelection]:
	var effective_by_id: Dictionary = {}
	for index: int in selections.size():
		var effective := _reflect_to_monster_caster(caster, caster_definition, selections[index], rng, StringName("magic.monster-area.reflect.%d" % index))
...
func resolve_monster_repeated_spell(caster: MonsterState, caster_definition: MonsterDefinition, selections: Array[SpellTargetSelection], spell: SpellDefinition, power_level: int, cast_level: int, rng: RealmzRng, before_selection: Callable = Callable(), spend_spell_points: bool = true, allow_empty: bool = false) -> RepeatedSpellResolution:
	if caster == null or caster_definition == null or spell == null or rng == null or power_level < 1 or (not allow_empty and selections.is_empty()) or selections.size() > power_level:
		return null
	return _resolve_monster_selection_sequence(caster, caster_definition, selections, spell, power_level, cast_level, rng, true, &"magic.monster-repeated", before_selection, spend_spell_points)
