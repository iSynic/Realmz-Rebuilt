...
- The Character route now exposes its party-order summary, reorder editor, persistent shared sheet, empty state, identity, complete tab rail, and content frame through authored scenes. Character Files now likewise exposes its complete library, empty state, history, eligibility, and inspection composition; exported `party_order_row.tscn`, `vault_character_card.tscn`, and `vault_history_row.tscn` provide the variable records. Tab-specific sheet records remain the next character migration slice. This boundary lowers runtime-created Controls to 372 without claiming the complete feature is finished.
