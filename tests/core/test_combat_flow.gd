...
func _test_batch_two_combat_auto_state() -> void:
	...
	assert_equal([intent.kind, intent.actor_id, intent.enabled], [PlayerIntent.Kind.SET_COMBAT_AUTO, first.id, true], "the public PlayerIntent factory carries the stable Auto toggle")
	...
	var vm_response := InteractionResponse.new("request.auto-vm", InteractionRequest.COMBAT, {"actorId": vm_future.id, "action": "set_auto", "targetId": "", "enabled": true})
	...
	assert_equal(vm_result.interaction.payload.get("autoCharacterIds"), [vm_future.id], "the refreshed typed combat request exposes its saved Auto selection")
