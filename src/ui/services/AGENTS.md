...
- `shop_interaction.tscn` owns the wide two-ledger and compact tabbed Shop compositions. Its script binds request-owned stock, packs, filters, offers, load, inline Pool/Share, and exact typed responses through exported record scenes; `ShopTransactionPresenter` binds the selected facts and core-supplied two-pack transfer availability. The wide center filter rail ends in Pool/Share; compact keeps those actions in its footer. A boxed readout shows detached selected-shopper gold and pooled gold in the wide footer or compact header. Items opens the shared browse-only application workspace: Back restores Shop, Money opens the pending Shop's Party Wealth pane, Done submits Shop leave, and Trade returns to Shop with two distinct shoppers selected. In that two-pack state, both pack rows and hover details omit merchant sale values and restrictions; Take/Give and either-direction drops submit core-checked exact-instance Trade instead of merchant Buy/Sell. Shop Keeper restores stock, sale values, and merchant restrictions. Money retains the pending transaction through Pool, Share, Swap, and changing responses, then returns through contextual Back.
...
- Party Wealth and Shop Money transfer and conversion buttons activate once on press, then repeat after 350 milliseconds at 90-millisecond intervals while held. Rebinding retains their nodes and stops a hold when its action becomes unavailable or the workspace changes. A refreshed request for the same Shop updates the pending Money pane in place; a different Shop or task tears it down.
