...
- The public exploration workflow also owns Search-mode versus Area-Search timing, hidden-land-secret entry with colocated-AP suppression until discovery, field Heal time/SP/RNG/recovery and save restoration, plus contextual Encounter availability, land-facing coordinate, draw/order, XAP 0 fallback, and continuation. The public inventory workflow owns exact item-805 Torch use and field/combat door-item XAP dispatch, charge/drop behavior, save/restore, and battle return. Presentation tests retain only command grouping, workspace structure, roster-mediated Trade, and responsive reachability.
...
- Exploration coverage proves the typed ordinary-movement presentation delta, bounded inherited visibility, restore/LOS full-projection fallback, and adjacent debug no-clip's incremental, RNG-neutral transaction. Presentation suites retain map/shell composition ownership rather than testing private caches.
