...
- `package_probe.gd` exercises the public package repository and session boundary for an arbitrary local `.realmz2` package without installing or committing it; its output separates repository progress phases, public package load, session start, view construction, and representative movement timings.
...
- `startup_probe.gd` measures lightweight front-door scene load, instantiation, synchronous `_ready`, first-frame menu visibility, background application readiness, and a scenario choice queued from that first frame. It separates responsive menu entry from gameplay construction and joins normal application teardown without treating teardown time as first-frame latency.
...
- `ui-assets/verify-classic-application-media.ps1` validates the complete committed integrated-sound manifest, exact resource keys, intro video and independent soundtrack contracts, provenance/license metadata, byte counts, and hashes plus every generated-chrome manifest path, dimension, and hash; Tier 2 and Tier 3 run it without requiring a Castle checkout.
