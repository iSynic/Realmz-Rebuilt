...
func _test_movement_input() -> void:
	...
	held.advance(0.179); assert_equal(pulses.size(), 1, "an ordinary click cannot cross the initial held-repeat threshold")
	held.advance(0.002); assert_equal(pulses.size(), 2, "a deliberate hold begins repeating after 180 milliseconds")
	held.advance(0.024); held.advance(0.002); assert_equal(pulses.size(), 3, "subsequent held steps use the selected 25 millisecond cadence")
	...
	assert_equal(held.interval_seconds(), 0.4, "the slowest movement setting uses the documented 400 millisecond interval"); held.set_speed_percent(100)
	assert_equal(held.interval_seconds(), 0.1, "the default movement setting sustains ten scheduled steps per second")
	held.stop(&"keyboard")
	held.advance(1.0)
