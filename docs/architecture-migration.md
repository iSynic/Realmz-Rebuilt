...
- Character creation now exports five recognizable step scenes from the retained party-setup workspace. Identity, Race/Caste, Appearance, Review, and Starting Spells expose every stable preview, selector, detail, alternate state, level rail, and allowance in Godot; the production controller constructs no Controls and instantiates only exported variable rows or the shared sheet. Runtime-created Controls are down to 60, while 22 files, 88 functions, 35 classes, 193 cross-object private calls, and 128 generic preload aliases remain above final targets.
