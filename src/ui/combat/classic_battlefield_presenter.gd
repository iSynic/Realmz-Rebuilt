...
func clear_playback_frame() -> void:
	interaction.playback_changed(_playback_frame, null)
	_playback_frame = null
...
func controller_pan(direction: Vector2i, center_pointer: bool = false) -> bool:
	...
	if center_pointer:
		var point := BattlefieldPresentationGeometry.coordinate_for_point(get_local_mouse_position(), _render_camera_top_left, visible_cells, size)
...
func _draw_movement_options(combat: CombatView, camera: Vector2i, visible_cells: Vector2i, draw_origin: Vector2) -> void:
	...
	for option: CombatMoveOptionView in combat.movement_options:
		...
		if option.enabled:
			...
			draw_string(get_theme_font(&"font", &"Label"), rect.position + Vector2(2.0, 20.0), label, HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 4.0, 11, Color(1.0, 0.94, 0.68))
		else:
			draw_rect(rect, Color(0.10, 0.08, 0.08, 0.52), true)
			draw_rect(rect, Color(0.70, 0.30, 0.26, 0.78), false, 1.0)
			draw_string(get_theme_font(&"font", &"Label"), rect.position + Vector2(2.0, 20.0), "—", HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 4.0, 12, Color(0.90, 0.62, 0.56))


func _gui_input(event: InputEvent) -> void:
	if _playback_frame == null and interaction.targeting == null and movement_preview != null and movement_preview.handle_pointer(event):
		accept_event()
		return
...
func _input(event: InputEvent) -> void:
	if not event is InputEventKey or not event.pressed or event.echo or event.keycode not in [KEY_CTRL, KEY_META]:
		return
	if not is_visible_in_tree() or get_viewport().gui_get_hovered_control() != self:
		return
	var hover := InputEventMouseMotion.new()
	hover.position = get_local_mouse_position()
	hover.ctrl_pressed = true
...
func _draw_result(target_rect: Rect2, combat: CombatView, camera: Vector2i, visible_cells: Vector2i, draw_origin: Vector2) -> void:
	if _playback_frame.display_text.is_empty():
		return
	if not target_rect.has_area():
		...
		var direction := (BattlefieldPresentationGeometry.cell_rect(target_coordinate, camera, draw_origin).get_center() - (draw_origin + Vector2(visible_cells) * NATIVE_CELL_SIZE * 0.5)).normalized()
		var arrow_center := badge.position + Vector2(12.0, 14.0)
		var arrow_base := arrow_center - direction * 4.0
		var arrow_side := Vector2(-direction.y, direction.x) * 4.0
		var arrow := PackedVector2Array([arrow_center + direction * 6.0, arrow_base + arrow_side, arrow_base - arrow_side])
		var text_color := Color(0.72, 1.0, 0.72) if _playback_frame.result_kind == &"healing" else Color(1.0, 0.95, 0.82)
		draw_rect(badge, Color(0.02, 0.025, 0.03, 0.94), true)
		draw_rect(badge, Color(0.86, 0.72, 0.30, 0.95), false, 2.0)
		draw_colored_polygon(arrow, Color(1.0, 0.86, 0.28))
		draw_string(get_theme_font(&"font", &"Label"), badge.position + Vector2(23.0, 19.0), "%s: %s" % [target_name, _playback_frame.display_text], HORIZONTAL_ALIGNMENT_LEFT, badge.size.x - 27.0, 14, text_color)
		return
	...
	draw_string(get_theme_font(&"font", &"Label"), result_rect.position + Vector2(1.0, 20.0), _playback_frame.display_text, HORIZONTAL_ALIGNMENT_CENTER, result_rect.size.x - 2.0, 14, text_color)
