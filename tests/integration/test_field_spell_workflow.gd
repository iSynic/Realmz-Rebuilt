...
func run() -> void:
	...
	var wizard_caster := restored._context.state.party.character_by_id(active_caster.id); wizard_caster.spell_points = 200; restored._context.rng = ScriptedRng.new([0, 0, 0, 0]); var wizard_eye := restored.submit_intent(PlayerIntent.cast_spell("classic.spell.1512", wizard_caster.id, "", 1))
