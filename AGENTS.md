...
- Launch through the project-owner-supplied Rebuilt card for a minimum three seconds. Render that card first, then load and construct the complete gameplay scene beneath it; keep the card visible longer if construction exceeds the minimum rather than parsing the scene on unsafe worker threads. Then reveal the real application front door without routine loading captions; keep scenario, saved-adventure, and Character Files actions disabled until construction completes, keep Quit available, and expose a visible Retry only for an actual load failure before handing off through public shell routes.
