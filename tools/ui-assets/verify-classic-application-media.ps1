...
$manifestPath = Join-Path $repoRoot "src/ui/assets/classic-application-media.json"
...
$fontManifestPath = Join-Path $repoRoot "src/ui/assets/fonts/font-assets.json"
...
$introManifestPath = Join-Path $repoRoot "src/ui/assets/ui/intro/intro-video.json"
...
if ($null -eq $introSoundtrack -or $introSoundtrack.source_sha256 -ne "c64e2792eba92284896084c3d48af9d595fdd98cda5e7a3d90931b2a9a21f0e5" -or $introSoundtrack.license -ne "Project-Owner-Supplied" -or $introSoundtrack.path -ne "res://src/ui/assets/ui/intro/rebuilt-intro-soundtrack.mp3" -or $introSoundtrack.bytes -ne 721197 -or $introSoundtrack.duration_ms -ne 30000 -or $introSoundtrack.codec -ne "mp3" -or $introSoundtrack.bit_rate -ne 192000 -or $introSoundtrack.sample_rate -ne 48000 -or $introSoundtrack.channels -ne 2 -or -not $introSoundtrack.loop -or -not $introSoundtrack.click_toggle -or -not $introSoundtrack.independent_from_video) {
...
$chromeManifestPath = Join-Path $repoRoot "src/ui/assets/ui/spritecook-assets.json"
