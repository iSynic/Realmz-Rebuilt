...
func _test_scene_composition() -> void:
	...
	assert_equal([world_panel.size_flags_horizontal, narrative_well.size_flags_horizontal, party_panel.size_flags_horizontal, (shell.find_child("WorldCommandGrid", true, false) as GridContainer).size_flags_horizontal, (shell.find_child("CommandScroll", true, false) as ScrollContainer).size_flags_vertical, (shell.find_child("CommandGrid", true, false) as GridContainer).size_flags_horizontal], [Control.SIZE_EXPAND_FILL, Control.SIZE_EXPAND_FILL, Control.SIZE_EXPAND_FILL, Control.SIZE_SHRINK_CENTER, Control.SIZE_EXPAND_FILL, Control.SIZE_SHRINK_CENTER], "the wide footer expands all three regions while centering both command groups"); assert_true(is_equal_approx(world_panel.size_flags_stretch_ratio, party_panel.size_flags_stretch_ratio) and narrative_well.size_flags_stretch_ratio > world_panel.size_flags_stretch_ratio and shell.find_child("WorldCommandDeckFrame", true, false) is PanelContainer and shell.find_child("CommandDeckFrame", true, false) is PanelContainer, "Adventure and Party use equal framed wells while the narrative receives the wider share")
