...
func _test_startup_shell() -> void:
	...
	var intro_animation := splash.find_child("RealmzIntroAnimation", true, false) as ClassicIntroAnimation; var intro_soundtrack := splash.find_child("RealmzIntroSoundtrack", true, false) as AudioStreamPlayer; assert_true(choose_scenario != null and load_adventure != null, "the splash exposes new and saved adventures as primary paths"); assert_true(splash.find_child("SplashIdentityPanel", true, false) != null and splash.find_child("SplashCommandPanel", true, false) != null and intro_animation != null and intro_animation.stream != null and not intro_animation.autoplay and intro_animation.loop and not intro_animation.audio_enabled and is_equal_approx(intro_animation.volume_db, -80.0) and intro_soundtrack != null and not intro_soundtrack.autoplay and intro_soundtrack.stream is AudioStreamMP3 and (intro_soundtrack.stream as AudioStreamMP3).loop and is_equal_approx(intro_soundtrack.volume_db, -80.0) and splash.find_child("RealmzIntroOrnament", true, false) is NinePatchRect and splash.find_child("SplashTitle", true, false) != null and splash.find_child("SplashSubtitle", true, false) != null and (splash.find_child("SplashComposition", true, false) as BoxContainer).vertical == false, "the canonical splash activates one silent clickable video loop and one idle independent soundtrack beside one anchored command stack"); intro_animation.toggle_audio(); assert_true(intro_animation.audio_enabled and is_equal_approx(intro_animation.volume_db, -80.0) and is_equal_approx(intro_soundtrack.volume_db, 0.0), "clicking the video enables only the independent soundtrack at the current master level"); intro_animation.toggle_audio(); assert_true(not intro_animation.audio_enabled and is_equal_approx(intro_animation.volume_db, -80.0) and intro_soundtrack.stream is AudioStreamMP3 and is_equal_approx(intro_soundtrack.volume_db, -80.0), "clicking again mutes the retained soundtrack without enabling embedded video audio"); router.set_layout_profile(UiLayoutProfile.for_viewport(Vector2(800, 600), PresentationSettings.UI_SCALE_AUTO), Vector2(800, 600)); assert_true((splash.find_child("SplashComposition", true, false) as BoxContainer).vertical, "the compact splash stacks identity and commands without changing their ownership"); router.set_layout_profile(profile, Vector2(1280, 720))
...
func _test_save_preview_workspace() -> void:
	...
	assert_true(load_selected != null and current_row != null and backup_row != null and corrupt_row != null and body.find_child("SaveSelectedSlot", true, false) != null and body.find_child("SaveNewSlot", true, false) != null and body.find_child("DisplaySettingsPanel", true, false) != null and body.find_child("AudioSettingsPanel", true, false) != null and body.find_child("AccessibilitySettingsPanel", true, false) != null and body.find_child("ControlsSettingsPanel", true, false) != null and body.find_child("DiagnosticsSettingsPanel", true, false) != null and body.find_child("ClassicExplorationVisibility", true, false) is CheckButton and (body.find_child("ClassicExplorationVisibility", true, false) as CheckButton).button_pressed and [body.find_child("InterfaceScalePicker", true, false), body.find_child("TypographyPicker", true, false), body.find_child("WindowModePicker", true, false)].all(func(picker: Node) -> bool: return picker is OptionButton and (picker as OptionButton).theme_type_variation == &"ClassicTheldrowOptionButton") and _buttons_in(body).any(func(button: Button) -> bool: return button.text == "Main Menu"), "system route uses backed preference domains, supports named and selected saves beside Quick Save, gives Display pickers the Rebuilt font role, and keeps a visible Main Menu exit")
...
func _test_scene_composition() -> void:
	...
	var spatial_view := GameView.new(1, true, null); var locked_dungeon := MapView.new("dungeon:test", "Locked Dungeon", &"dungeon", 1, 1, Vector2i.ZERO, [], false, [], {}, Vector2i.ZERO, -1, 1, false, false); assert_true(PresentationCoordinator.should_show_exploration_stage(&"spells", spatial_view, true) and not PresentationCoordinator.dungeon_view_toggle_available(locked_dungeon), "field Spells preserves the live exploration renderer and a locked dungeon requires Wizard's Eye before Space exposes the overhead view"); locked_dungeon.wizard_eye_active = true; assert_true(PresentationCoordinator.dungeon_view_toggle_available(locked_dungeon), "Wizard's Eye restores Castle's overhead-view toggle inside a locked dungeon"); shell.free()
