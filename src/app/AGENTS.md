...
- The composition root owns one `HeldMovementController` shared by keyboard and mouse exploration. It submits only typed single-step movement, stops the scheduler at every host/session boundary, and persists its 25–400 percent cadence in presentation settings schema 7 without entering `.r2save` or simulation state. The same host settings boundary applies the optional traveled-area preview, default-on Auto Note behavior, and Classic/readable typography choice to presentation only.
