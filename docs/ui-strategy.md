...
- Realmz Rebuilt ships the complete pinned integrated Classic `snd ` bank as versioned application media. Scenario-owned sound keys remain immutable package content and override the same exact application `(resource type, ID)` key. Character creation still receives detached stable appearance IDs and thumbnails from validated package media; built-in PICT/CICN families migrate only after their existing package asset-ID and readiness contracts are replaced deliberately. The app-owned bitmap-control and integrated-media catalogs remain provenance-checked and distinct from scenario media.
