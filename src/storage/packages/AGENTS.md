...
- `PackageDomainAssembler` coordinates typed package construction through public decoder and validator operations. After application-first exact-ID composition with legal scenario overlays, it constructs one `RealmzContent` directory whose `ScenarioContentCatalog`, `CharacterCatalog`, `ItemCatalog`, `SpellCatalog`, `CombatCatalog`, and `EconomyContentCatalog` own feature lookups directly. `PackageContentDecoder` is the stable record-family entry point over `PackageStoryContentDecoder`, `PackageCharacterContentDecoder`, and `PackageEncounterContentDecoder`. `PackageWorldDecoder` exposes world-family `decode_*` operations, `PackageScenarioDecoder.decode_scenario` owns scenario construction, and cross-reference/media validation uses named public validation or resolution methods; package collaborators must not call one another's private helpers.
