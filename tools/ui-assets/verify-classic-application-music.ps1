...
foreach ($record in @($manifest.tracks)) {
    ...
    Assert-Condition ([string]$record.path -match '^res://src/ui/shared/assets/classic-media/music/playlist-[0-9]{2}-[a-z0-9-]+\.ogg$') "playlist $playlistId path is invalid"
...
$musicRoot = Join-Path $repoRoot "src/ui/shared/assets/classic-media/music"
