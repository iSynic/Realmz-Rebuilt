...
The hotspot/test-performance tranche adds enforceable ratchets. Handwritten production GDScript is limited to 1,000 substantive lines per file and 150 per function after the hash-bound grandfather list reaches zero. Test GDScript begins at the measured 11,807-line ceiling, may only decrease during the tranche, and exits at no more than 7,900 substantive lines, 20 percent of production, and 1,200 lines in any suite. A budget is not permission to delete unique proof: every removed duplicate must name the surviving public owner, while RNG, VM, save, topology, package, and exact-once reward evidence remain protected.

Performance probes must measure the boundary they claim. The headless movement probe owns deterministic transaction/projection latency and incremental-event classification; it does not claim frame or draw latency. MCP ordinary play owns end-to-end input, layout, draw, and perceived cadence. A fast core probe cannot close a user-visible performance report when presentation still rebuilds controls or media.
