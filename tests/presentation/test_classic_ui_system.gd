...
func _test_scene_composition() -> void:
	...
	for viewport_size: Vector2 in [Vector2(800, 600), Vector2(1280, 720)]:
		...
		assert_true(rect.position.x >= 0.0 and rect.end.x <= viewport_size.x - profile.party_width and spell_rect == Rect2(viewport_size.x - expected_spell_width, profile.menu_height, expected_spell_width, viewport_size.y - profile.menu_height) and GameShell.exploration_footer_width(viewport_size, profile, &"spells") == spell_rect.position.x and GameShell.exploration_footer_width(viewport_size, profile, &"exploration") == viewport_size.x, "field spellcasting owns the complete right edge while the footer ends at its left edge at %s" % viewport_size)
	assert_equal([GameShell.party_roster_height(720.0, 28.0, 502.0, true), GameShell.party_roster_z_index(true)], [692.0, 81], "combat spellcasting expands above the inert Party-command footer and owns the complete right rail"); assert_equal([GameShell.party_roster_height(720.0, 28.0, 502.0, false), GameShell.party_roster_z_index(false)], [502.0, 14], "closing the combat spellbook restores the ordinary six-character roster stage"); assert_equal([GameShell.combat_spellbook_roster_width(800.0, 208.0, 1.0, true), GameShell.combat_spellbook_roster_width(800.0, 208.0, 1.0, false), GameShell.combat_spellbook_roster_width(1280.0, 352.0, 1.0, true), GameShell.combat_spellbook_stage_width(592.0, 800.0, 352.0), GameShell.combat_spellbook_stage_width(928.0, 1280.0, 352.0)], [352.0, 208.0, 352.0, 448.0, 928.0], "combat spellcasting overlays enough of the compact battlefield to keep its complete selector and all targeting actions inside the viewport, then restores the ordinary roster width")
...
func _test_automatic_workflow_routes() -> void:
	...
	assert_equal(GameShell.route_change_reason(no_session), "Choose a campaign first.", "gameplay routes are disabled on the splash and campaign library")
	var setup_view := GameView.new(1, true, null)
	setup_view.party_setup_available = true
	assert_equal(GameShell.route_change_reason(setup_view), "Begin the adventure first.", "Explore and other browsing routes stay disabled until party setup commits Begin Adventure")
	...
	assert_equal(GameShell.route_change_reason(view), "Resolve the current interaction first.", "a mandatory post-battle response disables misleading route changes such as Adventure Explore")
