...
- The Journal route keeps player location notes, authored journal entries, and acquired player maps visibly separate. It may edit only the detached current-location note and may browse only the current land/dungeon stream in source ordinal order. Revert draft is presentation-owned modern wording and emits no intent; it does not claim equivalence to Castle's unresolved dialog exit labels. Historical-note topology recentering and darkness display remain unavailable until implemented from detached facts.
