...
- The schema-v2 hash changed in place because `.realmz2` remains unpublished; `.r2save` v3 is unchanged. Fumbles, attack half-units, missile toggling, full equipment side effects, and ordinary monster accuracy remain future cases rather than inferred behavior.

Next rolling priority: ordinary monster accuracy/damage. Its current 2.0 formula omits Castle's damage-plus to-hit term, defender-luck draw, day bonus, evil-conditional protection, minimum ten-percent floor, and weapon metadata. Attack half-units should follow only after their active-turn preparation state is modeled and serialized.
