...
This batch broadens contextual and owner-bound encounter callers. AOGM's zero-slot contextual RNG defect is repaired, with equal repeated entry, wrong-set repetition and exact reward/region-disable/default reentry; its immediate high-risk aggregate passes all 4212 assertions and the package, architecture, export and inventory gates. Wrath dungeon 1 AP 44 now has equal complete fourteen-input routes through actual Skills, Silver Leaf's Detect Trap and Disarm Trap, typed six-tumbler success, 1800 XP, AP replacement with XAP 927, displacement and ordinary reentry into Complex 29. Restoring the detected-trap prompt yields identical final game/RNG state; 55 focused assertions pass. Recipes now represent all 13 bundled scenarios under their recorded package/library identities, without changing certification counts. Wrath failed disarm now also has equal seven-input replays: the armed-trap warning precedes one all-party Lightning Strike, then returns to Thief actions without XP or AP replacement. Restoring that warning reproduces the same final game/RNG state and exact-once damage. Lock/reward outcomes remain next, followed by batch closeout and selection recomputation. Native CPU/dungeon performance, complete combat replay and full campaign certification remain separate; reported failures still take priority.
