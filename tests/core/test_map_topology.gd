...
func run() -> void:
	...
	assert_equal(land.topology.find_revealed_path(Vector2i(1, 1), Vector2i.ZERO, world_state, &"land", false, [Vector2i(1, 1), Vector2i.ZERO]), [Vector2i.ZERO], "revealed-only routing preserves authoritative diagonal admission")
	assert_true(land.topology.find_revealed_path(Vector2i(1, 1), Vector2i.ZERO, world_state, &"land", false, [Vector2i(1, 1)]).is_empty(), "unknown destinations cannot enter a planned route")
	...
	assert_equal(land.topology.find_revealed_path(Vector2i(7, 7), Vector2i(4, 7), world_state, &"land", false, land.topology.visible_cells(Vector2i.ONE, 8, world_state, false)), [Vector2i(6, 7), Vector2i(5, 7), Vector2i(4, 7)], "equal-cost revealed routes prefer a straight line over diagonal zigzags")
	...
	var known_corridor: Array[Vector2i] = [Vector2i.ZERO, Vector2i(1, 0), Vector2i(2, 0)]
	...
	world_state.topology.open_door(door_probe.door_id); world_state.exploration.mark_visited("dungeon:0", Vector2i(1, 0)); world_state.exploration.mark_seen("dungeon:0", Vector2i(2, 0))
	...
	assert_true(restored.topology.door_is_open(door_probe.door_id), "door overlay survives serialization")
	assert_true(restored.topology.secret_is_discovered(hidden_feature.id), "secret discovery survives serialization")
	assert_true(restored.exploration.was_visited("dungeon:0", Vector2i(1, 0)) and restored.exploration.seen_coordinates("dungeon:0") == [Vector2i(1, 0), Vector2i(2, 0)] and prior_save.exploration.seen_coordinates("dungeon:0") == [Vector2i(1, 0)], "walked and exact sight-memory coordinates remain distinct, survive deterministic serialization, and safely seed current-schema saves that predate exact sight memory")
