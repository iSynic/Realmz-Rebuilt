...
func _test_startup_shell() -> void:
	...
	var intro_animation := splash.find_child("RealmzIntroAnimation", true, false) as TextureRect; assert_not_null(choose_scenario, "the splash exposes scenario selection as a primary path"); assert_true(splash.find_child("SplashIdentityPanel", true, false) != null and splash.find_child("SplashCommandPanel", true, false) != null and intro_animation != null and splash.find_child("RealmzIntroOrnament", true, false) is NinePatchRect and splash.find_child("SplashTitle", true, false) != null and splash.find_child("SplashSubtitle", true, false) != null and (splash.find_child("SplashComposition", true, false) as BoxContainer).vertical == false, "the canonical splash separates one framed deterministic animation from one anchored command stack"); router.set_layout_profile(UiLayoutProfile.for_viewport(Vector2(800, 600), PresentationSettings.UI_SCALE_AUTO), Vector2(800, 600)); assert_true((splash.find_child("SplashComposition", true, false) as BoxContainer).vertical, "the compact splash stacks identity and commands without changing their ownership"); router.set_layout_profile(profile, Vector2(1280, 720))
