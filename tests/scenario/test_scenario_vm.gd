...
func run() -> void:
	...
	_test_public_application_transitions(content); _test_public_world_state_opcodes(content)
...
func _test_scenario_wire_contracts() -> void:
	...
	context.set_application_hook(&"shop", ""); context.set_combatant("monster.0", 0, false, true); context.set_thief_action(0, "character.0")
	var context_wire: Dictionary = JSON.parse_string(JSON.stringify(context.to_data())); var restored_context := ScenarioExecutionContext.from_data(context_wire)
...
func _test_public_classic_encounter_iterations(content: RealmzContent) -> void:
	...
	var simple := SimpleEncounterDefinition.new(0, 1, simple_responses, true, 3, 0); var keep_simple := SimpleEncounterDefinition.new(1, 1, [SimpleEncounterResponse.new("keep", "Keep", "simple:1:result:0")], false, 99, 0); var break_simple := SimpleEncounterDefinition.new(2, 1, [SimpleEncounterResponse.new("break", "Break", "simple:2:result:0")], false, 99, 0); var replace_simple := SimpleEncounterDefinition.new(3, 1, [SimpleEncounterResponse.new("replace", "Replace", "simple:3:result:0")], false, 1, 0); var complex_texts: Array[String] = ["Knock", "Set fire", "", "", "", "", "", "", ""]
	var complex := ComplexEncounterDefinition.new(0, 1, 3, 0, [0, 1, 0, 0, 0, 0, 0, 0], [], [], [], [], true, false, 2, 0, 0, 0, complex_texts)
	var programs: Array[ScenarioProgramDefinition] = [
		...
		ScenarioProgramDefinition.new("root.simple-keep", &"trigger", "simple-keep", [ClassicActionDefinition.new(0, 4, 4, 1, false, []), ClassicActionDefinition.new(1, 84, 84, 1, false, [])]), ScenarioProgramDefinition.new("simple:1:result:0", &"simple-encounter-result", "0", [ClassicActionDefinition.new(0, 39, 39, 325, false, [])]), ScenarioProgramDefinition.new("xap:325", &"extra-action-point", "325", [ClassicActionDefinition.new(0, 24, 24, 0, false, [])]), ScenarioProgramDefinition.new("root.simple-break", &"trigger", "simple-break", [ClassicActionDefinition.new(0, 4, 4, 2, false, []), ClassicActionDefinition.new(1, 84, 84, 2, false, []), ClassicActionDefinition.new(2, 24, 24, 0, false, [])]), ScenarioProgramDefinition.new("simple:2:result:0", &"simple-encounter-result", "0", [ClassicActionDefinition.new(0, 34, 34, 0, false, [])]),
	...
	var cancel_repeat := cancelled_vm.resume(InteractionResponse.from_data(cancel_first.interaction.request_id, InteractionRequest.WORD_AND_ACTION, {"action": "choice", "slots": [0]}), cancelled_api)
...
func _test_public_thief_encounter(content: RealmzContent) -> void:
	...
	var source_complex := content.complex_encounter_by_id(0); var source_thief := content.thief_encounter_by_id(source_complex.thief_success) if source_complex != null else null
...
func _test_public_vm_repeated_combat_item(content: RealmzContent) -> void:
	...
	var selected_race: RaceDefinition = null; var selected_caste: CasteDefinition = null; var usable_low := 0; var usable_high := 0; var flask_spell := content.spell_by_classic_id(4409)
	...
	var battle := BattleDefinition.new("classic.battle.vm-repeated-item", 991, base_battle.monster_slots()); var battle_action := ClassicActionDefinition.new(0, 48, 48, battle.classic_id, false, [battle.classic_id, 0, 0, 0, 0])
	...
	var item_definitions := content.item_definitions(); item_definitions.append(item); item_definitions.append(flask); var spell_definitions := content.spell_definitions()
	...
	for seed: int in range(1, 33):
		...
		if entered.state == SessionStep.State.WAITING_FOR_INTERACTION and entered.interaction != null and entered.interaction.kind == InteractionRequest.COMBAT:
			var combat_body := entered.interaction.body as InteractionRequest.CombatRequestBody
