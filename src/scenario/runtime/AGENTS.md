...
- Terminal reward ownership lasts through every source-ordered reward workspace. Initial construction and each resumed reward operation checkpoint canonical game state and gameplay RNG; failures restore both, discard speculative events, and leave the owning continuation retryable. Opcode 48's optional fixed treasure is a separately persisted second stage; combat cleanup, the after-message, and VM return occur only after that stage completes.
