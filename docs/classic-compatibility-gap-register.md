...
| `CCG-009` | Projectiles | Character missile-item random power before target selection and projectile spells with nonzero specials remain explicitly unsupported. The demonstrated zero-cost monster spell-slot signature is repaired. | That repaired normalized signature has 90 stored monster-slot references across Castle in the Clouds, Grilochs Revenge, and Prelude to Pestilence; all three package probes now report it executable. Item references and ordinary-route reachability remain unaudited. | The common monster-slot rejection is removed, but some missile or thrown-item attacks still cannot enter or complete targeting. | Replay a referenced monster through ordinary combat, then inventory item/spell pairs and implement each remaining source-backed random-power or special family. |
...
| `CCG-023` | Monster spells | Classic special 56 with target type 8 is not executable for monster casting. | One normalized signature has six stored monster-slot references in Destroy the Necronomicon; battle placement and ordinary-route reachability remain unaudited. | An affected monster can reject the spell and fall back or finish its activation. | Adjudicate special 56 through Castle source/runtime evidence, then implement it through the existing spell effect owner with exact resource, RNG, turn-cost, and save/restore proof. |
