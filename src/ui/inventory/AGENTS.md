...
- A Shop-owned browse presentation reuses the same complete workspace with visible Back to shop, Money, and Done actions bound by `InventorySceneBinding` to the scene-owned exit column. It keeps inspection and character selection active, while Trade returns to the two-pack Shop instead of mounting the separate Trade workspace; other item mutations remain disabled with the exact Shop-ownership reason. When mounted as the root workspace, presentation reuses its scene-owned children rather than nesting or clearing another workspace.
