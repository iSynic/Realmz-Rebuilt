...
- The scenario-media refresh records each prior lean package identity, exact native `Scenario.rsrc` hash, accepted application-library identity, refreshed payload count, and any represented native key that remained unrefreshable. It replaces only represented, uniquely decoded scenario-owned media whose runtime content differs; visually equivalent PNG compression and ambiguous native occurrences remain unchanged.
