...
- The front-door Realmz intro animation is app-owned licensed Classic art, not scenario media. Its source GIF remains external; the deterministic half-size sampled frames, source hash, preserved timing, license, dimensions, and per-frame hashes live in `ui/intro/intro-animation.json` and are verified offline.
...
- Original controls use nearest-neighbor filtering at exact 1x/2x only. Fonts ship with pinned source, hashes, and licenses and introduce no runtime network dependency. `Theldrow-Classic.fnt` and its atlas are deterministic, metric-preserving conversions of licensed Castle `FONT` 1601; keep that native strike monochrome, nearest-neighbor sampled, and unstretched as reference evidence. Runtime Classic text uses `Theldrow-Classic-Vector.ttf`, generated deterministically from Samuel's scalable CC0 outlines with the exact FONT 1601 horizontal advances. Black Chancery, ChicagoFLF, and Castle's Inter Geneva substitute retain their exact pinned bytes. Alegreya remains the readable-mode fallback family.
