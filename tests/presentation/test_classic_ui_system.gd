...
func _test_automatic_workflow_routes() -> void:
	...
	var queued_off := ApplicationCombatPolicy.auto_change_to_queue(PlayerIntent.set_combat_auto("hero", false), true)
	...
	assert_true(queued_off == {"characterId": "hero", "enabled": false} and ApplicationCombatPolicy.auto_change_to_queue(PlayerIntent.set_combat_auto("hero", false), false).is_empty() and ApplicationCombatPolicy.auto_abort_ids(auto_view, {"queued": true}) == ["hero", "queued"] and persistent_response != null and (persistent_response.body as InteractionResponse.CombatBody).actor_id == "hero", "the host queues individual manual control, can enumerate a full-party Escape abort, and prepares only the next Auto activation")
