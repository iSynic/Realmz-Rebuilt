...
When adding or changing a request, update its body here, the matching decoder path in `interaction_request.gd` or `interaction_request_service_decoder.gd`, its response type, and the owning gameplay and presentation tests together. Nested records decode through the feature-named common, service, combat, reward, or selection value decoder; `InteractionValueDecoderSupport` contains only shared wire-shape predicates. Keep serialized kind names, field names, versions, and optional-field behavior stable unless an explicit save migration owns the change.
