...
func run() -> void:
	...
	var committed_state := session.snapshot().to_data()
	...
	assert_equal(session.snapshot().to_data(), committed_state, "duplicate rejection is transactional")
	...
	assert_equal(session.snapshot().to_data(), committed_state, "unknown-member rejection is transactional")
