...
- Typed combat-action requests expose the battle-owned character weapon mode and the exact legal mode switch. Missile mode omits melee attacks and carries an explicit disabled ranged reason until tactical positions, range, and line of sight are session-owned.
