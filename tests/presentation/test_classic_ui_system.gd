...
func _test_classic_choice_context() -> void:
	...
	assert_true(InteractionPresenter.uses_textbox_region(encounter_request) and not InteractionPresenter.uses_application_modal_region(encounter_request) and word_entry != null and word_entry.max_length == 39 and word_entry.theme_type_variation == &"ClassicTheldrowLineEdit" and encounter.find_child("EncounterWordActions", true, false) != null and _buttons_in(encounter).any(func(button: Button) -> bool: return button.text == "Speak") and encounter.handle_back() and encounter_responses[-1] == {"action": "back"}, "Encounter Action and Speak use the narrative well under the persistent command dock while Escape invokes authored Stop after local task unwind"); encounter.free()
...
func _test_scene_composition() -> void:
	...
	var modal_parent := Control.new(); var modal_presenter := load("res://src/presentation/interaction_presenter.tscn").instantiate() as InteractionPresenter; modal_parent.add_child(modal_presenter); var textbox_overlay_style := InteractionPresenter.textbox_theme_variation(); var combat_footer_style := InteractionPresenter.textbox_theme_variation(ClassicUiFixtureGallery.request_for(InteractionRequest.COMBAT)); modal_presenter.call("_update_modal_shield", true); var modal_shield := modal_parent.get_node("LockedModalShield") as Control; var first_order := [modal_shield.get_index(), modal_presenter.get_index()]; modal_presenter.call("_update_modal_shield", true); var repeated_order := [modal_shield.get_index(), modal_presenter.get_index()]; var floating_choice_rect := InteractionPresenter.floating_choice_rect(Rect2(0.0, 28.0, 992.0, 502.0), Rect2(286.0, 530.0, 620.0, 190.0), Vector2(286.0, 46.0)); var compact_choice_rect := InteractionPresenter.floating_choice_rect(Rect2(0.0, 28.0, 592.0, 410.0), Rect2(10.0, 438.0, 780.0, 162.0), Vector2(286.0, 46.0)); assert_true(textbox_overlay_style == &"ClassicTextboxOverlay" and combat_footer_style == &"ClassicOpenRight" and first_order[0] < first_order[1] and repeated_order[0] < repeated_order[1] and repeated_order == first_order and InteractionPresenter.uses_floating_choice_modal(InteractionRequest.yes_no("floating-choice", "Do you follow him?", "Yes", "No")) and floating_choice_rect.end.y <= 522.0 and floating_choice_rect.size == Vector2(520.0, 116.0) and compact_choice_rect.end.x <= 582.0 and compact_choice_rect.end.y <= 430.0, "narrative interactions use one opaque textbox surface, combat exposes the textured command slate, locked modals retain pointer order, and AP choices stay together above the narrative region in both supported profiles"); var flash_sounds: Array[int] = []; modal_presenter.presentation_sound_requested.connect(func(sound_id: int) -> void: flash_sounds.append(sound_id)); modal_presenter.queue_classic_flash_messages([{"text": "Map one", "soundId": 30005}, {"text": "Map two", "soundId": 30005}, {"text": "Map three", "soundId": 30005}]); var flash_panel := modal_parent.find_child("ClassicFlashMessage", true, false) as PanelContainer; var flash_continue := modal_parent.find_child("ClassicFlashContinue", true, false) as Button; flash_continue.pressed.emit(); var second_flash := modal_parent.find_child("ClassicFlashMessage", true, false) as PanelContainer; var second_text := (modal_parent.find_child("ClassicFlashText", true, false) as Label).text; flash_continue.pressed.emit(); var third_flash := modal_parent.find_child("ClassicFlashMessage", true, false) as PanelContainer; assert_true(flash_panel == second_flash and second_flash == third_flash and third_flash.size == Vector2(520.0, 118.0) and second_text == "Map two" and (modal_parent.find_child("ClassicFlashText", true, false) as Label).text == "Map three" and flash_sounds == [30005, 30005, 30005], "source-owned flash messages reuse one compact ordered click boundary and play their cue once per displayed record"); modal_parent.free()
