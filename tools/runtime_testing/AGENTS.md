...
- Fixture-only `battle-victory` invocation requires an active combat fixture and the exact expected Classic Battle ID, then uses the existing debug victory transaction so ordinary rewards, allies, after-battle behavior, and VM continuation remain the owning paths. It is forced-victory continuation evidence and never ordinary-combat proof; live sessions remain rejected.
...
- Rebuilt readiness reports committed, presented, presentation-drawn, deferred, and current playback-frame identities separately from the transport-level `drawnRevision`; diagnostic consumers must not compare those two revision domains as if they were interchangeable.
