...
Inventory and Character are the reference conversion. Their scenes expose named stable regions that survive a rerender; the small paired screen scripts provide typed access to those nodes, and `InventoryScreenController` and `CharacterScreenController` repopulate only their changing contents. Allies and Bestiary share the same scene-authored list-and-detail pattern through `CreatureLibraryScreen`. Maps/Notes, Money and services, Spells, System, and Character Files follow the same division. A maintainer changes the durable arrangement in the scene, the screen's local behavior in its paired script, and data-to-control binding in the named controller. Exploration and Combat are honest route markers: their visible structure already belongs to `game_shell.tscn`, so their route files do not contain duplicate dummy controls.
