...
func run() -> void:
	...
	assert_equal(loaded.content.caste_by_id("classic.caste.1").attributes.maximum_damage_bonus(), 5, "caste strength caps retain their source field meaning")
	assert_true(loaded.content.caste_by_id("classic.caste.1").progression.initial_ability_value(13) is int and loaded.content.caste_by_id("classic.caste.1").progression.level_ability_die(13) is int and loaded.content.caste_by_id("classic.caste.1").progression.victory_threshold(29) is int, "caste abilities and all thirty victory thresholds cross the validating package boundary")
