...
func _test_startup_shell() -> void:
	...
	assert_not_null(choose_scenario, "the splash exposes scenario selection as a primary path"); assert_true(splash.find_child("SplashIdentityPanel", true, false) != null and splash.find_child("SplashCommandPanel", true, false) != null and splash.find_child("RealmzIntroAnimation", true, false) != null and splash.find_child("RealmzIntroOrnament", true, false) != null and splash.find_child("SplashTitle", true, false) != null and splash.find_child("SplashSubtitle", true, false) != null and (splash.find_child("SplashComposition", true, false) as BoxContainer).vertical == false, "the canonical splash separates one framed animated application identity from one anchored command stack"); router.set_layout_profile(UiLayoutProfile.for_viewport(Vector2(800, 600), PresentationSettings.UI_SCALE_AUTO), Vector2(800, 600)); assert_true((splash.find_child("SplashComposition", true, false) as BoxContainer).vertical, "the compact splash stacks identity and commands without changing their ownership"); router.set_layout_profile(profile, Vector2(1280, 720))
...
func _test_fixture_gallery_coverage() -> void:
	...
	assert_true(spell_component.find_child("LevelSpellList", true, false).item_count == 4, "the level spell stage renders the complete detached candidate list")
...
func _test_character_creator_workflow() -> void:
	...
	view.race_options = [DefinitionOptionView.new("race.human", "Human", "Adaptable.", ["caste.sorcerer"]), DefinitionOptionView.new("race.2", "Race 2")]
	view.caste_options = [DefinitionOptionView.new("caste.sorcerer", "Sorcerer", "Arcane caster.", ["race.human"]), DefinitionOptionView.new("caste.2", "Caste 2")]
	...
	assert_equal(setup.setup_mode, &"creator", "creator opens from party assembly"); assert_true(not setup.party_pane.visible and setup.creator_page.find_child("IdentityPreview", true, false) != null and setup.creator_page.find_child("IdentityFields", true, false) != null and setup.creator_page.find_child("IdentityCampaignContext", true, false) != null, "character creation owns the full setup stage without the assembly roster")
	prior_character_list.free(); setup.name_edit.text = "Ari"; setup.creator_next_button.pressed.emit(); assert_true(setup.creator_page.find_child("RaceSelectorPanel", true, false) != null and setup.creator_page.find_child("CasteSelectorPanel", true, false) != null and setup.race_list.item_count == 1 and setup.caste_list.item_count == 1 and (setup.creator_page.find_child("RaceDescription", true, false) as Label).text == "Adaptable." and (setup.creator_page.find_child("CasteDescription", true, false) as Label).text == "Arcane caster.", "Race and Caste retain equal backed selectors while literal fallback records stay out of ordinary creation")
	setup.creator_step = 2; setup.render_creator_step(); assert_true(setup.creator_page.find_child("AppearancePreview", true, false) != null and setup.creator_page.find_child("PortraitThumbnailStrip", true, false) != null and setup.creator_page.find_child("CombatIconThumbnailStrip", true, false) != null and setup.creator_page.find_child("PortraitChoice_257", true, false) != null and setup.creator_page.find_child("CombatIconChoice_9000", true, false) != null, "Appearance pairs large exact-media previews with recommended-first portrait and independently selectable tactical thumbnail strips")
	...
	setup.creator_cancel_button.pressed.emit(); assert_equal(setup.setup_mode, &"assembly", "creator cancellation rebuilds assembly after its prior dynamic controls are freed"); var spell_state := CharacterState.new("creator.spells", "Ari", 12, 12); spell_state.spellcaster_type = 1; view.character_draft = CharacterView.new(spell_state); view.character_draft_spell_points_total = 3; view.character_draft_spell_points_remaining = 2; view.character_draft_spell_options = [CharacterSpellOptionView.new(SpellDefinition.new("classic.spell.1101", 1101, "Discover Magic", "Reveals magic."), 1, true), CharacterSpellOptionView.new(SpellDefinition.new("classic.spell.1102", 1102, "Flame Hands", "Calls flame."), 2, false)]; setup.setup_mode = &"creator"; setup.creator_step = 4; setup.render_creator_step(); var spell_buttons := _buttons_in(setup.creator_page); var selected_spells := spell_buttons.filter(func(button: Button) -> bool: return button.text.begins_with("Discover Magic")); var available_spells := spell_buttons.filter(func(button: Button) -> bool: return button.text.begins_with("Flame Hands")); var selected_spell := selected_spells[0] as Button if not selected_spells.is_empty() else null; var available_spell := available_spells[0] as Button if not available_spells.is_empty() else null; assert_true(setup.creator_page.find_child("StartingSpellLevelRail", true, false) != null and setup.creator_page.find_child("StartingSpellListPanel", true, false) != null and setup.creator_page.find_child("StartingSpellRecord", true, false) != null and setup.creator_page.find_child("StartingSpellAllowance", true, false) != null and selected_spell != null and selected_spell.toggle_mode and selected_spell.button_pressed and available_spell != null, "Starting Spells reuses the spellbook level/list/detail hierarchy and exposes ordinary pressed toggle state"); available_spell.pressed.emit(); var spell_payload := creator_intents[-1].payload as PlayerIntent.StringListPayload; assert_equal(spell_payload.values, ["classic.spell.1101", "classic.spell.1102"], "ordinary spell toggles preserve selections on the current and hidden levels without a modifier key")
	router.free()
