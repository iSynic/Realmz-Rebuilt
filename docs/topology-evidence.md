...
- `src/realmz_orig/centerpict.c:198-238` places a 32×32 CICN over the party's land cell. Ordinary foot travel selects CICN `175 + 11 * face`, yielding 175 or 186; notes, boats, camp, and horseback states select different resources. Realmz 2 uses the preserved CICN 186 foot-party bitmap as its current ordinary land marker instead of fabricating a geometric indicator; unsupported alternate travel states remain explicit gaps.
...
- `ClassicMapPresenter` consumes only `GameView`; its clipped, party-centered native 32-pixel atlas view, preserved native-size foot-party CICN, minimap, movement cues, and opt-in debug facts cannot become simulation authority. The geometric party ring is a missing-asset fallback only. Normal play does not draw fabricated land-edge walls, AP diamonds, random-region boxes, or cell grids over Classic art.
