...
- The composition root owns one `HeldMovementController` shared by keyboard and mouse exploration. It submits one immediate typed single-step movement, requires a 180-millisecond hold before repetition begins, then uses the persisted 25–400 percent cadence from ten scheduled steps per second at 100 percent without entering `.r2save` or simulation state. It stops the scheduler at every host/session boundary. The same host settings boundary applies the optional traveled-area preview, default-on Auto Note behavior, Classic/readable typography choice, separate effects/music volumes, global music enablement, and twenty playlist modes to presentation only.
