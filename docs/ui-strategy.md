...
- Classic typography is the default. Body, narrative, dialog, and menu text use Samuel's scalable CC0 Theldrow outlines with horizontal advances deterministically replaced by the exact Castle `FONT` 1601 metrics. The original bitmap strike remains native-size evidence rather than being stretched across UI scales. Castle's Black Chancery supplies ornamental headings and compact command labels; Castle's open Inter substitute supplies Geneva-style utility controls; and ChicagoFLF remains limited to source-backed 3D help text. Preferences offers a readable modern mode using Alegreya/Alegreya Sans. Pinned commits, exact hashes, dual-source conversion provenance, and licenses live under `src/presentation/assets/fonts/`; the runtime makes no network request.
