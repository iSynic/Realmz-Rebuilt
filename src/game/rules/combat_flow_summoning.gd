## Implements deterministic combat flow summoning rules without presentation dependencies.
...
func probe_choice(state: GameState, content: RealmzContent, caster_id: String, spell: SpellDefinition, power_level: int) -> CombatSpellCastProbe:
	...
	for y: int in range(bounds.position.y, bounds.end.y):
		for x: int in range(bounds.position.x, bounds.end.x):
			...
			if _context.battlefield.coordinate_target_is_valid(state.combat.battlefield, terrain_set, caster_id, coordinate, maximum_range, require_line_of_sight) and _context.battlefield.monster_footprint_is_open(state.combat.battlefield, terrain_set, coordinate, 0):
...
func probe_coordinates(state: GameState, content: RealmzContent, caster_id: String, spell: SpellDefinition, power_level: int, target_coordinates: Array[Vector2i]) -> CombatSpellCastProbe:
	...
	for coordinate: Vector2i in target_coordinates:
		...
		if not _context.battlefield.coordinate_target_is_valid(state.combat.battlefield, terrain_set, caster_id, coordinate, maximum_range, require_line_of_sight):
		...
		if not _context.battlefield.monster_footprint_is_open(state.combat.battlefield, terrain_set, coordinate, 0):
...
func cast_character_summon(state: GameState, content: RealmzContent, caster: CharacterState, spell: SpellDefinition, power_level: int, rng: RealmzRng, target_coordinates: Array[Vector2i], event_source: String = "classic", spend_spell_points: bool = true, count_spell_cast: bool = true) -> CombatFlowResult:
	...
	for index: int in target_coordinates.size():
		...
		var summoned := _context.monsters.build_monster(definition, instance_id, 1 if caster.traitor else 0, state.difficulty, state.clock.day(), rng)
		if summoned == null:
		...
		CombatSpellEventBuilder.append_projectile(events, caster.id, summoned.id, spell, event_source)
...
func cast_monster_summon(state: GameState, content: RealmzContent, caster: MonsterState, spell: SpellDefinition, power_level: int, rng: RealmzRng, target_coordinates: Array[Vector2i]) -> CombatFlowResult:
	...
	for index: int in target_coordinates.size():
		...
		state.combat.append_turn_actor(summoned.id)
		summoned_ids.append(summoned.id)
		CombatSpellEventBuilder.append_projectile(events, caster.id, summoned.id, spell, "classic-monster")
...
func _automatic_coordinate(state: GameState, content: RealmzContent, caster_id: String, caster_traitor: bool, spell: SpellDefinition, power_level: int) -> Vector2i:
	...
	for y: int in range(bounds.position.y, bounds.end.y):
		for x: int in range(bounds.position.x, bounds.end.x):
			var coordinate := Vector2i(x, y)
			if _context.battlefield.coordinate_target_is_valid(state.combat.battlefield, terrain_set, caster_id, coordinate, maximum_range, require_line_of_sight) and _context.battlefield.monster_footprint_is_open(state.combat.battlefield, terrain_set, coordinate, 3):
