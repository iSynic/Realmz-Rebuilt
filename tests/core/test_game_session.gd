...
func _test_interaction_value_decoder_contracts() -> void:
	...
	var inventory_item := {"instanceId": "item.instance", "itemId": "classic.item.1", "name": "Dagger", "identified": true, "equipped": false, "charges": 0, "sellPrice": 4, "canSell": true, "sellReason": "", "canIdentify": false, "identifyReason": "Known"}
	...
	var reward_item := {"instanceId": "reward.item", "definitionId": "classic.item.1", "name": "Dagger", "charges": 0, "identified": true, "description": "", "facts": [fact]}
	...
	assert_true(valid_values.all(func(value: Variant) -> bool: return value != null), "every representative nested request value decodes through the strict current wire contract")
	assert_true([InteractionCommonValueDecoder.availability({"enabled": true, "reason": "", "unknown": 1}), InteractionServiceValueDecoder.service_character({"id": "hero", "name": "Hero"}, &"unknown"), InteractionRewardValueDecoder.reward_character(ordinary_reward_character, &"unknown"), InteractionSelectionValueDecoder.encounter_action({"id": "x", "kind": "unknown", "label": "Unknown"})].all(func(value: Variant) -> bool: return value == null), "shared, service, reward, and encounter values reject unknown fields, modes, or discriminators")
