...
| `CCG-010` | Monster macros | Immediate area and side-group macro spells now resolve through the shared spell owners, including damage, healing, conditions, spell-point changes, allegiance, transformation, defeat cleanup, and nested death-macro queuing. Queued fields and open-space/summoning signatures remain separate unsupported families. | No opcode-17 row is directly present in a referenced battle/death macro among the 13 bundled packages; transitive calls and third-party exposure remain unaudited. | The demonstrated immediate-family failure is removed, but a macro using a queued field or open-space spell still fails explicitly. | Audit transitive and third-party macro calls, then implement only demonstrated queued/open-space signatures while preserving retained-source geometry and nested continuation. |
