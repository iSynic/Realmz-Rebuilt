...
- `verify-classic-application-media.ps1` verifies unique IDs/resource keys, all six stock landlook atlases, the expected integrated sound, shared/stock-supply item-CIcon, and combat-CIcon counts and required stock-supply identities, the project-owner-supplied launch card plus intro-video and independent-soundtrack provenance/playback contracts, SpriteCook and ImageGen chrome plus status/command-glyph provenance, paths, dimensions, and every committed byte count and SHA-256 without requiring an external Castle checkout.
