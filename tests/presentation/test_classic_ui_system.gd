...
func _test_field_spell_workspace() -> void:
	...
	var cast := body.find_child("SpellCastAction", true, false) as BaseButton; var level_one := body.find_child("SpellLevel1", true, false) as Button; assert_true(cast != null and body.find_child("SpellCharacterSelector", true, false) != null and body.find_child("SpellLevelRail", true, false) != null and body.find_child("KnownSpellList", true, false).size_flags_stretch_ratio == 2.0 and body.find_child("SelectedSpellRecord", true, false).size_flags_vertical == Control.SIZE_FILL and level_one.text == "Level 1", "the field spellbook gives its list priority over a compact record while retaining caster, Castle-labelled level rail, and fixed Cast controls"); cast.pressed.emit()
