...
func save(content: RealmzContent, slot_id: String) -> bool:
	if content == null:
		_shell.set_status("Save failed • no package loaded", true)
		return false
	var saved := _repository_host.save(content, slot_id, _session.session().snapshot())
	_shell.set_status("Saved %s" % slot_id if saved else "Save failed • %s" % _repository_host.last_error(), not saved)
	if saved:
		refresh(content)
		_shell.show_activity_indicator(&"save")
	return saved


func load(content: RealmzContent, slot_id: String, backup: bool = false) -> SessionStep:
	if content == null:
		_shell.set_status("Load failed • no package loaded", true)
	...
	if envelope == null:
		_shell.set_status("Load failed • %s" % _repository_host.last_error(), true)
	...
	_shell.set_status(status, step.state == SessionStep.State.FAILED)
	return step


func refresh(content: RealmzContent) -> void:
	_shell.set_save_previews(_repository_host.previews(content))
