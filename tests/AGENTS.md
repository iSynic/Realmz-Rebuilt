...
- The public exploration workflow also owns Search-mode versus Area-Search timing, field Heal time/SP/RNG/recovery and save restoration, plus contextual Encounter draw/order/continuation. The public inventory workflow owns exact item-805 Torch use. Presentation tests retain only command grouping, workspace structure, roster-mediated Trade, and responsive reachability.
