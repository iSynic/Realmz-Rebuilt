...
- World-canvas zoom scales the retained 2D map as one presenter so discovery, darkness, diagnostics, guard cells, and pointer movement share its local 32-pixel geometry. The first-person dungeon keeps its 400x225 camera and integer-fit field of view; optional canvas smoothing filters only its already-enlarged composed viewport.
