...
- The committed Classic application library is a Providence-built application package with a pinned campaign/package identity and adjacent provenance record. It contains stock Race, Caste, spell, appearance, item, and application media records; it contains no commercial scenario payload. Release exports include it explicitly. Runtime startup trusts its shipped immutable bytes after matching the pinned manifest identity, while external campaign packages retain the ordinary untrusted import/install path.
