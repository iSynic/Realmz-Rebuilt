...
- Developer preview commands enter exact Simple/Complex Encounter, standalone Extra Action Point program, Battle, Treasure, and Shop definitions through their ordinary scenario, combat, reward, and service interactions. Standalone Extra AP entry preserves the package owner identity but supplies only the current isolated exploration location; it does not claim Timed, hook, item, random-rectangle, or other caller semantics. Direct Shop preview uses unrestricted acceptance ranges because contextual ranges remain caller-owned; all resumable preview operations suppress snapshots until their final interaction closes.
...
- A pending Shop admits exact-instance Trade between party members through its own typed response. The shared inventory workflow validates custody and destination, and the same Shop continuation refreshes without closing or repeating a purchase.
