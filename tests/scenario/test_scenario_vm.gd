...
func _test_public_application_transitions(content: RealmzContent) -> void:
	...
	var map := content.world.player_map_by_classic_id(1); var shop := content.shop_by_id("classic.shop.0"); var shop_offer := api.execute_classic(ClassicActionDefinition.new(0, 6, 6, 0, false, []), "shop.offer"); var shop_result := api.request_available_shop("shop.open"); var shop_data := shop_result.interaction.body.to_data(); var unpaid := api.execute_classic(ClassicActionDefinition.new(0, 33, 33, 0, false, [5000, 2, 6, 0, 0]), "wealth.unpaid"); assert_equal([shop_offer.state, state.active_shop_id, shop_offer.events[0].kind, shop_offer.events[1].kind, shop_offer.events[1].payload.get("soundId"), shop_result.state, shop_data["stock"][0]["stockKey"], shop_data["stock"][0]["index"], shop_data["stock"][0]["category"], shop_data["stock"][0].has("description"), shop_data["characters"][0]["portraitId"], shop_data["characters"][0]["load"], unpaid.value, unpaid.events[1].payload], [ScenarioRuntimeOperationResult.State.COMPLETED, shop.id, &"shop_available", &"sound_requested", 30005, ScenarioRuntimeOperationResult.State.WAITING, "base:817", 817, "supplies", true, shopper.portrait_id, 0, false, {"text": "The party does not have enough gold.", "soundId": 6000, "source": "classic-opcode-33"}], "Shop exposes its contextual source cue and sparse stock facts while failed opcode 33 requests Castle warning 50 with sound 6000"); var first_stock := content.item_by_id(shop.item_ids()[0]); var saved_icon_id := first_stock.icon_id; first_stock.icon_id = 0; var iconless_shop := api.request_available_shop("shop.iconless"); first_stock.icon_id = saved_icon_id; assert_true(iconless_shop.state == ScenarioRuntimeOperationResult.State.WAITING and not iconless_shop.interaction.body.to_data()["stock"][0].has("iconId"), "a source-valid shop item without a CICN remains an actionable typed row instead of collapsing the interaction")
