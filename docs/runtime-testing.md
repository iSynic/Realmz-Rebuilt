...
Griloch Temple AP 35 additionally proves contextual Bank/Temple availability and Keep Codes through ordinary entry. Actual Pool collects the six starters' 760 gold; Leave Temple returns it to the bank exactly once without time/RNG. Restoring the pooled Temple preserves that exit contract, and walking away clears services without duplicating wealth. Paid services and execution of authored application hooks remain unclaimed. The next selected candidate is Destroy the Necronomicon AP 3's Choice-to-forced-Simple reward route, prioritizing a new response/reward context over another word variant.

This four-workflow batch adds no testing capabilities. Captured complete replays take 5.44 seconds for nested Back, 7.42 seconds for Clouds, 3.31 seconds for Mithril and 2.80 seconds for Temple; saved-state suffixes are measured separately in their evidence jobs. Fixture preparation, source adjudication, recipe collation and aggregate verification are setup costs, not replay time.
