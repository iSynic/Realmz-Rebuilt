...
- `DevelopmentPreviewRequest` owns the strict developer-only request vocabulary shared with Providence. `DevelopmentPreviewSession` validates the temporary package through the ordinary application-plus-scenario boundary, creates the deterministic in-memory party, and enters an Action Point, Simple Encounter, map location, exact scenario-owned scrolling `TEXT`, or exact Classic Battle target. A Battle is ready only after the ordinary debug command produces both an active detached combat surface and its typed combat action request. `DevelopmentPreviewResultWriter` owns the strict result envelope. Export-excluded tools provide the headless and interactive hosts; public release startup never selects them.
