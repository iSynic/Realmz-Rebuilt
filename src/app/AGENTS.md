...
- Debug builds alone bind F12 to `DebugToolsHost`. The host exposes the presentation-only floating tool surface, retains unsaved no-clip state and a bounded recent history translated from committed Auto events, and translates every mutation into a typed public `SessionDebugCommand`; release UI, saves, and ordinary gameplay intents never carry these controls, logs, or flags.
