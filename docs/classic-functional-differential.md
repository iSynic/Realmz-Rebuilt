...
The rule, clock, spell, complete monster-special attack table, battle-allegiance, battle-reward, persistence, `showageupdate` click boundaries, and the source-settled character melee resolver are covered. Battle placement distance no longer contaminates melee rules; magical-plus, blunt, sharp, and corrected specific-item gates use distinct typed monster fields. Critical ability values, fumble preference/recovery, attack-point cadence, missile toggling, and full equipment side effects remain separate unresolved boundaries; preserving Castle's two critical draws does not pretend those missing ability values are implemented. The next rolling combat slice should correct the ordinary monster hit formula or introduce the missing combat/package state before enabling one of those branches.
