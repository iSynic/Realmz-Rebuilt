...
- Player Auto and NPC combat decision-making may deliberately improve on Castle, including movement routing, action and weapon selection, spell choice, and ally-safe targeting. Keep every improvement deterministic, rules-legal, presentation-independent, and explicitly documented rather than treating Castle's weak AI as a fixed fidelity requirement. Persistent Auto must yield after each character activation so the player can return any roster member to manual control during an ongoing battle; Escape is the host safety hatch that disables every party Auto toggle and resumes manual control at the next activation boundary.
