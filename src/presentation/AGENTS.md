...
- The Services route owns the ordinary money workspace. It renders detached pooled, banked, and personal denominations, character load, Pool/Share, and exact Swap increments; every action uses the core-provided availability and emits one `MONEY_ACTION`. Character selection and Done are presentation-only state and never mutate wealth.
- The Spells route renders only core-provided field-cast, Make Scroll, and scroll-slot Use actions. It emits the existing typed spell intent with stable character/spell/slot/power identities and never calculates costs, context eligibility, targets, or scroll consumption. The exploration command reflects session state as Camp or Break Camp; unavailable held Rest and automatic departure are not simulated locally.
