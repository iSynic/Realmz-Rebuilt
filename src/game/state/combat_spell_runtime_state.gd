...
func _valid_new_field(spell_id: String, caster_id: String, center: Vector2i, rotation: int, shape: int, queue_icon: int, power_level: int, cast_level: int, duration: int) -> bool:
	return _turns != null and can_queue_persistent_field() and not spell_id.is_empty() and not caster_id.is_empty() and BattlefieldState.contains(center) and rotation >= 0 and rotation <= 3 and shape >= 1 and shape <= 127 and queue_icon != 0 and queue_icon >= -128 and queue_icon <= 127 and power_level >= 1 and power_level <= 7 and cast_level >= 0 and cast_level <= 7 and duration >= 1 and duration <= 32_767 and not _turns.turn_order().is_empty() and _turns.turn_index >= 0 and _turns.turn_index < _turns.turn_order().size()
