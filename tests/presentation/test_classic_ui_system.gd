...
func _test_shop_component() -> void:
	...
	var ledgers := component.find_child("ShopExchangeLedgers", true, false); var inventory_scroll := component.find_child("InventoryScroll", true, false) as ScrollContainer; var stock_scroll := component.find_child("ShopStockScroll", true, false) as ScrollContainer; assert_true(component.find_child("ShopHeader", true, false) != null and ledgers != null and component.find_child("SelectedInventoryColumn", true, false).get_index() < component.find_child("ShopExchangeDivider", true, false).get_index() and component.find_child("ShopExchangeDivider", true, false).get_index() < component.find_child("ShopStockColumn", true, false).get_index() and component.find_child("ShopControlSpine", true, false) != null and component.find_child("ShopControlStrip", true, false).custom_minimum_size.y == 76.0 and component.find_child("ShopDetailStrip", true, false).custom_minimum_size.y == 82.0 and ["ShopLeftLoadPanel", "ShopSelectedShopperPanel", "ShopTransactionContainer", "ShopRouteControls", "ShopRightLoadPanel", "ShopKeeperRestore", "ShopItems", "ShopMoney", "ShopDone", "ShopItemDescription", "ShopItemStats"].all(func(name: String) -> bool: return component.find_child(name, true, false) != null) and [["ShopKeeperRestore", &"command.shop_original"], ["ShopItems", &"command.inventory"], ["ShopMoney", &"command.money"]].all(func(spec: Array) -> bool: var button := component.find_child(spec[0], true, false) as ClassicBitmapButton; return button != null and button.has_visual_art() and button.visual_asset_id == spec[1]) and component.find_children("ShopBuyer_*", "Button", true, false).size() == 1 and component.find_children("ShopSeller_*", "Button", true, false).size() == 1 and [inventory_scroll.custom_minimum_size.y, stock_scroll.custom_minimum_size.y] == [322.0, 322.0] and component.find_children("*", "ScrollContainer", true, false).size() == 2 and component.size_flags_vertical == Control.SIZE_SHRINK_BEGIN and component.get_combined_minimum_size().y <= 600.0 and InteractionPresenter.interaction_vertical_scroll_mode(request) == ScrollContainer.SCROLL_MODE_DISABLED, "shop keeps the adventurer ledger left and stock right around the source-shaped category spine, framed lower controls, source-art routes, two shopper identities, fixed control/detail strips, and nine rows while only its ledgers scroll"); var compact_shop := ShopInteraction.new(); compact_shop.configure(ClassicMediaCatalog.new(null, ApplicationMediaCatalog.new()), true); compact_shop.build(request); assert_true([(compact_shop.find_child("InventoryScroll", true, false) as ScrollContainer).custom_minimum_size.y, (compact_shop.find_child("ShopStockScroll", true, false) as ScrollContainer).custom_minimum_size.y] == [286.0, 286.0] and compact_shop.get_combined_minimum_size().y <= 560.0, "the optional compact Shop preserves nine source rows and both lower strips without application-page scrolling"); compact_shop.free(); assert_true(component.find_child("StockIcon_base_0", true, false).find_child("ContentImage", true, false) != null and component.find_child("InventoryIcon_item_unknown", true, false).find_child("ContentImage", true, false) != null and ["weapons", "armor", "limb_armor", "magic", "supplies"].all(func(id: String) -> bool: var button := component.find_child("ShopFilter_%s" % id, true, false) as ClassicBitmapButton; return button != null and button.has_visual_art() and button.visual_asset_id == StringName("inventory.category.%s" % id)) and [component.find_child("Stock_base_0", true, false), component.find_child("Inventory_item_unknown", true, false)].all(func(button: Button) -> bool: return button.theme_type_variation == &"ClassicItemLedgerButton" and not button.button_pressed), "shop rows stay unselected on the white ledger while all five divider filters bind their native application art")
...
func _test_inventory_workspace() -> void:
	...
	var inventory_popover := body.find_child("ClassicItemDetailPopover", true, false); inventory_popover.set("modifier_active", true); (body.find_child("InventoryItem_inventory_item", true, false) as Control).mouse_entered.emit(); assert_true(["CastleInventoryMainSplit", "InventoryItemBrowser", "InventoryCharacterCommandRail", "InventoryCharacterFacts", "InventoryActionDock", "InventoryCharacterSelector", "InventoryItemInspector", "InventorySelectedItemRecord"].all(func(node_name: String) -> bool: return body.find_child(node_name, true, false) != null) and (inventory_popover.find_child("ClassicItemDetailPanel", true, false) as PanelContainer).visible and (inventory_popover.find_child("ClassicItemDetailTitle", true, false) as Label).text == "Longsword" and (inventory_popover.find_child("ClassicItemDetailDescription", true, false) as Label).text == "A balanced sword.", "inventory follows Castle's hierarchy while Alt-hover exposes only its detached item record in the shared floating pane"); assert_true(buttons.any(func(button: BaseButton) -> bool: return button.tooltip_text == "Equip"), "inventory exposes the typed Equip action"); assert_true(body.find_children("ContentImage", "TextureRect", true, false).size() == 3 and body.find_children("ContentImage", "TextureRect", true, false).all(func(image: TextureRect) -> bool: return image.custom_minimum_size.x > 0.0) and body.find_children("ContentImageUnavailable", "Label", true, false).is_empty(), "inventory keeps each exact item image visibly sized in the list, record, and transient detail without exposing a variable-width resource ID")
