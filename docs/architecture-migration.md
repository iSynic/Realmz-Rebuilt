...
- Character creation now exports `character_creation_appearance_step.tscn` from the retained party-setup workspace. Its complete preview, portrait and battle-icon catalogs, paging, empty/error states, and six stable thumbnail positions per variable row are editable; `PartySetupAppearanceEditor` constructs no Controls, lowering runtime-created Controls to 75, cross-object private calls to 193, and generic preload aliases to 128.
...
- Large session, combat, application, interaction, and shell scripts were divided once, but 23 files still exceed the final 600-line limit, 90 functions exceed 60 lines, and 35 top-level scripts exceed the final method-count limits.
