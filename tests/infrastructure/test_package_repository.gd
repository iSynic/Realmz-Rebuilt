...
func run() -> void:
	...
	if fixture_archive.file_exists("manifest.json"):
		var manifest: Dictionary = JSON.parse_string(fixture_archive.read_file("manifest.json").get_string_from_utf8()); var malformed_content: Dictionary = JSON.parse_string(fixture_archive.read_file("content.json").get_string_from_utf8()); var world: Dictionary = JSON.parse_string(fixture_archive.read_file("world.json").get_string_from_utf8()); var scenario: Dictionary = JSON.parse_string(fixture_archive.read_file("scenario.json").get_string_from_utf8()); var boundary_content := malformed_content.duplicate(true); boundary_content["messages"].append({"id": 0, "text": ""}); boundary_content["messages"].append({"id": 2999, "text": "A".repeat(255)}); var boundary := PackageDomainAssembler.new().assemble(manifest, boundary_content, world, scenario, loaded.media.assets()); var malformed_encounter: Dictionary = malformed_content["complexEncounters"][0]; malformed_encounter["promptMessageId"] = 628; var sentinel_item: Dictionary = malformed_content["items"][0]; sentinel_item["specificCasteId"] = "classic.caste.-32768"; var normalized := PackageDomainAssembler.new().assemble(manifest, malformed_content, world, scenario, loaded.media.assets()); var reversed_world: Dictionary = world.duplicate(true); reversed_world["maps"][0]["randomRectangles"][1]["battleRange"] = [1, -1]; var reversed_range := PackageDomainAssembler.new().assemble(manifest, malformed_content, reversed_world, scenario, loaded.media.assets()); var invalid_branch_scenario: Dictionary = scenario.duplicate(true); invalid_branch_scenario["programs"].append({"id": "xap:9876", "ownerKind": "extra-action-point", "ownerId": "9876", "instructions": [{"kind": "classicAction", "slot": 0, "rawOpcode": 85, "opcode": 85, "id": 0, "gosub": false, "extraCode": [1, 0, 1, 0, 0]}]}); var invalid_branch := PackageDomainAssembler.new().assemble(manifest, malformed_content, world, invalid_branch_scenario, loaded.media.assets()); assert_equal([boundary.message_by_id(0).text if boundary != null else "assembly-failed", boundary.message_by_id(2999).text.length() if boundary != null else -1, loaded.content.message_by_id(628), normalized.message_by_id(628).text if normalized != null else "assembly-failed", normalized.item_definitions()[0].specific_caste_id if normalized != null else "assembly-failed", reversed_range.world.map_by_id("land:0").random_region_by_index(1).battle_maximum if reversed_range != null else 0, invalid_branch], ["", 255, null, "", "classic.caste.-32768", -1, null], "empty and maximum Data SD2 records cross public package assembly, missing prompt and item sentinels normalize, an inverted battle range survives, and every opcode 85 random destination must resolve during strict validation")
		var world_opcode_scenario: Dictionary = scenario.duplicate(true); world_opcode_scenario["programs"].append({"id": "xap:9877", "ownerKind": "extra-action-point", "ownerId": "9877", "instructions": [{"kind": "classicAction", "slot": 0, "rawOpcode": 92, "opcode": 92, "id": 0, "gosub": false, "extraCode": [0, 1, 0, -500, -1, 0, 0, 0, 0, 0]}, {"kind": "classicAction", "slot": 1, "rawOpcode": 92, "opcode": 92, "id": 1, "gosub": false, "extraCode": [-1, -1, 0, 100, 0, 1, 0, 0, 0, 0]}]}); var world_opcode_content := PackageDomainAssembler.new().assemble(manifest, malformed_content, world, world_opcode_scenario, loaded.media.assets()); assert_true(world_opcode_content != null and world_opcode_content.scenario.program_by_id("xap:9877").instruction_count() == 2 and world_opcode_content.scenario.program_by_id("xap:9877").instruction_at(1).extra_code.size() == 10, "strict package assembly preserves opcode 92's consecutive second row and admits Castle's PC slot-zero fallback indices")
		var scrolling_scenario: Dictionary = scenario.duplicate(true); scrolling_scenario["programs"].append({"id": "xap:9878", "ownerKind": "extra-action-point", "ownerId": "9878", "instructions": [{"kind": "classicAction", "slot": 0, "rawOpcode": 62, "opcode": 62, "id": -200, "gosub": false, "extraCode": [0, 0, 0, 0, 0]}]}); var scrolling_assembler := PackageDomainAssembler.new(); var scrolling_content := scrolling_assembler.assemble(manifest, malformed_content, world, scrolling_scenario, loaded.media.assets()); assert_true(scrolling_content != null and scrolling_content.scenario.program_by_id("xap:9878") != null, "strict package assembly admits opcode 62 when its exact signed TEXT resource is present: %s" % scrolling_assembler.error_message()); var missing_scrolling_scenario: Dictionary = scenario.duplicate(true); missing_scrolling_scenario["programs"].append({"id": "xap:9879", "ownerKind": "extra-action-point", "ownerId": "9879", "instructions": [{"kind": "classicAction", "slot": 0, "rawOpcode": 62, "opcode": 62, "id": -201, "gosub": false, "extraCode": [0, 0, 0, 0, 0]}]}); var missing_scrolling_content := PackageDomainAssembler.new().assemble(manifest, malformed_content, world, missing_scrolling_scenario, loaded.media.assets()); assert_equal(missing_scrolling_content, null, "strict package assembly rejects opcode 62 when its exact signed TEXT resource is absent")
