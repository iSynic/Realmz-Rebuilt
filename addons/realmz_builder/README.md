...
The profiles are Wide, Compact, Empty, Long Content, Unavailable, and Error. Selecting one adds an ownerless clone of the edited scene, then passes deterministic detached data through the clone's production binder. The clone, its variable row scenes, and the small profile badge have no scene owner, so saving cannot serialize any preview content or alter the scene being authored. Older registered scenes may temporarily provide `apply_editor_preview(profile)` while they move to the production-bound fixture catalog.
