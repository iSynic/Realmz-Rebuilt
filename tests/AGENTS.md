...
- The test-budget ratchet began at 11,807 substantive lines. Release preparation corrected a stale 8,023-line value to the measured 8,556-line accepted tree after restoring every file/function/suite cap; it may not grow from that value and remains bounded to 20 percent of production and 1,200 lines per suite. These caps eliminate duplicate/incidental proof; they never authorize removal of a unique high-risk or source-backed owner.
...
- Exploration coverage proves the typed ordinary-movement presentation delta, a zero-shift incremental dungeon quarter-turn that retains unchanged party dependencies, typed allocation across a previously absent 8x8 copy-on-write chunk without mutating the earlier snapshot, hourly fatigue/spell/condition recovery's mutable-party refresh without a complete campaign reprojection, bounded inherited visibility, restore/LOS full-projection fallback, adjacent debug no-clip's incremental RNG-neutral transaction, and exact default versus guarded host-requested detached map spans at map edges. Presentation suites retain map/shell composition ownership rather than testing private caches.
