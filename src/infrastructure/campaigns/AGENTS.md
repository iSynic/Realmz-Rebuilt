...
- `tools/verify_bundled_scenarios.ps1` checks the exact archive set, file bytes, SHA-256 values, manifest identities, Providence compiler revision, authored player-map titles, exact asset resolution and base-plus-overlay separation for every emitted land CICN, City of Bywater source-snapshot identity, its dormant Ranthog Action Point preservation, and its crypt-door Encounter prompt/actions.
