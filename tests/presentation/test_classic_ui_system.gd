...
func _test_scene_composition() -> void:
	...
	for viewport_size: Vector2 in [Vector2(800, 600), Vector2(1280, 720)]:
		var profile := UiLayoutProfile.for_viewport(viewport_size, PresentationSettings.UI_SCALE_AUTO)
		var rect := ScreenNavigator.campaign_rect_for(profile, viewport_size); var spell_rect := ScreenNavigator.spell_workspace_rect_for(profile, viewport_size); var expected_spell_width := (288.0 if profile.id == UiLayoutProfile.COMPACT else 420.0) * profile.ui_scale
