...
`GameShell` binds the complete scene and coordinates its collaborators. `GameShellLayoutController` resizes the authored controls using the calculations in `GameShellLayoutPolicy`; `GameShellMenuController` owns menu definitions and dispatch; `GameShellAvailability` owns player-facing reasons for disabled routes and application actions; `GameShellCommandController` owns the dynamic command records; and `GameShellPicturePresenter` decodes and presents scenario pictures in the authored stage. A shell edit should begin in `game_shell.tscn`, then continue in only the collaborator that owns the affected behavior.
