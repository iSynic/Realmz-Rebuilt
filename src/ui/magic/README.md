...
`SpellsScreenController` binds detached `SpellView` records to those scenes, preserves presentation-only selection and filtering, and emits typed intents. It does not decide whether a cast is legal or mutate the playthrough. `SpellDetailFormatter` turns already-supplied facts into display labels. `ClassicSpellSelectionChrome` provides the shared level rail and spell list used here, during character creation, and during Level Up.
