...
- `DevelopmentPreviewRequest` owns the strict developer-only request vocabulary shared with Providence. `DevelopmentPreviewSession` validates the temporary package through the ordinary application-plus-scenario boundary, creates the deterministic in-memory party, and enters an Action Point, Simple Encounter, map location, exact scenario-owned scrolling `TEXT`, or exact Classic Battle, Treasure, or Shop target. Battle, Treasure, and Shop readiness requires the ordinary typed interaction surface; direct Shop preview is unrestricted because caller-owned acceptance ranges are not part of `Data SD`. `DevelopmentPreviewResultWriter` owns the strict result envelope. Export-excluded tools provide the headless and interactive hosts; public release startup never selects them.
