...
Payment-result branching, complete shared artwork, corrected War and twelve byte-preserving scenario migrations are integrated. Seven current-package acceptance recipes now prove merchant refusal, purchase, decline and saved reentry, corrected Simple 52 refusal, Rhum quest-set GOSUB/return and the Magic Darts trap. The Rhum nested post-teleport save exposed a separate restore-admission defect; its narrow repair passes fail-before/pass-after public exploration proof and restores the original rejected save to byte-identical uninterrupted completion. The renewed detached aggregate passes all 4212 assertions plus package/provenance, export, architecture and inventory gates with only the live editor's unrelated project autoload changes excluded. Clean pinned reference checks and Providence formatting, warnings-denied clippy and full workspace tests pass separately. GAP-PAY-001/002 are retired; historical failed-package evidence remains unchanged. This closes the bounded payment/correction batch, not pooled-payment variants, present-ally outcomes, native CPU/dungeon performance, complete combat replay or full campaign certification. Next broaden the remaining Wrath/AOGM recipe representatives after selecting high-risk caller contexts; reported failures still take priority.
