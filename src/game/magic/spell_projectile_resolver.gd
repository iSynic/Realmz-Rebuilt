...
func resolve_character_projectile(caster: CharacterState, caste: CasteDefinition, projectile_item: ItemDefinition, target: MonsterState, spell: SpellDefinition, power_level: int, rng: RealmzRng) -> ProjectileResolution:
	if caster == null or projectile_item == null or target == null or spell == null or rng == null or power_level < 0 or absi(spell.spell_class) != 9 or absi(spell.damage_type) != 9 or absi(spell.special) not in [0, 7, 49]:
		return null
	...
	damage = _special_projectile_damage(spell, target.current_health, damage)
	var hit_count := 0
	var miss_count := 0
	var total_damage := 0
...
func resolve_monster_projectile(caster: MonsterState, projectile_item: ItemDefinition, target: CharacterState, spell: SpellDefinition, power_level: int, rng: RealmzRng) -> ProjectileResolution:
	...
	target.current_health -= damage
	_apply_projectile_special(spell, target)
	return ProjectileResolution.new(true, 1, 0, damage, damage, duration, target.current_health <= 0)


static func _special_projectile_damage(spell: SpellDefinition, target_health: int, damage: int) -> int:
	return target_health + 10 if absi(spell.special) == 49 else damage


static func _apply_projectile_special(spell: SpellDefinition, target: CharacterState) -> void:
