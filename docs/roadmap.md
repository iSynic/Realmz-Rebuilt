...
- `BattlefieldRules` now owns the complete-footprint adjacency and one-step query shared by simulation, detached `CombatView`, typed battle requests, and the interaction component. Party actors can make reaction-free steps in all eight directions. Cost uses Castle's maximum destination-footprint terrain time plus one for each changed axis; Remake's minimum-footprint implementation was rejected.
- Melee target selection is now proximity-gated for party, monster, ally, and charmed-character paths. A nonadjacent character attack fails before RNG or turn mutation. A nonadjacent monster cannot apply physical damage; it emits an explicit `tactical-movement-not-implemented` event and advances until source-backed monster pathing exists.
- A step whose old or new perimeter contains a hostile is rejected transactionally until Castle's guarding-before/after and every-enemy withdrawal sequences can be represented as serializable continuations. This intentionally prevents a partial movement slice from skipping reactions. Remake's once-per-round `reaction_ready` behavior is not equivalent and was not imported. Rejected attacks and moves leave positions, attack reserve, movement, active-turn state, and RNG unchanged.
