...
- Startup now embeds the complete `front_door_menu.tscn`, and campaign selection authors its selected record, alternate library states, package-operation status, install dialog, and refresh actions. `CampaignLibraryController` binds both scenes and instantiates only the campaign selector's exported package-row scene; it constructs no Controls, lowering runtime-created Controls to 91.
