...
func _test_snapshot_rng_and_age_persistence(content: RealmzContent) -> void:
	...
	assert_equal(session.snapshot().game_state.clock.total_minutes(), 5, "one held Area Search pulse owns Castle's five indoor timeclicks")
	...
	assert_equal(constructor_copy.game_state.clock.total_minutes(), 5, "SessionSnapshot detaches constructor-owned game state")
	...
	assert_equal(held_snapshot.game_state.clock.total_minutes(), 5, "a snapshot is detached from later game-state mutations")
	...
	assert_equal(restored.snapshot().game_state.clock.total_minutes(), 10, "restored Area Search advances the persisted clock by another five indoor timeclicks")
...
func _test_combat_and_reward_persistence(content: RealmzContent) -> void:
	...
	if fumble_item != null:
		...
		var combat_body := CombatContinuationBody.new()
		combat_body.battle_id = fumble_session._context.state.combat.battle_id
		fumble_session._context.session_continuation = CombatContinuations.ally_selection(combat_body)
