...
func _test_player_collision_initiates_melee() -> void:
	...
	var east_request: Dictionary = request.body.to_data().get("movement", []).filter(func(option: Dictionary) -> bool: return option.get("direction") == [1, 0])[0]
	...
	var combatants: Array = request.body.to_data().get("combatants", [])
	...
	assert_equal([combatants[1].get("range"), combatants[1].get("blocked"), request.body.to_data().get("enemiesRemaining")], [1, false, 1], "the core supplies target range, LOS, and enemy count rather than asking presentation to derive combat facts")
	...
	var host_result := host_api._resume_battle(_classic_battle_continuation(host_state.combat.battle_id), InteractionResponse.from_data("request.collision-host", InteractionRequest.COMBAT, {"actorId": host_character.id, "action": "move", "targetId": "", "destination": [46, 45]}), "request.collision-host")
	...
	var malformed_host := host_api._resume_battle(_classic_battle_continuation(host_state.combat.battle_id), InteractionResponse.from_data("request.collision-host", InteractionRequest.COMBAT, {"actorId": host_character.id, "action": "move", "targetId": "", "destination": [46, 45], "autoSwitchToMelee": "yes"}), "request.collision-host")
...
func _test_source_backed_combat_spell_item_use() -> void:
	...
	assert_true(request.body.to_data().get("actions", []).has("use_item"), "the scenario host advertises item use only when a source-probed option exists")
	assert_true(request.body.to_data().get("itemCasts", []).any(func(option: Dictionary) -> bool: return option.get("itemInstanceId") == instance.id and option.get("targetId") == monster.id and option.get("power") == 1), "the scenario interaction carries exact item, spell-power, and target identities")
	...
	var host_result := host_api._resume_battle(_classic_battle_continuation(state.combat.battle_id), InteractionResponse.from_data("request.item-use", InteractionRequest.COMBAT, {"actorId": character.id, "action": "use_item", "targetId": monster.id, "itemInstanceId": instance.id}), "request.item-use")
	...
	var invalid_host_result := host_api._resume_battle(_classic_battle_continuation(state.combat.battle_id), InteractionResponse.from_data("request.item-use", InteractionRequest.COMBAT, {"actorId": character.id, "action": "use_item", "targetId": monster.id}), "request.item-use")
...
func _test_source_backed_combat_scroll_use() -> void:
	...
	assert_true(request.body.to_data().get("actions", []).has("use_scroll"), "the scenario host advertises combat scroll use only when a source-probed option exists")
	assert_true(request.body.to_data().get("scrollCasts", []).any(func(option: Dictionary) -> bool: return option.get("scrollSlot") == 0 and option.get("spellId") == spell.id and option.get("power") == 2 and option.get("targetId") == monster.id), "the combat interaction carries exact scroll, spell-power, and target identities")
...
func _test_detached_turn_start_movement_refresh() -> void:
	...
	assert_equal(request.body.to_data().get("movementRemaining"), character.maximum_movement, "the typed combat request carries the restored turn-start allowance")
	var request_west: Dictionary = {}
	for option: Dictionary in request.body.to_data().get("movement", []):
...
func _test_character_and_monster_retreat() -> void:
	...
	assert_equal([confirmation.state, confirmation.interaction.kind, confirmation.interaction.body.to_data().get("yesLabel"), confirmation.interaction.body.to_data().get("noLabel")], [SessionStep.State.WAITING_FOR_INTERACTION, InteractionRequest.YES_NO, "Embrace Cowardice", "Stay and Fight"], "explicit Escape yields Castle's typed confirmation without mutating combat")
	...
	var confirmation_save := save_round_trip(session.snapshot())
	...
	var edge_host_actor := _character("character.edge-retreat-actor")
	var edge_companion := _character("character.edge-retreat-companion")
	var edge_host_field := _blank_battlefield()
	edge_host_field.place_character(edge_host_actor.id, Vector2i(2, 45))
	edge_host_field.place_character(edge_companion.id, Vector2i(45, 44))
	edge_host_field.place_monster(monster.id, Vector2i(20, 45), 0)
	var edge_host_state := GameState.new(PartyState.new("map.test", Vector2i.ZERO, [edge_host_actor, edge_companion]), RealmzClock.new())
	...
	var edge_prompt := edge_host_api._resume_battle(_classic_battle_continuation(edge_host_state.combat.battle_id), InteractionResponse.from_data("request.edge-retreat", InteractionRequest.COMBAT, {"actorId": edge_host_actor.id, "action": "retreat_edge", "targetId": "", "destination": [1, 45]}), "request.edge-retreat")
	...
	assert_true(edge_accepted.events.any(func(event: DomainEvent) -> bool: return event.kind == &"combatant_retreated" and event.payload.get("actorId") == edge_host_actor.id and event.payload.get("mode") == "edge"), "the edge response commits the source-owned retreat mode")
	assert_equal([edge_host_state.combat.battlefield.has_actor(edge_host_actor.id), edge_host_state.combat.battlefield.has_actor(edge_companion.id), edge_host_actor.prestige_penalty], [false, true, 200], "edge Escape removes one actor and applies the penalty exactly once")
...
func _test_character_weapon_mode_toggle_and_restore() -> void:
	...
	assert_equal(combat_view.legal_actions, [&"switch_weapon", &"finish", &"defend", &"auto"], "the detached view cannot advertise melee attack or illegal close-range Escape while missile mode is active, but exposes the settled current-turn Auto command")
	...
	assert_equal([request.body.to_data().get("weaponMode"), request.body.to_data().get("actions")], ["missile", ["switch_weapon", "finish", "defend", "auto", "delay", "undo"]], "the typed interaction preserves the same legal actions as the detached view and exposes fresh-activation Delay and Undo")
	assert_equal([request.body.to_data().get("retreat", {}).get("enabled"), request.body.to_data().get("retreat", {}).get("nearestEnemyRange")], [false, 1], "the typed interaction explains the source-backed close-range Escape blocker")
	assert_equal(request.body.to_data().get("weaponSwitch", {}).get("targetMode"), "melee", "the typed switch response names its source-owned destination mode")
	assert_false(String(request.body.to_data().get("rangedAttack", {}).get("reason", "")).is_empty(), "the typed interaction explains why Fire is disabled")
	...
	var setup_footprint: Array[Vector2i] = setup_view.battlefield.monster_footprint(setup_monster.id)
	assert_equal(setup_footprint, BattlefieldState.footprint_cells(setup_state.combat.battlefield.monster_position(setup_monster.id), setup_state.combat.battlefield.monster_size(setup_monster.id)), "presentation receives the exact typed session-owned Classic footprint")
	var absent_footprint: Array[Vector2i] = setup_view.battlefield.monster_footprint("monster.absent")
	assert_equal(absent_footprint, [], "an absent or removed monster exposes a typed empty footprint that the tactical renderer can skip")
...
func _test_source_backed_projectile_fire() -> void:
	...
	if host_state != null:
		...
		assert_equal(melee_request.body.to_data().get("weaponSwitch", {}).get("targetMode"), "missile", "the typed melee request offers the source-backed missile mode")
		var switched := switch_api._resume_battle(_classic_battle_continuation(host_state.combat.battle_id), InteractionResponse.from_data(melee_request.request_id, InteractionRequest.COMBAT, {"actorId": host_character.id, "action": "switch_weapon", "targetId": ""}), melee_request.request_id)
		...
		assert_equal(switched.interaction.body.to_data().get("weaponMode"), "missile", "the resumed request exposes missile mode without spending the turn")
		assert_equal(switched.interaction.body.to_data().get("targets", []).map(func(target: Dictionary) -> String: return String(target.get("id"))), [host_monster.id], "the resumed Fire picker contains only the core-proven hostile target")
		...
		if restored_host_state != null:
			...
			var fired := restored_api._resume_battle(_classic_battle_continuation(restored_host_state.combat.battle_id), InteractionResponse.from_data(switched.interaction.request_id, InteractionRequest.COMBAT, {"actorId": host_character.id, "action": "attack", "targetId": host_monster.id}), switched.interaction.request_id)
...
func _test_source_backed_character_spell_casting() -> void:
	...
	if host_state != null:
		...
		assert_true(host_request.body.to_data().get("spellCasts", []).any(func(option: Dictionary) -> bool: return option.get("spellId") == spell.id and option.get("power") == 1 and option.get("targetId") == monster.id and option.get("targetMode") == "combatant"), "the runtime request exposes the core-proven single-target cast")
		var host_cast := host_api._resume_battle(_classic_battle_continuation(host_state.combat.battle_id), InteractionResponse.from_data(host_request.request_id, InteractionRequest.COMBAT, {"actorId": character.id, "action": "cast_spell", "targetId": monster.id, "spellId": spell.id, "power": 1}), host_request.request_id)
...
func _test_character_automatic_group_spell() -> void:
	...
	if host_state != null:
		...
		assert_true(host_request.body.to_data().get("spellCasts", []).any(func(option: Dictionary) -> bool: return option.get("spellId") == spell.id and option.get("power") == 1 and option.get("targetId") == "" and option.get("targetMode") == "automatic"), "the runtime request preserves the targetless automatic-group ABI")
		var host_cast := host_api._resume_battle(_classic_battle_continuation(host_state.combat.battle_id), InteractionResponse.from_data(host_request.request_id, InteractionRequest.COMBAT, {"actorId": caster.id, "action": "cast_spell", "targetId": "", "spellId": spell.id, "power": 1}), host_request.request_id)
...
func _test_character_fixed_area_spell() -> void:
	...
	if host_state != null:
		...
		assert_true(host_request.body.to_data().get("spellCasts", []).any(func(option: Dictionary) -> bool: return option.get("spellId") == spell.id and option.get("power") == 1 and option.get("targetMode") == "area" and option.get("areaShape") == 3), "the runtime request carries the rules-owned Data AD shape")
		var host_cast := host_api._resume_battle(_classic_battle_continuation(host_state.combat.battle_id), InteractionResponse.from_data(host_request.request_id, InteractionRequest.COMBAT, {"actorId": caster.id, "action": "cast_spell", "targetId": "", "spellId": spell.id, "power": 1, "targetCoordinate": [45, 45], "rotation": 0}), host_request.request_id)
...
func _test_source_backed_monster_spell_casting() -> void:
	...
	assert_false(events.any(func(event: DomainEvent) -> bool: return event.kind == &"combat_monster_action" and event.payload.get("action") == "cast"), "a completed source-backed cast does not fall through to the unknown-action feedback branch")
...
func _test_battle_owned_fumble_and_exact_recovery() -> void:
	...
	var recovered := rules.combat_flow.apply_fumble_recovery(restored, _content([definition], [weapon]), &"assign", instance.id, character.id)
...
func _test_batch_two_delay_bandage_and_bleeding() -> void:
	var rules := RealmzRules.new()
	var actor := _character("character.delay-actor")
	actor.normal_attacks = 2
	actor.attack_bonus = 1
	var following := _character("character.delay-following")
	var monster_definition := _monster_definition("monster.delay", [])
	var monster := MonsterState.new("monster.delay.instance", monster_definition.id, monster_definition.name, 50, 50, 1)
	var state := _state(actor, monster, "battle.delay")
	assert_true(state.party.add_character(following), "Delay fixture adds a second loyal actor without changing the active character")
	...
	var content := _content([monster_definition])
	var delay_probe := rules.combat_flow.probe_delay(state, actor.id)
	assert_true(delay_probe.allowed, "Castle Delay is available at a fresh loyal character activation")
	var delayed := rules.combat_flow.submit_action(state, content, actor.id, &"delay", "", ScriptedRng.new([0, 0, 0]))
	assert_true(delayed.ok, "Delay commits through the ordinary combat command path")
	assert_true(delayed.events.any(func(event: DomainEvent) -> bool: return event.kind == &"combat_turn_delayed" and event.payload.get("actorId") == actor.id and not event.payload.get("roundAdvanced")), "a normal Delay moves the actor behind the remaining current-round order")
	assert_equal(state.combat.turn_order(), [following.id, monster.id, actor.id], "Delay preserves the source order while moving the active actor to the current-round tail")
	assert_equal([state.combat.active_actor_id(), actor.attacks_remaining], [following.id, 1], "Delay subtracts normal attacks plus attack bonus while preserving the carried half-attack without selecting the delayed actor again")
	var delayed_save := GameState.from_data(JSON.parse_string(JSON.stringify(state.to_data())))
	assert_not_null(delayed_save, "Delay initiative state survives the central save aggregate")
	if delayed_save != null:
		assert_equal([delayed_save.combat.turn_order(), delayed_save.combat.active_actor_id()], [[following.id, monster.id, actor.id], following.id], "save restoration preserves the delayed actor's exact current-round position")

	var last_actor := _character("character.delay-last")
	last_actor.normal_attacks = 2
	var last_monster := MonsterState.new("monster.delay-last.instance", monster_definition.id, monster_definition.name, 50, 50, 1)
	var last_state := _state(last_actor, last_monster, "battle.delay-last")
	last_state.combat.set_turn_order([last_monster.id, last_actor.id])
	last_state.combat.turn_index = 1
	var last_probe := rules.combat_flow.probe_delay(last_state, last_actor.id)
	assert_true(last_probe.allowed, "the last actor may still request Delay at a fresh activation")
	var last_round_advanced := last_state.combat.delay_active_actor()
	assert_true(last_round_advanced, "the corrected last-slot Delay advances to the next round")
	assert_equal([last_state.combat.round_number, last_state.combat.turn_index, last_state.combat.turn_order()], [2, 0, [last_monster.id, last_actor.id]], "last-slot Delay does not reselect the same actor within the current round")

	var spent_state := _state(_character("character.delay-spent"), monster, "battle.delay-spent")
	var spent_actor := spent_state.party.character_by_id("character.delay-spent")
	rules.combat_flow._prepare_character_turn(spent_state.combat, spent_actor)
	spent_actor.movement -= 1
	assert_false(rules.combat_flow.probe_delay(spent_state, spent_actor.id).allowed, "Delay is unavailable after the active character has spent movement")

	var healer := _character("character.bandage-healer")
	var bandage_following := _character("character.bandage-following")
	var bleeding_one := _character("character.bandage-one")
	bleeding_one.current_health = -1
	var bleeding_two := _character("character.bandage-two")
	bleeding_two.current_health = -2
	var bandage_monster := MonsterState.new("monster.bandage.instance", monster_definition.id, monster_definition.name, 50, 50, 1)
	var bandage_state := _state(healer, bandage_monster, "battle.bandage")
	assert_true(bandage_state.party.add_character(bandage_following), "Bandage fixture adds a live following actor to expose the activation boundary")
	assert_true(bandage_state.party.add_character(bleeding_one), "Bandage fixture adds the first bleeding recipient")
	assert_true(bandage_state.party.add_character(bleeding_two), "Bandage fixture adds the second bleeding recipient")
	bandage_state.combat.battlefield.place_character(bandage_following.id, Vector2i(45, 46))
	bandage_state.combat.battlefield.remove_character(bleeding_one.id)
	bandage_state.combat.battlefield.remove_character(bleeding_two.id)
	bandage_state.combat.set_turn_order([healer.id, bandage_following.id, bleeding_one.id, bleeding_two.id, bandage_monster.id])
	assert_true(bandage_state.combat.set_character_bleeding(bleeding_one.id, true), "combat owns the first bleeding flag")
	assert_true(bandage_state.combat.set_character_bleeding(bleeding_two.id, true), "combat owns the second bleeding flag")
	var candidates := rules.combat_flow.bandage_candidate_ids(bandage_state)
	assert_equal(candidates, [bleeding_one.id, bleeding_two.id], "Bandage candidates follow party order and include only actual bleeding recipients")
	var bandage_probe := rules.combat_flow.probe_bandage(bandage_state, healer.id, bleeding_one.id)
	assert_true(bandage_probe.allowed, "Bandage is legal at a fresh activation when a bleeding recipient exists")
	var bandaged := rules.combat_flow.submit_action(bandage_state, _content([monster_definition]), healer.id, &"bandage", bleeding_one.id, ScriptedRng.new([0, 0, 0, 0]))
	assert_true(bandaged.ok, "Bandage commits through the ordinary combat command path")
	assert_false(bandage_state.combat.is_character_bleeding(bleeding_one.id), "Bandage clears exactly the selected recipient")
	assert_true(bandage_state.combat.is_character_bleeding(bleeding_two.id), "Bandage does not clear another bleeding recipient")
	assert_equal([bandage_state.combat.active_actor_id(), healer.attacks_remaining, healer.movement], [bandage_following.id, 0, 0], "Bandage ends the acting character's activation and returns control to the next legal actor")
	assert_true(bandaged.events.any(func(event: DomainEvent) -> bool: return event.kind == &"combatant_bandaged" and event.payload.get("targetId") == bleeding_one.id), "Bandage publishes the selected recipient in its feedback event")
	assert_true(bandaged.events.any(func(event: DomainEvent) -> bool: return event.kind == &"sound_requested" and event.payload.get("soundId") == 10105), "Bandage preserves Castle's source sound")

	var bandage_save := GameState.from_data(JSON.parse_string(JSON.stringify(bandage_state.to_data())))
	assert_not_null(bandage_save, "combat bleeding and Bandage state survive the central save aggregate")
	if bandage_save != null:
		assert_equal(bandage_save.combat.bleeding_character_ids(), [bleeding_two.id], "save restoration preserves the remaining bleeding recipient")
	var corrupt_bleeding: Dictionary = bandage_state.to_data()
	corrupt_bleeding["combat"]["bleedingCharacterIds"] = [bandage_following.id]
	assert_equal(GameState.from_data(corrupt_bleeding), null, "restore rejects a positive-health character carrying an impossible bleeding flag")
	var unknown_bleeding: Dictionary = bandage_state.to_data()
	unknown_bleeding["combat"]["turnOrder"].append("character.unknown-bleeding")
	unknown_bleeding["combat"]["bleedingCharacterIds"] = ["character.unknown-bleeding"]
	assert_equal(GameState.from_data(unknown_bleeding), null, "the central aggregate rejects combat-owned bleeding IDs outside the party")

	var round_actor := _character("character.bleeding-survivor")
	var round_one := _character("character.bleeding-progress")
	round_one.current_health = -1
	var round_two := _character("character.bleeding-death")
	round_two.current_health = -9
	var round_monster := MonsterState.new("monster.bleeding.instance", monster_definition.id, monster_definition.name, 50, 50, 1)
	var round_state := _state(round_actor, round_monster, "battle.bleeding-round")
	round_state.party.add_character(round_one)
	round_state.party.add_character(round_two)
	round_state.combat.battlefield.remove_character(round_one.id)
	round_state.combat.battlefield.remove_character(round_two.id)
	round_state.combat.set_turn_order([round_actor.id, round_one.id, round_two.id, round_monster.id])
	round_state.combat.set_character_bleeding(round_one.id, true)
	round_state.combat.set_character_bleeding(round_two.id, true)
	var bleeding_events: Array[DomainEvent] = []
	var bleeding_rng := ScriptedRng.new([0, 8192])
	for _step: int in 4:
		rules.combat_flow._advance_turn(round_state, bleeding_rng, bleeding_events)
	assert_equal([round_one.current_health, round_two.current_health], [-2, -10], "each round boundary applies one party-ordered bleeding tick and clears the -10 death boundary")
	assert_equal(bleeding_rng.snapshot().draw_count, 2, "bleeding preserves Castle's survivor sound draw followed by the default party-wide warning draw")
	assert_equal(bleeding_rng.trace().map(func(entry: Dictionary) -> String: return entry["tag"]), ["combat.bleeding.%s" % round_one.id, "combat.bleeding.warning-sound"], "bleeding randomness preserves the character result and party warning order")
	assert_true(bleeding_events.any(func(event: DomainEvent) -> bool: return event.kind == &"combatant_bled_to_death" and event.payload.get("characterId") == round_two.id), "the -10 bleeding boundary publishes the source-ordered death event")
	var warning_events: Array[DomainEvent] = bleeding_events.filter(func(event: DomainEvent) -> bool: return event.kind == &"combat_bleeding_warning")
	assert_equal(warning_events.size(), 1, "the default Classic bleeding warning remains a distinct presentation event")
	if not warning_events.is_empty():
		assert_equal(warning_events[0].payload.get("soundId"), 10121 if bleeding_rng.trace()[1].get("result") < 50 else 10123, "the warning presentation event carries the sound selected by its recorded draw")


func _test_batch_two_turn_undead_and_restore() -> void:
	var rules := RealmzRules.new()
	var actor := _character("character.turn-undead")
	actor.normal_attacks = 2
	assert_true(actor.set_ability_value(13, 100), "Turn Undead fixture grants the source ability at index 13")
	var type_flags := _ints(8)
	type_flags[1] = 1
	var undead_definition := MonsterDefinition.new("monster.turn-undead", 230, "Nether Test", 1, 0, 1, 0, 0, type_flags, _ints(8), _ints(6), _ints(3), [], [], [], [])
	undead_definition.can_summon = 1
	var first := MonsterState.new("monster.turn-undead.first", undead_definition.id, undead_definition.name, 20, 20, 1)
	var second := MonsterState.new("monster.turn-undead.second", undead_definition.id, undead_definition.name, 20, 20, 1)
	var state := _state(actor, first, "battle.turn-undead")
	assert_true(state.combat.add_monster(second), "Turn Undead fixture retains source target order for a second undead")
	state.combat.battlefield.place_monster(second.id, Vector2i(48, 45), 0)
	state.combat.set_turn_order([actor.id, first.id, second.id])
	var content := _content([undead_definition])
	assert_equal(rules.combat_flow.turn_undead_target_ids(state, content), [first.id, second.id], "Turn Undead targets eligible hostile monsters in MonsterState order")
	assert_true(rules.combat_flow.probe_turn_undead(state, content, actor.id).allowed, "an eligible loyal priest can Turn Undead before using the activation")
	var rng := ScriptedRng.new([8192, 18022])
	var result := rules.combat_flow._turn_undead(state, content, actor, rng)
	assert_true(result.ok, "Turn Undead resolves through the source-backed threshold path")
	assert_equal([first.current_health, state.combat.battlefield.has_actor(first.id)], [0, false], "a margin from one through twenty-nine destroys the undead and removes its footprint")
	assert_equal(second.traitor, false, "a margin of thirty or more turns the undead to the acting character's allegiance")
	assert_equal(actor.experience, 75, "Turn Undead awards twenty-five or fifty times hit dice in source target order")
	assert_equal(actor.attacks_remaining, 1, "Turn Undead consumes two attack units while preserving the carried half-attack")
	assert_true(state.combat.has_used_turn_undead(actor.id), "Turn Undead is marked used before its target outcomes complete")
	assert_true(result.events.any(func(event: DomainEvent) -> bool: return event.kind == &"combat_turn_undead_resolved" and event.payload.get("targetId") == first.id and event.payload.get("result") == "destroyed" and event.payload.get("threshold") == 25), "Turn Undead publishes the source threshold and destruction result")
	assert_true(result.events.any(func(event: DomainEvent) -> bool: return event.kind == &"combat_turn_undead_resolved" and event.payload.get("targetId") == second.id and event.payload.get("result") == "turned" and event.payload.get("effectResourceId") == 12056 and event.payload.get("effectFrameCount") == 8), "Turn Undead publishes the proven eight-frame turned effect identity")
	assert_equal(rng.trace().map(func(entry: Dictionary) -> String: return entry["tag"]), ["combat.turn-undead.%s" % first.id, "combat.turn-undead.%s" % second.id], "Turn Undead consumes one tagged d100 per eligible target in source order")

	var restored := GameState.from_data(JSON.parse_string(JSON.stringify(state.to_data())))
	assert_not_null(restored, "Turn Undead's used state and changed allegiance survive save restoration")
	if restored != null:
		assert_true(restored.combat.has_used_turn_undead(actor.id), "restore preserves the once-per-battle Turn Undead marker")
		assert_equal(restored.combat.monster_by_id(second.id).traitor, false, "restore preserves a turned monster's battle allegiance")
		restored.combat.turn_index = 0
		restored.party.character_by_id(actor.id).attacks_remaining = 2
		assert_false(rules.combat_flow.probe_turn_undead(restored, content, actor.id).allowed, "a restored character cannot Turn Undead a second time in the same battle")
	var unknown_turner: Dictionary = state.to_data()
	unknown_turner["combat"]["turnOrder"].append("character.unknown-turner")
	unknown_turner["combat"]["turnUndeadActorIds"] = ["character.unknown-turner"]
	assert_equal(GameState.from_data(unknown_turner), null, "the central aggregate rejects Turn Undead history outside the party")

	state.priest_turning_allowed = false
	assert_false(rules.combat_flow.probe_turn_undead(state, content, actor.id).allowed, "campaign priest-turning permission remains a rules-owned gate")
	var excluded_definition := MonsterDefinition.new("monster.turn-undead.excluded", 231, "Excluded Nether", 1, 0, 1, 0, 0, type_flags, _ints(8), _ints(6), _ints(3), [], [], [], [])
	excluded_definition.can_summon = -1
	var excluded := MonsterState.new("monster.turn-undead.excluded.instance", excluded_definition.id, excluded_definition.name, 20, 20, 1)
	var excluded_state := _state(_character("character.turn-undead.excluded"), excluded, "battle.turn-undead.excluded")
	assert_true(rules.combat_flow.turn_undead_target_ids(excluded_state, _content([excluded_definition])).is_empty(), "Providence's signed -1 form of Castle byte 255 remains excluded from Turn Undead targets")


func _test_batch_two_combat_auto_state() -> void:
	var first := _character("character.auto-zeta")
	var second := _character("character.auto-alpha")
	var state := GameState.new(PartyState.new("map.test", Vector2i.ZERO, [first, second]), RealmzClock.new())
	assert_true(state.set_combat_auto(first.id, true), "persistent Auto accepts a living party character")
	assert_true(state.set_combat_auto(second.id, true), "persistent Auto accepts a second living party character")
	assert_equal(state.combat_auto_character_ids(), [second.id, first.id], "persistent Auto IDs are exposed in stable sorted order")
	var data: Dictionary = JSON.parse_string(JSON.stringify(state.to_data()))
	assert_equal(data.get("combatAutoCharacterIds"), [second.id, first.id], "the save payload uses the stable sorted Auto ID order")
	var restored := GameState.from_data(data)
	assert_not_null(restored, "persistent Auto state round-trips through GameState serialization")
	if restored != null:
		assert_equal(restored.combat_auto_character_ids(), [second.id, first.id], "restore retains both persistent Auto characters")
	var old_data: Dictionary = data.duplicate(true)
	old_data.erase("combatAutoCharacterIds")
	var old_save := GameState.from_data(old_data)
	assert_not_null(old_save, "older saves without the optional Auto field remain readable")
	if old_save != null:
		assert_true(old_save.combat_auto_character_ids().is_empty(), "older saves default persistent Auto to an empty set")
	var invalid_data: Dictionary = data.duplicate(true)
	invalid_data["combatAutoCharacterIds"] = ["character.missing"]
	assert_equal(GameState.from_data(invalid_data), null, "restore rejects a persistent Auto ID outside the party")
	var dead_data: Dictionary = data.duplicate(true)
	dead_data["party"]["characters"][1]["currentHealth"] = 0
	assert_equal(GameState.from_data(dead_data), null, "restore rejects a persistent Auto ID for a dead character")
	second.current_health = 0
	assert_true(state.set_combat_auto(second.id, false), "internal cleanup can disable Auto after the tracked character dies")
	assert_equal(state.combat_auto_character_ids(), [first.id], "death cleanup removes exactly the disabled Auto character")
	state._combat_auto_character_ids[second.id] = true
	state.prune_combat_auto_characters()
	assert_equal(state.combat_auto_character_ids(), [first.id], "combat defeat prunes persistent Auto without touching the surviving character")
	var defeated_actor := _character("character.auto-defeated")
	var defeated_definition := _monster_definition("monster.auto-defeated", [])
	var defeated_monster := MonsterState.new("monster.auto-defeated.instance", defeated_definition.id, defeated_definition.name, 50, 50, 1)
	var defeated_state := _state(defeated_actor, defeated_monster, "battle.auto-defeated")
	assert_true(defeated_state.set_combat_auto(defeated_actor.id, true), "defeat fixture enables persistent Auto before removal")
	defeated_actor.current_health = -1
	RealmzRules.new().combat_flow._mark_character_bleeding(defeated_state, defeated_actor, true)
	assert_false(defeated_state.combat_auto_enabled(defeated_actor.id), "killbody-backed defeat clears persistent Auto immediately, including recoverable bleeding bodies")
	assert_true(defeated_state.combat.is_character_bleeding(defeated_actor.id), "immediate Auto cleanup does not discard the recoverable bleeding state")
	assert_true(state.party.remove_character(first.id), "the Auto fixture removes its final tracked character")
	state.prune_combat_auto_characters()
	assert_true(state.combat_auto_character_ids().is_empty(), "party removal prunes persistent Auto state")
	var intent := PlayerIntent.set_combat_auto(first.id, true)
	var auto_payload := intent.payload as PlayerIntent.CombatAutoPayload
	assert_equal([intent.kind, auto_payload.character_id, auto_payload.enabled], [PlayerIntent.Kind.SET_COMBAT_AUTO, first.id, true], "the public PlayerIntent factory carries the stable Auto toggle")

	var session_actor := _character("character.auto-session-active")
	var session_manual := _character("character.auto-session-manual")
	var session_future := _character("character.auto-session-future")
	var session_definition := _monster_definition("monster.auto-session", [])
	var session_monster := MonsterState.new("monster.auto-session.instance", session_definition.id, session_definition.name, 1000, 1000, 1)
	var session_state := _state(session_actor, session_monster, "battle.auto-session")
	assert_true(session_state.party.add_character(session_manual), "active-toggle integration fixture adds its manual stop character")
	assert_true(session_state.party.add_character(session_future), "active-toggle integration fixture adds an inactive future character")
	session_state.combat.battlefield.place_character(session_manual.id, Vector2i(44, 45))
	session_state.combat.battlefield.place_character(session_future.id, Vector2i(44, 46))
	session_state.combat.set_turn_order([session_actor.id, session_manual.id, session_future.id, session_monster.id])
	session_state.party_setup_completed = true
	var session_content := _content([session_definition])
	var session := GameSession.new()
	assert_equal(session.start(session_content, 31).state, SessionStep.State.COMPLETED, "persistent Auto integration fixture starts a validated session")
	session._state = session_state
	session._runtime_api = RealmzRuntimeApi.new(session_content, session_state, session._rng, session._scenario_action_state, session._rules)
	var active_toggle := session.submit_intent(PlayerIntent.set_combat_auto(session_actor.id, true))
	assert_equal(active_toggle.state, SessionStep.State.COMPLETED, "enabling persistent Auto on the active character starts its activation immediately")
	assert_equal([session._state.combat.active_actor_id(), session._state.combat_auto_character_ids()], [session_manual.id, [session_actor.id]], "active persistent Auto stops at the next manual actor while retaining the saved toggle")
	assert_true(active_toggle.events.any(func(event: DomainEvent) -> bool: return event.kind == &"combat_auto_changed" and event.payload.get("enabled")), "the public intent publishes the persistent Auto toggle before automatic action events")
	assert_equal(active_toggle.events.filter(func(event: DomainEvent) -> bool: return event.kind == &"sound_requested" and String(event.payload.get("source", "")).begins_with("classic-combat-auto")).map(func(event: DomainEvent) -> int: return event.payload.get("soundId")), [147, 141], "enabling active persistent Auto preserves Castle's toggle and Auto-button sound order")
	var inactive_toggle := session.submit_intent(PlayerIntent.set_combat_auto(session_future.id, true))
	assert_equal(inactive_toggle.state, SessionStep.State.COMPLETED, "enabling persistent Auto on an inactive character is accepted at the combat boundary")
	assert_equal([session._state.combat.active_actor_id(), session._state.combat_auto_character_ids()], [session_manual.id, [session_actor.id, session_future.id]], "the inactive toggle waits for its next activation without changing the current actor")
	assert_equal(inactive_toggle.events.filter(func(event: DomainEvent) -> bool: return event.kind == &"sound_requested").map(func(event: DomainEvent) -> int: return event.payload.get("soundId")), [147], "an inactive persistent Auto toggle plays only Castle's toggle-on sound")
	var session_save := session.snapshot()
	assert_not_null(session_save, "persistent Auto remains a committed save boundary after active and inactive toggles")
	var session_restored := GameSession.new()
	assert_equal(session_restored.restore(session_content, session_save).state, SessionStep.State.COMPLETED, "whole-session restore accepts the optional persistent Auto state")
	assert_equal(session_restored._state.combat_auto_character_ids(), [session_actor.id, session_future.id], "whole-session restore preserves both persistent Auto selections")
	var disabled_toggle := session_restored.submit_intent(PlayerIntent.set_combat_auto(session_future.id, false))
	assert_equal(disabled_toggle.events.filter(func(event: DomainEvent) -> bool: return event.kind == &"sound_requested").map(func(event: DomainEvent) -> int: return event.payload.get("soundId")), [139], "disabling persistent Auto preserves Castle's toggle-off sound")
	var vm_actor := _character("character.auto-vm-active")
	var vm_future := _character("character.auto-vm-future")
	var vm_monster := MonsterState.new("monster.auto-vm.instance", session_definition.id, session_definition.name, 1000, 1000, 1)
	var vm_state := _state(vm_actor, vm_monster, "battle.auto-vm")
	assert_true(vm_state.party.add_character(vm_future), "scenario Auto fixture adds the inactive character whose toggle must survive the pending battle boundary")
	vm_state.combat.battlefield.place_character(vm_future.id, Vector2i(44, 45))
	vm_state.combat.set_turn_order([vm_actor.id, vm_future.id, vm_monster.id])
	var vm_api := RealmzRuntimeApi.new(session_content, vm_state, RealmzRng.new(37), ScenarioActionState.new())
	...
	var vm_result := vm_api._resume_battle(_classic_battle_continuation(vm_state.combat.battle_id), vm_response, vm_response.request_id)
	...
	if vm_restored != null:
		assert_equal(vm_restored.combat_auto_character_ids(), [vm_future.id], "restore preserves the VM-issued inactive Auto selection without advancing combat")

	var rules := RealmzRules.new()
	var auto_definition := _monster_definition("monster.auto", [])
	var auto_actor := _character("character.auto-turn")
	auto_actor.normal_attacks = 2
	var auto_monster := MonsterState.new("monster.auto.instance", auto_definition.id, auto_definition.name, 1000, 1000, 1)
	var auto_state := _state(auto_actor, auto_monster, "battle.auto-turn")
	var auto_result := rules.combat_flow.run_auto_turn(auto_state, _content([auto_definition]), auto_actor.id, RealmzRng.new(17))
	assert_true(auto_result.ok, "current-turn Auto resolves one complete activation")
	assert_equal([auto_state.combat.round_number, auto_state.combat.active_actor_id()], [2, auto_actor.id], "current-turn Auto stops when the actor returns in the next round")
	assert_equal(auto_result.events.filter(func(event: DomainEvent) -> bool: return event.kind == &"combat_auto_started").size(), 1, "current-turn Auto starts exactly one activation")
	var command_actor := _character("character.auto-command")
	var command_monster := MonsterState.new("monster.auto-command.instance", auto_definition.id, auto_definition.name, 1000, 1000, 1)
	var command_state := _state(command_actor, command_monster, "battle.auto-command")
	var command_result := rules.combat_flow.submit_action(command_state, _content([auto_definition]), command_actor.id, &"auto", "", RealmzRng.new(19))
	assert_true(command_result.ok, "the public current-turn Auto command resolves through the bounded automation routine")
	assert_equal(command_result.events.filter(func(event: DomainEvent) -> bool: return event.kind == &"sound_requested" and event.payload.get("source") == "classic-combat-auto-button").map(func(event: DomainEvent) -> int: return event.payload.get("soundId")), [141], "the public current-turn Auto command preserves Castle's button sound exactly once")

	var bandage_actor := _character("character.auto-bandage")
	var bandage_following := _character("character.auto-bandage-following")
	var bandage_target := _character("character.auto-bandage-target")
	bandage_target.current_health = -1
	var bandage_monster := MonsterState.new("monster.auto-bandage.instance", auto_definition.id, auto_definition.name, 1000, 1000, 1)
	var auto_bandage_state := _state(bandage_actor, bandage_monster, "battle.auto-bandage")
	auto_bandage_state.party.add_character(bandage_following)
	auto_bandage_state.party.add_character(bandage_target)
	auto_bandage_state.combat.battlefield.place_character(bandage_following.id, Vector2i(45, 46))
	auto_bandage_state.combat.set_turn_order([bandage_actor.id, bandage_following.id, bandage_target.id, bandage_monster.id])
	auto_bandage_state.combat.set_character_bleeding(bandage_target.id, true)
	var auto_bandage_rng := ScriptedRng.new([0])
	var auto_bandage_result := rules.combat_flow.run_auto_turn(auto_bandage_state, _content([auto_definition]), bandage_actor.id, auto_bandage_rng)
	assert_true(auto_bandage_result.ok, "Auto Bandage resolves before movement or attack")
	assert_false(auto_bandage_state.combat.is_character_bleeding(bandage_target.id), "Auto Bandage clears the first party-ordered bleeding recipient")
	assert_equal(auto_bandage_rng.trace().map(func(entry: Dictionary) -> String: return entry["tag"]), ["combat.auto.%s.bandage-sound" % bandage_actor.id], "Auto Bandage preserves Castle's additional source-ordered sound draw")
	assert_true(auto_bandage_result.events.any(func(event: DomainEvent) -> bool: return event.kind == &"sound_requested" and event.payload.get("source") == "classic-combat-auto-bandage" and event.payload.get("soundId") == 10121), "Auto Bandage publishes its rolled follow-up sound after the Bandage cue")

	var macro_actor := _character("character.auto-turn-macro")
	macro_actor.set_ability_value(13, 100)
	var macro_flags := _ints(8)
	macro_flags[1] = 1
	var macro_definition := MonsterDefinition.new("monster.auto-turn-macro", 232, "Macro Nether", 1, 0, 1, 0, 0, macro_flags, _ints(8), _ints(6), _ints(3), [], [], [], [])
	macro_definition.can_summon = 1
	macro_definition.death_macro = 321
	var macro_monster := MonsterState.new("monster.auto-turn-macro.instance", macro_definition.id, macro_definition.name, 20, 20, 1)
	var macro_state := _state(macro_actor, macro_monster, "battle.auto-turn-macro")
	var macro_result := rules.combat_flow.run_auto_turn(macro_state, _content([macro_definition]), macro_actor.id, ScriptedRng.new([8192]))
	assert_true(macro_result.ok, "Auto stops successfully at a mandatory Turn Undead death-macro boundary")
	assert_equal(macro_state.combat.pending_spell_death_macro_id(), macro_monster.id, "Auto retains the exact mandatory death-macro subject")
	assert_true(macro_result.events.any(func(event: DomainEvent) -> bool: return event.kind == &"monster_death_macro_requested" and event.payload.get("combatantId") == macro_monster.id), "Auto publishes the mandatory interaction boundary without selecting another action")
	var macro_restored := GameState.from_data(JSON.parse_string(JSON.stringify(macro_state.to_data())))
	assert_not_null(macro_restored, "the mandatory Auto stop is a restorable combat boundary")
	if macro_restored != null:
		assert_equal([macro_restored.combat.pending_spell_death_macro_id(), macro_restored.combat.active_actor_id()], [macro_monster.id, macro_actor.id], "restore retains the Auto-issued death macro and exact issuing actor")

	var chain_first := _character("character.auto-chain-first")
	var chain_second := _character("character.auto-chain-second")
	var chain_manual := _character("character.auto-chain-manual")
	var chain_monster := MonsterState.new("monster.auto-chain.instance", auto_definition.id, auto_definition.name, 1000, 1000, 1)
	var chain_state := _state(chain_first, chain_monster, "battle.auto-chain")
	assert_true(chain_state.party.add_character(chain_second), "persistent Auto fixture adds its second enabled character")
	assert_true(chain_state.party.add_character(chain_manual), "persistent Auto fixture adds the manual stop character")
	chain_state.combat.battlefield.place_character(chain_second.id, Vector2i(46, 46))
	chain_state.combat.battlefield.place_character(chain_manual.id, Vector2i(44, 45))
	chain_state.combat.set_turn_order([chain_first.id, chain_second.id, chain_manual.id, chain_monster.id])
	chain_state.set_combat_auto(chain_first.id, true)
	chain_state.set_combat_auto(chain_second.id, true)
	var chain_result := rules.combat_flow.run_persistent_auto_characters(chain_state, _content([auto_definition]), RealmzRng.new(23))
	assert_true(chain_result.ok, "persistent Auto resolves adjacent enabled characters through the same rules-owned routine")
	assert_equal(chain_state.combat.active_actor_id(), chain_manual.id, "persistent Auto stops at the first living manual party actor")
	assert_equal(chain_result.events.filter(func(event: DomainEvent) -> bool: return event.kind == &"combat_auto_started").map(func(event: DomainEvent) -> String: return event.payload.get("actorId")), [chain_first.id, chain_second.id], "persistent Auto preserves turn-order activation identity")

	var limit_actor := _character("character.auto-limit")
	limit_actor.maximum_movement = 0
	limit_actor.movement = 0
	var limit_monster := MonsterState.new("monster.auto-limit.instance", auto_definition.id, auto_definition.name, 32_767, 32_767, 1)
	limit_monster.conditions.set_value(ConditionRules.HELPLESS, 1)
	var limit_state := _state(limit_actor, limit_monster, "battle.auto-limit")
	limit_state.combat.battlefield.place_monster(limit_monster.id, Vector2i(60, 60), 0)
	limit_state.set_combat_auto(limit_actor.id, true)
	var limit_rng := RealmzRng.new(29)
	assert_not_null(GameState.from_data(limit_state.to_data()), "the operation-limit fixture is a valid restorable transaction checkpoint")
	var limit_state_before := JSON.stringify(limit_state.to_data())
	var limit_rng_before := limit_rng.snapshot()
	var limit_result := rules.combat_flow.run_persistent_auto_characters(limit_state, _content([auto_definition]), limit_rng)
	assert_false(limit_result.ok, "persistent Auto fails explicitly at its bounded activation limit")
...
func _test_bounded_classic_undo_and_restore() -> void:
	...
	if restored != null:
		...
		assert_equal(request.body.to_data().get("undo"), {"enabled": true, "reason": ""}, "the typed battle request exposes core-owned Undo availability")
