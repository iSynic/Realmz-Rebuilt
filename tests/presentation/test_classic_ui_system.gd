...
func _test_classic_choice_context() -> void:
	...
	var hero_spell := SpellDefinition.new("classic.spell.1107", 1107, "Magic Darts", "A compact bolt of magical force."); var mage_spell := SpellDefinition.new("classic.spell.1306", 1306, "Brimstones", "Burning stones strike the target."); var hero_view := CharacterView.new(CharacterState.new("hero", "Hero", 8, 10)); hero_view.spells = [SpellView.new(hero_spell)]; var mage_view := CharacterView.new(CharacterState.new("mage", "Mage", 8, 10)); mage_view.spells = [SpellView.new(mage_spell)]; var encounter_view := GameView.new(1, true, null); encounter_view.party_members = [hero_view, mage_view]; var encounter := EncounterInteraction.new(); encounter.configure(null, encounter_view); var encounter_responses: Array[Dictionary] = []; encounter.response_body_submitted.connect(func(body: InteractionResponse.ComplexEncounterBody) -> void: encounter_responses.append(body.to_data())); var encounter_request := ClassicUiFixtureGallery.request_for(InteractionRequest.WORD_AND_ACTION); encounter.build(encounter_request)
	...
	var item_command := encounter.find_child("EncounterCommandItem", true, false) as ClassicBitmapButton; item_command.command_requested.emit(&"item"); var item_is_standard := encounter.find_child("EncounterCatalogList", true, false) != null and (encounter.find_child("EncounterCatalogList", true, false) as Control).theme_type_variation == &"ClassicItemLedger" and _buttons_in(encounter).any(func(button: Button) -> bool: return button.text == "Torch" and button.theme_type_variation == &"ClassicItemLedgerButton") and encounter.find_child("EncounterCatalogNextCharacter", true, false) != null and encounter.handle_back(); var spell_command := encounter.find_child("EncounterCommandSpell", true, false) as ClassicBitmapButton; spell_command.command_requested.emit(&"spell"); var caster_picker := encounter.find_child("SpellCharacterSelector", true, false) as OptionButton; var caster_count := caster_picker.item_count; caster_picker.select(1); caster_picker.item_selected.emit(1); await (Engine.get_main_loop() as SceneTree).process_frame; caster_picker = encounter.find_child("SpellCharacterSelector", true, false) as OptionButton; var level_three := encounter.find_child("SpellLevel3", true, false) as Button; var spell_choose := encounter.find_child("EncounterSpellChoose", true, false) as ClassicBitmapButton; spell_choose.pressed.emit(); var spell_is_standard := caster_count == 2 and caster_picker.selected == 1 and encounter.find_child("ClassicSpellbookWorkspace", true, false) != null and encounter.find_child("KnownSpellList", true, false) != null and level_three.button_pressed and _buttons_in(encounter).any(func(button: Button) -> bool: return button.text.begins_with("Brimstones")) and encounter_responses[-1] == {"action": "spell", "classicSpellId": 1306, "characterId": "mage"} and encounter.handle_back(); assert_true(item_is_standard and spell_is_standard, "Encounter Items reuse the standard item ledger while Encounter Spells reuse the standard caster-aware spellbook and return its exact selected caster/spell pair")
